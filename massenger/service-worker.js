/* MASSERGER Service Worker v7.30
   ...
   • v7.04: + Notification click handler (focus หน้าแอปเดิม)
   ============================================================ */
const CACHE_VERSION = 'masserger-v7.30';
...
// v7.04: 🔔 Notification click handler — focus หน้าแอปเดิม (ไม่เปิด tab ใหม่)
self.addEventListener('notificationclick', (event) => {
  event.notification.close();
  event.waitUntil(
    (async () => {
      const wins = await self.clients.matchAll({ type: 'window', includeUncontrolled: true });
      // หาแอปที่เปิดอยู่แล้ว → focus
      for (const w of wins) {
        if (w.url && w.url.includes(self.location.origin)) {
          try { return await w.focus(); } catch(_) {}
        }
      }
      // ไม่เจอ → เปิดใหม่
