...
async function loadMoreTasks(){
  ...
  try{
    const BATCH=30;
    // ใช้ _baseFetchedCount เป็น offset (แยกจาก S.tasks.length) — archive + realtime inserts ไม่ทำให้เลื่อนผิด
...
async function loadTasks(force=false){
  ...
  if(isFirstLoad){
    // ── รอบแรก: 30 รายการล่าสุดเท่านั้น → UI เปิดเร็วสุดบน mobile ──
    // ไม่มี double-fetch bg แล้ว: เพิ่มเติมให้โหลดผ่าน infinite scroll (loadMoreTasks) เมื่อใกล้ bottom
    // หรือ realtime WS สำหรับ updates → ประหยัด network มากขึ้น
    if(force){_noMoreBase=false;_archiveLoaded=false;_archiveOffset=0;_baseFetchedCount=0;}
    const first=await sbGet("msg_tasks",baseFilter+"&limit=30");
    if(first.length>0||S.tasks.length===0){S.tasks=first;}
    _baseFetchedCount=first.length; // base offset = จำนวน rows ที่โหลดมาแล้ว
    // ถ้าได้กลับมาน้อยกว่า 30 = DB มีแค่นี้ ไม่ต้อง loadMore อีก
    if(first.length<30)_noMoreBase=true;
    invalidateDerived();_taskIndex=null;VL._fil=null;
    _pruneCardCache();
    ...
    // ── Poll ปกติ: โหลด 30 (ลดจาก 80 — realtime เป็นหลัก poll เป็น fallback) ──
    const fetched=await sbGet("msg_tasks",baseFilter+"&limit=30");
    if(fetched.length>0||S.tasks.length===0||force){S.tasks=fetched;_baseFetchedCount=fetched.length;}
    if(fetched.length<30)_noMoreBase=true;
    invalidateDerived();_taskIndex=null;VL._fil=null;
...
function renderSidebar(cnt,pending,isSA,isAdmin){
  ...
  return '<div style="padding:14px 18px 12px;border-bottom:1px solid #2a3347"><div style="font-size:9px;letter-spacing:2px;color:#06C755;font-weight:700;font-family:monospace">MSG SYSTEM</div><div style="font-size:14px;font-weight:800;color:#fff;margin-top:2px">ระบบสั่งงานแมส</div></div>'
    ...
    +'<button onclick="doLogout()" style="margin-top:8px;background:none;border:1px solid #313d52;border-radius:6px;color:#64748b;font-size:11px;padding:4px 10px;cursor:pointer;width:100%">ออกจากระบบ</button>'
    +'</div>';
}
...
window.addEventListener("focus",()=>{
  ...
  const now=Date.now();
  if(now-_focusThrottle<15000)return; // throttle 15 วิ (brief: prevent loadTasks on every focus)
  _focusThrottle=now;
  _notifDismissed=false;
  // ── Smart refresh: skip loadTasks ถ้า realtime live + fetch ล่าสุด < 15 วิ ──
  // (realtime คอย sync อยู่แล้ว ไม่ต้อง hard reload)
  if(!document.hidden){
    const isLive=S.realtimeStatus==="live";
    const recent=(now-_lastFetch)<15000;
    if(!isLive||!recent){
      loadTasks(true); // reload เฉพาะเมื่อ realtime หลุด หรือ fetch เก่าเกิน 15 วิ
    }
  }
  setTimeout(checkAndShowNotif,800);
