...
function renderFinishModal(){
  ...
  return '<div class="modal-overlay" onclick="if(event.target===this)setState({finishTaskId:null,showRecip:false})">'
    ...
    +'<button onclick="setState({finishTaskId:null,showRecip:false})" style="background:none;border:1px solid #313d52;border-radius:8px;color:#64748b;font-size:18px;padding:2px 8px;cursor:pointer;line-height:1">×</button>'
    +'</div>'
...
// ── Adaptive compression: บีบซ้ำจนกว่าจะ ≤ 800KB (รับรูปกล้องมือถือใหญ่ๆ ได้) ──
async function _compressAdaptive(file){
  // รอบ 1: คุณภาพปกติ
  let r=await compressImage(file,900,0.65);
  if(r.file.size<=800*1024)return r;
  // รอบ 2: ลด quality
  r=await compressImage(file,900,0.5);
  if(r.file.size<=800*1024)return r;
  // รอบ 3: ลด resolution + quality
  r=await compressImage(file,720,0.45);
  if(r.file.size<=800*1024)return r;
  // รอบ 4: บีบหนักสุด
  return await compressImage(file,640,0.4);
}
...
async function uploadFinishImage(id,file,customTime){
  ...
  try{
    // Force .jpg extension (compressImage บีบเป็น JPEG เสมอ — ไม่ใช้ ext จาก file.name)
    ...
    const res=await fetch(SUPA_URL+"/storage/v1/object/msg-images/"+fname,{
      ...
      headers:{
        ...
        "x-upsert":"true",   // ← Allow overwrite (กัน 400 ถ้าชื่อซ้ำ)
        "Cache-Control":"max-age=31536000"
      },
      body:file
    });
    if(res.ok){
      // อัปโหลดสำเร็จ → ใช้ public URL
      finishTask(id,SUPA_URL+"/storage/v1/object/public/msg-images/"+fname,customTime);
    }else{
      // ── อัปโหลดไม่สำเร็จ → ห้าม fallback base64 ──
      const errText=await res.text().catch(()=>res.statusText);
      console.warn("upload failed",res.status,errText);
      showToast("❌ อัปโหลดรูปไม่สำเร็จ ("+res.status+") กรุณาลองใหม่","warn");
    }
  }catch(err){
    console.warn("upload error",err);
    showToast("❌ อัปโหลดรูปไม่สำเร็จ — ตรวจสอบเน็ต/ลองใหม่","warn");
  }
}
...
async function uploadSignatureImage(id,dataURL,customTime){
  ...
  try{
    // แปลง DataURL → Blob (มาตรฐานเดียวกับรูป upload)
    ...
    if(res.ok){
      // ลายเซ็น upload สำเร็จ → ใช้ public URL ลงงาน (เหมือนรูป) ✅
      finishTask(id,SUPA_URL+"/storage/v1/object/public/msg-images/"+fname,customTime);
    }else{
      const errText=await res.text().catch(()=>res.statusText);
      console.warn("signature upload failed",res.status,errText);
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ ("+res.status+") กรุณาลองใหม่","warn");
    }
  }catch(err){
    console.warn("signature upload error",err);
