...
console.log("%c[NJ Dispatch BUILD]","color:#06C755;font-weight:bold","1775622052-closefix","| Loaded at",new Date().toLocaleTimeString());

// ── EARLY GLOBAL: __closeDetail — single source of truth ──
// ห้ามประกาศซ้ำที่อื่น (ใช้ตัวนี้ตัวเดียว)
window.__closeDetail = function(){
  try{
    if(window.S) S.selId = null;
    const m = document.getElementById("zone-modal");
    if(m) m.innerHTML = "";
    if(typeof render === "function") render();
  }catch(e){
    console.error("close detail failed", e);
  }
};

// ── Single delegated click — รับทุกปุ่มปิดที่มี data-close="detail" หรือ class กำหนด ──
if(typeof document!=="undefined"){
  document.addEventListener("click", function(e){
    const btn = e.target && e.target.closest && e.target.closest('[data-close="detail"], .modal-x, .detail-close, .btn-close');
    if(!btn) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  document.addEventListener("keydown", function(e){
    if((e.key==="Escape"||e.keyCode===27) && window.S && window.S.selId){
      window.__closeDetail();
    }
  });
...
function renderDetail(){
  ...
  return '<div class="modal-overlay" onclick="if(event.target===this)setState({selId:null})">'
    ...
    +'<button type="button" data-close="detail" onclick="window.__closeDetail()" style="background:none;border:1px solid #313d52;border-radius:8px;color:#94a3b8;font-size:22px;padding:4px 12px;cursor:pointer;line-height:1;font-weight:700">×</button>'
    +'</div></div>'
