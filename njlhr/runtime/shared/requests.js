...
/* ============ [RUN-123] ส่งข้อมูลเพิ่มเติม (ตอบกลับ "ขอข้อมูลเพิ่ม") ============
   ขอบเขต: คำขอลาเท่านั้น · ไม่แตะ OT / ลงชื่อย้อนหลัง
   กฎไฟล์แนบใช้ชุดเดียวกับฟอร์มขอลาเดิม (.pdf .doc .docx .xls .xlsx image/* · 5MB)
   Upload ใช้ sbUploadLeaveFile() ของเดิม — ไม่สร้าง Upload Flow ใหม่
   Contract p_files คงรูปเดิมของ form.js: {name, url=storage path, size} */
var LVIR_ACCEPT=".pdf,.doc,.docx,.xls,.xlsx,image/*";
var LVIR_MAX=5*1024*1024;
var lvirBusy=false;
function lvirKind(f){var n=String(f.name||"").toLowerCase();
...
function lvirSize(n){var v=Number(n);if(!isFinite(v)||v<=0)return"-";
  return v>=1048576?Math.round(v/104857.6)/10+" MB":Math.max(1,Math.round(v/1024))+" KB"}
/* ใบลานี้กำลังรอให้ "ผู้ยื่น" ส่งข้อมูลเพิ่มเติมอยู่หรือไม่
   ใช้ ui_status จาก RPC เท่านั้น (Server เป็นเจ้าของความจริง) */
function lvNeedInfo(r){
  if(!r)return false;
  return String(r.ui_status||"").toUpperCase()==="NEED_MORE_INFO"
      && String(r.status||"").toUpperCase()==="PENDING"}
/* คำถามล่าสุดของผู้อนุมัติ — ผู้ขอ · ข้อความ · วันที่ */
function lvInfoAsk(r){
  var ev=apLastEvent(r,["INFO"]);
  if(!ev)return null;
  return{who:apEvWho(ev),note:apEvNote(ev),at:apAt(apEvAtRaw(ev))}}
function lvirAskHTML(a){
  if(!a)return"";
  return'<div class="ap-sum ap-red lvir-ask">'+
    '<div class="ap-sum-h">🔴 <b>ต้องส่งข้อมูลเพิ่มเติม</b></div>'+
    '<div class="ap-sum-r"><span>ผู้ขอข้อมูลเพิ่ม</span><b>'+esc(a.who||"—")+"</b></div>"+
    '<div class="ap-sum-r"><span>ข้อมูลที่ต้องการ</span><b>'+esc(a.note||"—")+"</b></div>"+
    '<div class="ap-sum-r"><span>วันที่ขอ</span><b>'+esc(a.at||"—")+"</b></div></div>"}
/* เปิด Modal ส่งข้อมูลเพิ่มเติม
   id      = leave_id เดิม (ห้ามสร้างคำขอใหม่)
   onDone  = callback หลังส่งสำเร็จ (ให้หน้าที่เรียกรีเฟรชเอง) */
function lvInfoReplyOpen(id,onDone){
  if(!id)return;
  var files=[];
  openModal("ส่งข้อมูลเพิ่มเติม",
    '<div class="muted"><span class="spinner"></span> กำลังโหลด…</div>',
    '<button type="button" class="btn btn-ghost" id="lvir-cancel">ยกเลิก</button>',
    {wide:true,fullMobile:true});
  var cx=document.getElementById("lvir-cancel");if(cx)cx.onclick=closeModal;
  sbRpc("njhr_leave_detail",{p_token:sbToken(),p_leave_id:id}).then(function(d){
    var body=document.querySelector("#modal-root .modal-body");
    var foot=document.querySelector("#modal-root .modal-foot")||
             document.querySelector("#modal-root .modal-footer");
    if(!body)return;
    if(!d||!lvNeedInfo(d)){
      body.innerHTML='<div class="form-error">คำขอนี้ไม่ได้อยู่ระหว่างรอข้อมูลเพิ่มเติม</div>';
      return}
    body.innerHTML=lvirAskHTML(lvInfoAsk(d))+
      '<label class="field"><span>ข้อความตอบกลับ <i class="req">*</i></span>'+
      '<textarea id="lvir-note" rows="3" placeholder="พิมพ์ข้อมูลที่ผู้อนุมัติขอเพิ่ม"></textarea></label>'+
      '<div class="field"><span>เอกสารแนบ <small class="muted">ไม่บังคับ · ไฟล์ละไม่เกิน 5MB</small></span>'+
      '<div class="otj-flist" id="lvir-files"></div>'+
      '<label class="btn btn-ghost btn-sm otj-attach">'+icon("plus")+" เพิ่มไฟล์แนบ"+
      '<input type="file" id="lvir-file" hidden multiple accept="'+LVIR_ACCEPT+'"></label></div>'+
      '<div class="form-error" id="lvir-err" role="alert" style="white-space:pre-line"></div>';
    if(foot)foot.insertAdjacentHTML("beforeend",
      '<button type="button" class="btn btn-primary" id="lvir-send">ส่งข้อมูลเพิ่มเติม</button>');
    function renderFiles(){
      var box=document.getElementById("lvir-files");if(!box)return;
      box.innerHTML=files.length?files.map(function(f,i){
        return'<div class="otj-file"><span class="otj-fname">'+icon("fileText","ic-sm")+" "+esc(f.name)+
          '<small class="muted"> · '+esc(f.kind)+" · "+lvirSize(f.size)+"</small></span>"+
          '<button type="button" class="btn-icon ic-red" data-lvirdel="'+i+'" aria-label="ลบไฟล์">'+
          icon("x")+"</button></div>"}).join(""):'<small class="muted">ยังไม่มีไฟล์แนบ</small>';
      Array.prototype.forEach.call(box.querySelectorAll("[data-lvirdel]"),function(b){
        b.onclick=function(){files.splice(parseInt(b.getAttribute("data-lvirdel"),10),1);renderFiles()}})}
    renderFiles();
    var fi=document.getElementById("lvir-file");
    if(fi)fi.onchange=function(){
      var pend=Array.prototype.slice.call(this.files),err=document.getElementById("lvir-err");
      err.textContent="";
      pend.forEach(function(file){
        if(file.size>LVIR_MAX){err.textContent="ไฟล์ "+file.name+" เกิน 5MB — ไม่แนบ";return}
        if(files.some(function(x){return x.name===file.name&&x.size===file.size}))return;
        files.push({file:file,name:file.name,size:file.size,kind:lvirKind(file)})});
      this.value="";renderFiles()};
    var send=document.getElementById("lvir-send");
    if(send)send.onclick=function(){
      var btn=this,err=document.getElementById("lvir-err");
      var note=String((document.getElementById("lvir-note")||{}).value||"").trim();
      err.textContent="";
      if(lvirBusy||btn.disabled)return;
      if(!note){err.textContent="กรุณากรอกข้อความตอบกลับ";return}
      lvirBusy=true;btn.disabled=true;
      btn.innerHTML='<span class="spinner"></span> กำลังส่ง…';
      function fail(m){lvirBusy=false;btn.disabled=false;
        btn.textContent="ส่งข้อมูลเพิ่มเติม";err.textContent=m}
      var up=S.sbUploadLeaveFile;
      Promise.all(files.map(function(f){return up(f.file)})).then(function(ups){
        return sbRpc("njhr_leave_info_reply",{p_token:sbToken(),p_leave_id:id,p_note:note,
          p_files:ups.map(function(f){return{name:f.name,url:f.path,size:f.size}})})
      }).then(function(){
        lvirBusy=false;closeModal();
        toast("ส่งข้อมูลเพิ่มเติมแล้ว รอผู้อนุมัติตรวจสอบ","success");
        if(typeof onDone==="function")onDone()
      })["catch"](function(er){fail(er&&er.message||"ส่งข้อมูลเพิ่มเติมไม่สำเร็จ")})}
  })["catch"](function(er){
    var body=document.querySelector("#modal-root .modal-body");
    if(body)body.innerHTML='<div class="form-error">'+
      esc(er&&er.message||"โหลดรายละเอียดไม่สำเร็จ")+"</div>"})}
/* ปุ่มมาตรฐาน — ใช้ชุดเดียวกันทั้ง Desktop / Mobile / Modal */
function lvInfoReplyBtn(r,cls){
  if(!lvNeedInfo(r))return"";
  return'<button type="button" class="btn btn-warn btn-sm lvir-btn'+(cls?" "+cls:"")+
    '" data-lvreply="'+esc(String(r.id))+'">'+icon("edit","ic-sm")+" ตอบกลับ / แนบไฟล์เพิ่ม</button>"}
NJHR.compat.scope.lvNeedInfo=lvNeedInfo;NJHR.compat.scope.lvInfoAsk=lvInfoAsk;NJHR.compat.scope.lvInfoReplyOpen=lvInfoReplyOpen;NJHR.compat.scope.lvInfoReplyBtn=lvInfoReplyBtn;NJHR.compat.scope.apStatus=apStatus;NJHR.compat.scope.apChip=apChip;NJHR.compat.scope.apSummaryHTML=apSummaryHTML;NJHR.compat.scope.apStepsHTML=apStepsHTML;NJHR.compat.scope.apPerson=apPerson;NJHR.compat.scope.RQ_CARDS=RQ_CARDS;NJHR.compat.scope.bindFileButtons=bindFileButtons;NJHR.compat.scope.epNum=epNum;NJHR.compat.scope.lvCode=lvCode;NJHR.compat.scope.lvIsVacation=lvIsVacation;NJHR.compat.scope.lvRemainDays=lvRemainDays;NJHR.compat.scope.lvShowTimeline=lvShowTimeline;NJHR.compat.scope.lvUsedDays=lvUsedDays;NJHR.compat.scope.otDMY=otDMY;NJHR.compat.scope.otJobEndDate=otJobEndDate;NJHR.compat.scope.otJobHours=otJobHours;NJHR.compat.scope.otJobsHTML=otJobsHTML;NJHR.compat.scope.otJobsOf=otJobsOf;NJHR.compat.scope.otSpan=otSpan;NJHR.compat.scope.reqInfoBar=reqInfoBar;NJHR.compat.scope.rqPick=rqPick;NJHR.compat.scope.rqState=rqState;NJHR.compat.scope.showTimeline=showTimeline})();
