  ...
  function dashAdmin(el) {
    ...
    function kpi(route, label, val, cls) {
      return '<a href="' + route + '" class="kpi ' + (cls || '') + '"><small>' + label + '</small><b>' + val + '</b></a>';
    }
    var recentLeaves = db.leaves.slice().sort(function (a, b) { return b.createdAt.localeCompare(a.createdAt); }).slice(0, 5);
    ...
    el.innerHTML =
      '<div class="kpi-grid">' +
      kpi('#/employees', 'พนักงานทั้งหมด', actives.length) +
      kpi('#/reports', 'เข้างานวันนี้', todayAtt.length, 'k-green') +
      kpi('#/reports', 'มาสายวันนี้', late, 'k-yellow') +
      kpi('#/calendar', 'ลางานวันนี้', onLeaveToday, 'k-blue') +
      kpi('#/reports', 'ขาดงานวันนี้', absent, 'k-red') +
      kpi('#/ot', 'OT วันนี้', otToday) +
      kpi('#/approvals', 'รายการรออนุมัติ', pc, pc ? 'k-red' : '') +
      kpi('#/payroll', 'เงินเดือน ' + TH_MONTHS[pr.month - 1].slice(0, 3) + '.', stMap[pr.status] ? stMap[pr.status][0] : pr.status) +
      '</div>' +
      '<div class="dash-cols">' +
      '<div class="col">' +
      '  <div class="card"><div class="card-head"><h3>คำขอลาล่าสุด</h3><a class="link" href="#/approvals">ดูทั้งหมด</a></div>' +
      (recentLeaves.length ? '<div class="list">' + recentLeaves.map(function (l) {
        var lt = leaveType(l.typeId);
        return '<div class="list-row">' + avatarHTML(empName(l.empId), 36) +
          '<div class="grow"><b>' + esc(empName(l.empId)) + '</b><small>' + esc(lt.name) + ' · ' + fmtDate(l.startDate) + (l.endDate !== l.startDate ? ' – ' + fmtDate(l.endDate) : '') + '</small></div>' +
          statusBadge(l.status) + '</div>';
      }).join('') + '</div>' : emptyState('ยังไม่มีคำขอลา')) + '</div>' +
      ...
      '<div class="list">' + db.announcements.filter(function (a) { return a.active; }).slice(0, 3).map(function (a) {
        return '<div class="list-row"><div class="grow"><b>' + (a.pinned ? icon('pin', 'ic-sm ic-red') + ' ' : '') + esc(a.title) + '</b><small>' + fmtDate(a.date) + ' · ' + esc(a.by) + '</small></div></div>';
      }).join('') + '</div></div>' +
    ...
    setTimeout(function () {
      ...
      sbRpcList('njhr_notify_list', { p_token: sbToken(), p_limit: 3, p_offset: 0 }).then(function (rows) {
        ...
        box.innerHTML = rows.length ? '<div class="list">' + rows.map(function (n) {
          return '<a class="list-row" href="' + esc(n.link) + '"><div class="grow"><b>' + esc(n.title) + '</b>' +
