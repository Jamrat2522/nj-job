  ...
  function asRenderWfList(el) {
    ...
    box.innerHTML = '<div class="wf-sets">' + asWfs.map(function (w) {
      ...
      return '<div class="wf-set' + wfStat + (open ? ' on' : '') + '" data-as-wf="' + esc(w.workflow_id) + '">' +
        '<div class="wf-set-head"><b>' + esc(asWfTitle(w)) + '</b>' +
        (w.request_type === 'BOTH'
          ? '<span class="badge badge-ok">ลา + OT</span>'
          : '<span class="badge badge-mut">' + (w.request_type === 'OT' ? 'การขอ OT' : 'การลางาน') + '</span>') +
        ...
        '<button class="btn-icon" data-as-wfedit="' + esc(w.workflow_id) + '" aria-label="แก้ไข / ตั้งขั้นอนุมัติ" title="แก้ไข / ตั้งขั้นอนุมัติ">' + icon('edit') + '</button>' +
        '<button class="btn-icon ic-red" data-as-wfdel="' + esc(w.workflow_id) + '" aria-label="ลบชุด">' + icon('x') + '</button></div>' +
        (open
          ? '<div class="wf-set-depts">' + asScopeChips(w) + '</div>' +
            // Timeline แนวนอน: ผู้ยื่น → ขั้นที่ 1 … → สำเร็จ (เติมข้อมูลจริงหลังโหลด)
            '<div class="wf-set-tl" id="wftl-' + esc(w.workflow_id) + '">' +
            (w.step_count
              ? '<span class="wf-tl-load">กำลังโหลดขั้นอนุมัติ…</span>'
              : '<span class="wf-tl-none">ยังไม่มีขั้นอนุมัติ — กดแก้ไขเพื่อเพิ่ม</span>') + '</div>'
          : '') +
        '</div>';
    }).join('') + '</div>';
    asLoadTimelines();
