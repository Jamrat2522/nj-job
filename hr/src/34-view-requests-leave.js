  ...
  function viewRequests(el) {
    ...
    el.innerHTML =
      ...
      '<div class="form-error rq-err" id="rq-err" role="alert"></div>';
    rqRenderMenu(el);

    if (!sbReady()) {
      document.getElementById('rq-err').textContent = 'ยังไม่ได้ตั้งค่าการเชื่อมต่อ Supabase';
      return;
    }
  ...
  function rqRenderBal(el) {
    ...
    d.className = 'rq-bal';
    d.innerHTML = '<div class="rq-bal-h" style="grid-column:1/-1">สรุปสิทธิ์การลา</div>' +
      RQ_CARDS.map(function (c) {
        ...
        return '<div class="rq-bal-i ' + c.cls + '"><div class="rq-bal-ic">' + c.em + '</div>' +
          ...
          '<b>' + (r ? (Math.round(left * 10) / 10) : '—') + '</b>' +
          '<small>' + (r ? 'วัน' : 'ไม่มีสิทธิ์') + '</small></div>';
      }).join('');
    box.replaceWith(d);
  }
  ...
  function rqRenderMenu(el) {
    ...
    box.innerHTML = items.map(function (it) {
      return '<button type="button" class="rq-card" data-go="' + it[0] + '">' +
  ...
  function viewLeave(el) {
    ...
    el.innerHTML =
      '<div class="bal-cards" id="lv-bal"></div>' +
  ...
  function lvRenderList(el, rows) {
    ...
    list.innerHTML = rows.length ? rows.map(function (l) { return leaveCard(l, true); }).join('')
  ...
  function leaveCard(l, mineView) {
    ...
    var sub = (l.department || (l.emp_code ? '' : dept((currentEmp() || {}).deptId)) || '—') + ' · ' + lvCode(l.id);
