  ...
  function rptBalHead(types) {
    ...
    types.forEach(function (c) {
      var n = lvType(c).name;
      h = h.concat([n + ' · สิทธิ์ต่อปี', n + ' · ยกมา', n + ' · ใช้แล้ว', n + ' · รออนุมัติ', n + ' · คงเหลือ']);
    });
    return h;
  }
  ...
  function rptCells(r) {
    ...
    return [rptDateBE(r.date), r.title, r.name, r.dept || '-', r.in, r.out, late ? 'มาสาย' : r.status,
  ...
  function rptLoadSb(el, seq) {
    ...
    if (isAtt) {
      rptFetchAtt().then(function (rows) {
        ...
        var cells = rptAttRows.map(rptCells);
