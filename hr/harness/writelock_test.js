...
(async()=>{
 ...
 await c.route('**/rest/v1/rpc/*',route=>{const fn=route.request().url().split('/rpc/')[1].split('?')[0];
  ...
  hits[fn]=(hits[fn]||0)+1;
  const slow = fn==='njhr_dept_save' || fn==='njhr_dept_delete';
  setTimeout(()=>route.fulfill({status:200,contentType:'application/json',headers:{'access-control-allow-origin':'*'},
    body:JSON.stringify(F.respond(fn,bd))}), slow?1500:0);});
 ...
 await p.evaluate(()=>{location.hash='#/departments';});await p.waitForTimeout(2500);
 // ยิงปุ่มลบแผนกเดียวกันรัว 4 ครั้ง (payload เหมือนกัน)
 hits['njhr_dept_delete']=0;
 await p.evaluate(()=>{const b=document.querySelector('[data-dp-del]'); for(let i=0;i<4;i++) b.click();});
 await p.waitForTimeout(3000);
 console.log(label);
 console.log('  กดปุ่มลบ "แผนกเดียวกัน" 4 ครั้ง → njhr_dept_delete ยิง '+(hits['njhr_dept_delete']||0)+' ครั้ง');
 // ยิงปุ่มลบ "คนละแผนก" พร้อมกัน 3 ปุ่ม (payload ต่างกัน) — ต้องผ่านทั้งหมด
 hits['njhr_dept_delete']=0;
 const n=await p.evaluate(()=>{const bs=document.querySelectorAll('[data-dp-del]');let k=0;
   for(let i=0;i<Math.min(3,bs.length);i++){bs[i].click();k++;} return k;});
 await p.waitForTimeout(3000);
 console.log('  กดปุ่มลบ "คนละแผนก" '+n+' ปุ่มพร้อมกัน → ยิง '+(hits['njhr_dept_delete']||0)+' ครั้ง');
