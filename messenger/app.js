...
function computeDerived(){
  ...
  // 🔒 Staff/User เห็นเฉพาะงานที่ตัวเองสร้าง
  // Admin/Superadmin เห็นทั้งหมด
  const tasks=isAdmin
    ?S.tasks
    :S.tasks.filter(t=>t.created_by===me);

  const wait=tasks.filter(t=>t.status==="wait");
  const going=tasks.filter(t=>t.status==="going");
  const done=tasks.filter(t=>t.status==="done");
  const fil=S.filter==="all"?tasks:S.filter==="wait"?wait:S.filter==="going"?going:done;
  _derived={
    tasks,wait,going,done,fil,
    cnt:{all:tasks.length,wait:wait.length,going:going.length,done:done.length},
...
function invalidateDerived(){_derived=null;}
...
// สิทธิ์จบงาน: messenger เสมอ + staff/user ที่ superadmin เปิดให้
...
async function getUsers(){
  ...
  if(_usersCache&&Date.now()-_usersCacheTime<60000)return _usersCache;
  try{
...
async function getJobNo(date,tasks){
  // เช็คจากข้อมูลใน memory ก่อน
  const same=tasks.filter(t=>t.date===date);
  let seq=same.length+1;
  let jobNo=date.replace(/-/g,"")+"-"+pad(seq);
  // ตรวจสอบซ้ำกับ DB จริง (ป้องกันกรณีหลายคนสร้างพร้อมกัน)
  if(!DEMO){
    while(true){
      const exists=await sbGet("msg_tasks","job_number=eq."+jobNo+"&limit=1");
      if(!exists||exists.length===0)break;
      seq++;
      jobNo=date.replace(/-/g,"")+"-"+pad(seq);
    }
  }
  return jobNo;
}
...
const S={
  ...
  editTaskId:null,
  realtimeStatus:"connecting", // connecting / live / offline
  gpsTracking:false,gpsTaskId:null,gpsLat:null,gpsLng:null,gpsPermission:"unknown",
...
function setState(obj){
  // invalidate derived เฉพาะเมื่อ tasks/users/filter เปลี่ยน
  if("tasks" in obj||"users" in obj||"filter" in obj)invalidateDerived();
  // reset listPage เมื่อ filter เปลี่ยน
  if("filter" in obj&&obj.filter!==S.filter)obj.listPage=1;
  Object.assign(S,obj);
  render();
}
...
async function loadTasks(force=false){
  // ถ้าเพิ่ง fetch ไปไม่ถึง 10 วิ ไม่ต้อง fetch ซ้ำ (ยกเว้น force)
  const now=Date.now();
  if(!force&&now-_lastFetch<10000)return;
  _lastFetch=now;
  S.loading=S.tasks.length===0; // แสดง loading เฉพาะครั้งแรก
  if(S.loading)render();
  if(DEMO){
    S.tasks=[
  ...
  // ดึงเฉพาะ 200 รายการล่าสุด + เลือก column ที่จำเป็น
  S.tasks=await sbGet("msg_tasks","order=created_at.desc&limit=200");
  invalidateDerived();
...
async function createTask(){
  ...
  try{
    const jobNo=await getJobNo(date,S.tasks);

    // 🔢 เช็คซ้ำก่อน insert
    if(!DEMO){
      const dup=await sbGet("msg_tasks","job_number=eq."+jobNo+"&limit=1");
      if(dup&&dup.length>0){
        showToast("⚠️ JOB "+jobNo+" ซ้ำ กรุณาลองใหม่","warn");
        setState({saving:false});_creatingTask=false;return;
      }
    }
    ...
    const task={id:uid(),job_number:jobNo,job_nj:jobnj,company,type,detail,place,date,
      ...
      status:"wait",created_by:S.currentUser?.username||"ผู้ใช้",
      image_url:null,messenger_name:null,start_time:null,end_time:null,recipient:null};

    if(!DEMO){await sbInsert("msg_tasks",task);await loadTasks(true);}
    else S.tasks=[task,...S.tasks];

    setState({saving:false,page:"list",fCompany:"",fJobNJ:"",fDetail:"",fPlace:"",fPickup:"",fDeliver:"",fInspectDate:"",fPayDate:"",fFile:null,fPreview:null,fDate:todayStr()});
...
async function acceptTask(id){
  ...
  setState({selId:null,gpsTaskId:id});
  await requestGpsAndStart(id,m);
  showToast("🏃 "+m+" รับงานแล้ว! "+t,"warn");
}

async function cancelTask(id){

// ── Edit Task (เฉพาะสถานะรอรับ) ──
function openEditTask(id){
  const t=S.tasks.find(x=>x.id===id);if(!t||t.status!=="wait")return;
...
async function saveEditTask(){
  ...
  const updates={status:"wait",start_time:null,messenger_name:null};
  if(!DEMO)await sbUpdate("msg_tasks",id,updates);
  S.tasks=S.tasks.map(x=>x.id===id?{...x,...updates}:x);
  // หยุด GPS และลบตำแหน่ง
  stopGpsTracking();
  if(!DEMO)await clearMessengerLocation(S.currentUser?.username||"");
  invalidateDerived();
  setState({selId:null,gpsTracking:false,gpsTaskId:null,showRecip:false});
  showToast("↩ ยกเลิกแล้ว — งานกลับสู่รอรับ","ok");
}

async function requestGpsAndStart(taskId,messengerName){
  if(!navigator.geolocation){
    setState({gpsPermission:"denied"});
    showGpsAlert("unsupported");
    return;
  }
  // เช็ค permission state ก่อน
  if(navigator.permissions){
    try{
      const perm=await navigator.permissions.query({name:"geolocation"});
      setState({gpsPermission:perm.state});
      // ฟัง permission change อัตโนมัติ
      perm.onchange=()=>{
        setState({gpsPermission:perm.state});
        if(perm.state==="denied"&&S.gpsTracking){
          stopGpsTracking();
          showToast("⛔ GPS ถูกปิดกั้น — หยุด tracking","warn");
        }else if(perm.state==="granted"&&S.gpsTaskId&&!S.gpsTracking){
          startGpsTracking(S.gpsTaskId,S.currentUser?.username||"");
        }
      };
      if(perm.state==="denied"){showGpsAlert("denied");return;}
    }catch{}
  }
  // ขอ permission จริง (popup จะขึ้นถ้ายังไม่เคยอนุญาต)
  startGpsTracking(taskId,messengerName);
}

function showGpsAlert(type){
  const msgs={
    unsupported:"⚠️ อุปกรณ์ไม่รองรับ GPS",
    denied:"⛔ GPS ถูกปิดกั้น\nกรุณาไปเปิดที่:\nSettings → Safari/Chrome → Location → Allow",
  };
  showToast(msgs[type]||"⚠️ GPS error","warn");
}
async function finishTask(id,imgUrl,customTime){
  ...
  if(remark)updates.finish_remark=remark; // บันทึกหมายเหตุ
  if(!DEMO)await sbUpdate("msg_tasks",id,updates);
  S.tasks=S.tasks.map(x=>x.id===id?{...x,...updates}:x);
  stopGpsTracking();
  if(!DEMO)await clearMessengerLocation(S.currentUser?.username||"");
  setState({showRecip:false,recipName:"",finishPreview:null,finishFile:null,finishRemark:"",finishMode:"photo",signatureData:null,selId:null,gpsTracking:false,gpsTaskId:null});
...
function render(){
  ...
  if(!document.getElementById("zone-hdr")){
    app.className="wrap";
  ...
  // ── Zone: Sidebar ──
  const sbEl=document.getElementById("zone-sidebar");
  if(sbEl){
  ...
  if(S.page==="mapview"&&(d.isAdmin||canCreate(S.currentUser))){
    const mHtml=renderMapPage();
    if(_zoneCache["zone-main"]!==mHtml){patchZone("zone-main",mHtml);}
    patchZone("zone-modal","");return;
  }
  ...
  // ── Zone: Modal (เปลี่ยนเฉพาะเมื่อ selId เปลี่ยน) ──
...
function renderHdr(cnt,pending,isSA,isAdmin){
  ...
  return '<div class="hdr"><div class="hdr-blob"></div><div class="hdr-inner">'
    ...
    // GPS Toggle — เฉพาะแมสที่กำลังวิ่งงาน
    +(S.currentUser.role==="messenger"&&S.tasks.some(t=>t.status==="going"&&t.messenger_name===S.currentUser.username)
      ?'<button onclick="toggleGps()" style="background:'+(S.gpsTracking?"rgba(6,199,85,.15)":"rgba(239,68,68,.1)")+';border:1px solid '+(S.gpsTracking?"rgba(6,199,85,.4)":"rgba(239,68,68,.4)")+';border-radius:8px;padding:6px 10px;cursor:pointer;display:flex;align-items:center;gap:5px;font-size:12px;font-weight:700;color:'+(S.gpsTracking?"#06C755":"#ef4444")+'" title="'+(S.gpsTracking?"กด GPS กำลังทำงาน":"GPS ปิดอยู่ — กดเปิด")+'">'
        +(S.gpsTracking?'<span style="width:8px;height:8px;border-radius:50%;background:#06C755;animation:gpsPulse 1.4s infinite;display:inline-block"></span>GPS ON':'⛔ GPS OFF')
        +'</button>'
      :"")
    ...
    +(S.realtimeStatus==="live"
      ?'<span style="width:6px;height:6px;border-radius:50%;background:#06C755;animation:gpsPulse 1.4s infinite;display:inline-block"></span><span style="font-size:9px;color:#06C755;font-weight:700">LIVE</span>'
...
function renderSidebar(cnt,pending,isSA,isAdmin){
  ...
  return '<div style="padding:14px 18px 12px;border-bottom:1px solid #1e2d44"><div style="font-size:9px;letter-spacing:2px;color:#06C755;font-weight:700;font-family:monospace">MSG SYSTEM</div><div style="font-size:14px;font-weight:800;color:#fff;margin-top:2px">ระบบสั่งงานแมส</div></div>'
    ...
    +(isAdmin?'<div class="sb-lbl" style="margin-top:8px">ผู้ดูแล</div>'
      ...
      +'<div class="sb-item'+(S.page==="mapview"?" on":"")+'" onclick="setState({page:\'mapview\'})"><span>🗺️</span><span style="flex:1">แผนที่ติดตามแมส</span></div>'
    ...
    +(!isAdmin&&canCreate(S.currentUser)?'<div class="sb-lbl" style="margin-top:8px">ติดตาม</div>'
      +'<div class="sb-item'+(S.page==="mapview"?" on":"")+'" onclick="setState({page:\'mapview\'})"><span>🗺️</span><span style="flex:1">แผนที่ติดตามแมส</span></div>':"")
    +'</div>'
    +(["superadmin","admin"].includes(S.currentUser.role)
...
function renderList(d){
  const {cnt,fil,pending,isSA,isAdmin}=d;
  // GPS bar — ซ่อนจากแมสทั้งหมด ไม่ให้รู้ว่ามีติดตาม
  const gpsBar="";
  ...
  const expFilterPanel=S.showExportFilter
    ?'<div style="margin:0 16px 10px;background:#1a2235;border:1px solid #2a3a54;border-radius:12px;padding:14px">'
      ...
      +'<span id="exp-count" style="font-size:11px;color:#06C755;font-weight:700">'+getFilteredExportData().length+' รายการ</span>'
  ...
  const exp='<div class="export-bar">'
    ...
    +'</div>';
  let cards="";
  if(S.loading)cards='<div style="padding:0 16px">'+'<div class="skeleton" style="height:90px"></div>'.repeat(4)+'</div>';
    ...
    // ── Load More: แสดงทีละ PAGE_SIZE ──
    const maxShow=S.listPage*PAGE_SIZE;
    const visible=fil.slice(0,maxShow);
    const remaining=fil.length-visible.length;
    cards=visible.map(t=>{
    const r3='<div class="tc-r3">'
      ...
      +(t.status==="going"&&t.messenger_name&&(isAdmin||(canCreate(S.currentUser)&&t.created_by===S.currentUser.username))
        ?'<button onclick="openMapForMessenger(\''+esc(t.messenger_name)+'\')" style="background:rgba(59,130,246,.12);border:1px solid rgba(59,130,246,.4);color:#60a5fa;border-radius:6px;padding:2px 9px;font-size:11px;cursor:pointer;font-family:Sarabun,sans-serif;font-weight:700">📍 GPS</button>'
        :"")
      +'<button onclick="exportPDF(\'single\',\''+t.id+'\')" style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.3);color:#f87171;border-radius:6px;padding:2px 8px;font-size:11px;cursor:pointer;font-family:Sarabun,sans-serif">📄</button>'
      +'</div>'
      +'</div>';
    return '<div class="task-card '+TC[t.status]+'" onclick="setState({selId:\''+t.id+'\',showRecip:false,recipName:\'\'})">'
      ...
      +(t.job_nj?'<span style="font-family:monospace;font-size:12px;font-weight:700;color:#f59e0b;background:rgba(245,158,11,.12);padding:3px 9px;border-radius:6px;flex-shrink:0;margin-top:2px;border:1.5px solid rgba(245,158,11,.35)">NJ: '+esc(t.job_nj)+'</span>':"")
      +'<span class="tc-title">'+esc(t.type)+(t.detail?' — '+esc(t.detail.slice(0,35))+(t.detail.length>35?"…":""):"")+'</span>'
      +'<div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;flex-shrink:0;min-width:90px">'
      // สถานะ badge ใหญ่ขึ้น
      +'<span class="tc-badge '+BC[t.status]+'" style="font-size:11px;padding:4px 10px">'+SL[t.status]+'</span>'
      // วันที่
      +'<span style="font-size:11px;color:#94a3b8;font-family:monospace;white-space:nowrap">📅 '+esc(t.date||"–")+'</span>'
      // เวลา — แสดงตามสถานะ
      +(t.status==="wait"?'<span style="font-size:11px;color:#94a3b8;font-family:monospace;white-space:nowrap">⏰ '+esc(t.pickup_time||"–")+'</span>':"")
      +(t.status==="going"&&t.start_time?'<span style="font-size:11px;color:#3b82f6;font-family:monospace;white-space:nowrap;font-weight:700">🏃 '+esc(t.start_time)+'</span>':"")
      +(t.status==="done"&&t.end_time?'<span style="font-size:11px;color:#06C755;font-family:monospace;white-space:nowrap;font-weight:700">✅ '+esc(t.end_time)+'</span>':"")
      +'</div>'
      +'</div>'
      +'<div class="tc-r2">'
    ...
    // ปุ่ม Load More
    if(remaining>0){
      cards+='<div style="padding:16px;text-align:center">'
        +'<button onclick="setState({listPage:S.listPage+1})" style="background:#1a2235;border:1.5px solid #2a3a54;border-radius:12px;padding:10px 28px;color:#94a3b8;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s" onmouseenter="this.style.borderColor=\'#06C755\';this.style.color=\'#06C755\'" onmouseleave="this.style.borderColor=\'#2a3a54\';this.style.color=\'#94a3b8\'">'
        +'โหลดเพิ่ม '+remaining+' รายการ ▾</button>'
        +'</div>';
    }
  }
  return gpsBar+stats+filters+pWarn+expFilterPanel+exp+'<div class="task-list">'+cards+'</div>'
...
function renderDetail(){
  ...
  // ── Realtime จัดการอัปเดต modal อัตโนมัติแล้ว ──
  clearInterval(window._detailPoll);
  ...
  let action="";
  if(canFinishTask(S.currentUser)){
    if(t.status==="wait"){
      action='<button class="btn-green" style="background:#3b82f6;font-size:15px" onclick="acceptTask(\''+t.id+'\')">🏃 รับงาน — บันทึกเวลาอัตโนมัติ</button>'
        +'<div style="text-align:center;font-size:11px;color:#475569;margin-top:6px">⏱ ระบบจะบันทึกเวลาจริงทันทีที่กด</div>';
    }
  }
  // ปุ่มแก้ไข — เฉพาะสถานะ "รอรับ" และเป็น admin หรือเจ้าของงาน
  const canEdit=t.status==="wait"&&(canViewAll(S.currentUser)||t.created_by===S.currentUser?.username);
  if(canEdit){
    action+='<button onclick="openEditTask(\''+t.id+'\')" style="width:100%;margin-top:8px;padding:12px;border-radius:12px;border:1.5px solid rgba(99,102,241,.4);background:rgba(99,102,241,.06);color:#818cf8;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s" onmouseenter="this.style.background=\'rgba(99,102,241,.12)\'" onmouseleave="this.style.background=\'rgba(99,102,241,.06)\'">✏️ แก้ไขข้อมูลใบงาน</button>';
  }
  if(!canFinishTask(S.currentUser)&&!canEdit){
    action='<div style="font-size:12px;color:#64748b;text-align:center;padding:6px 0">เปลี่ยนเป็นโหมด "แมส" เพื่ออัปเดต</div>';
  }
  if(canFinishTask(S.currentUser)&&t.status==="going"){
    ...
    if(S.showRecip){
      const nowT=nowTime();
      action+='<div style="background:#0f172a;border:1px solid #2a3a54;border-radius:12px;padding:14px;margin-bottom:10px"><div style="font-size:12px;color:#06C755;font-weight:700;margin-bottom:12px">✅ ยืนยันส่งเสร็จ</div>'
        +'<div style="margin-bottom:12px;display:flex;align-items:center;gap:10px"><div style="background:rgba(6,199,85,.08);border:1px solid rgba(6,199,85,.2);border-radius:8px;padding:10px 16px;flex:1"><div style="font-size:10px;color:#64748b;font-weight:700;margin-bottom:3px">⏰ เวลาส่งเสร็จ (อัตโนมัติ)</div><div style="font-size:22px;font-weight:800;color:#06C755;font-family:monospace" id="finish-clock">'+nowT+'</div></div></div>'
        +'<div style="margin-bottom:12px"><div style="font-size:11px;color:#94a3b8;font-weight:700;margin-bottom:6px">📝 หมายเหตุ</div>'
        ...
        +'</div>'
        +(S.finishMode==="photo"
          ?'<div style="font-size:11px;color:#ef4444;font-weight:700;margin-bottom:8px">📸 แนบภาพหลักฐาน *บังคับ</div>'+(S.finishPreview?'<div style="display:flex;align-items:center;gap:10px;margin-bottom:4px"><img src="'+S.finishPreview+'" style="width:70px;height:70px;border-radius:8px;object-fit:cover;border:2px solid #06C755"><span style="font-size:12px;color:#06C755;font-weight:700">✅ แนบแล้ว</span><button onclick="setState({finishPreview:null,finishFile:null})" style="background:none;border:none;color:#64748b;font-size:18px;cursor:pointer;margin-left:auto">×</button></div>':'<label id="finish-photo-label" style="display:block;border:2px dashed #ef4444;border-radius:10px;padding:16px;text-align:center;color:#ef4444;font-size:13px;font-weight:700;cursor:pointer;margin-bottom:4px"><input type="file" accept="image/*" style="display:none" onchange="handleFinishFile(this)">📷 แตะเพื่อแนบภาพ</label>')
          :'<div style="font-size:11px;color:#3b82f6;font-weight:700;margin-bottom:8px">✍️ เซ็นชื่อ *บังคับ</div><div style="position:relative;border:2px solid #3b82f6;border-radius:10px;overflow:hidden;margin-bottom:8px;background:#fff"><canvas id="sig-canvas" width="400" height="150" style="width:100%;height:150px;display:block;touch-action:none;cursor:crosshair"></canvas>'+(S.signatureData?'<div style="position:absolute;top:6px;right:6px"><button onclick="clearSignature()" style="background:rgba(239,68,68,.8);border:none;border-radius:6px;color:#fff;font-size:11px;padding:3px 8px;cursor:pointer">ลบ</button></div>':"")+' </div>'+(S.signatureData?'<div style="font-size:12px;color:#06C755;font-weight:700;margin-bottom:4px">✅ เซ็นชื่อแล้ว</div>':'<div style="font-size:11px;color:#64748b;margin-bottom:4px">ลากนิ้วเพื่อเซ็นชื่อ</div>')
        )
        +'</div>'
        +'<button class="btn-green" onclick="confirmFinish()" style="'+((S.finishMode==="photo"&&!S.finishPreview)||(S.finishMode==="sign"&&!S.signatureData)?"opacity:.5;background:#475569":"")+'">'+(S.finishMode==="photo"?(S.finishPreview?"✅ ยืนยันส่งเสร็จ":"📷 แนบภาพก่อนยืนยัน"):(S.signatureData?"✅ ยืนยันส่งเสร็จ":"✍️ เซ็นชื่อก่อนยืนยัน"))+'</button>';
      setTimeout(()=>{const el=document.getElementById("finish-clock");if(el)setInterval(()=>{if(document.getElementById("finish-clock"))el.textContent=nowTime();},1000);if(S.finishMode==="sign")initSignatureCanvas();},100);
    }else{
      ...
      action+='<button onclick="cancelTask(\''+t.id+'\' )" style="width:100%;margin-top:8px;padding:12px;border-radius:12px;border:1.5px solid rgba(239,68,68,.4);background:rgba(239,68,68,.06);color:#f87171;font-size:13px;font-weight:700;cursor:pointer">↩ ยกเลิกการรับงาน — คืนสถานะรอรับ</button>';
    }
  }else if(t.status==="done"){
    action=(action||'')+'<button class="btn-green" disabled style="opacity:.5">✅ งานนี้เสร็จแล้ว</button>';
  }
  return '<div class="modal-overlay" onclick="if(event.target===this){clearInterval(window._detailPoll);setState({selId:null})}">'
    +'<div class="modal-box">'
    +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">'
    +'<div style="font-family:monospace;font-size:11px;color:#06C755;letter-spacing:1px">JOB '+esc(t.job_number||t.id.slice(0,8))+'</div>'
    +'<div style="display:flex;align-items:center;gap:8px">'
    +'<button onclick="refreshDetail()" id="detail-refresh-btn" style="background:rgba(6,199,85,.08);border:1px solid rgba(6,199,85,.25);border-radius:8px;color:#06C755;font-size:12px;font-weight:700;padding:4px 10px;cursor:pointer;display:flex;align-items:center;gap:4px">🔄 รีเฟรช</button>'
    +'<button class="modal-close" style="margin:0;line-height:1;font-size:22px" onclick="clearInterval(window._detailPoll);setState({selId:null})">×</button>'
    +'</div></div>'
    ...
    +(t.inspect_date?'<div class="detail-field"><div class="detail-lbl">🗓 วันที่ตรวจปล่อย</div><div class="detail-val" style="font-family:monospace;color:#f59e0b">'+esc(t.inspect_date)+'</div></div>':"")
    +(t.pay_date?'<div class="detail-field"><div class="detail-lbl">💰 จ่ายเงินถึงวันที่</div><div class="detail-val" style="font-family:monospace;color:#06C755">'+esc(t.pay_date)+'</div></div>':"")
    ...
    +(t.finish_remark?'<div class="detail-field"><div class="detail-lbl" style="color:#f59e0b">📝 หมายเหตุการจบงาน</div><div class="detail-val" style="color:#fbbf24;background:rgba(245,158,11,.06);border:1px solid rgba(245,158,11,.2);border-radius:8px;padding:10px 12px">'+esc(t.finish_remark)+'</div></div>':"")
...
function renderEdit(){
  ...
  return '<div class="create-wrap">'
    ...
    +'</div>'
    // NJ
    +'<div class="fg"><label class="fl">🔖 JOB NJ: <span style="font-size:10px;color:#475569;font-weight:400">คั่นด้วย , หรือกด Enter</span></label>'
    +'<div id="nj-tags" style="display:flex;flex-wrap:wrap;gap:6px;background:#0f172a;border:1.5px solid #2a3a54;border-radius:10px;padding:8px 10px;min-height:44px;cursor:text" onclick="document.getElementById(\'nj-inp\').focus()">'
    +((S.fJobNJ||"").split(",").filter(v=>v.trim()).map(v=>'<span style="background:rgba(245,158,11,.12);color:#f59e0b;border:1px solid rgba(245,158,11,.3);padding:2px 8px;border-radius:20px;font-size:12px;font-family:monospace;display:flex;align-items:center;gap:4px">'+esc(v.trim())+'<button onclick="removeNjTag(\''+esc(v.trim())+'\')" style="background:none;border:none;color:#f59e0b;cursor:pointer;font-size:12px;line-height:1;padding:0">×</button></span>').join(""))
    +'<input id="nj-inp" placeholder="'+(S.fJobNJ?"":"พิมพ์เลขแล้วกด Enter...")+'" style="border:none;background:transparent;color:#e2e8f4;font-size:13px;font-family:monospace;outline:none;min-width:120px;flex:1" onkeydown="addNjTag(event)" oninput="this.placeholder=\'\'"></div></div>'
    // บริษัท
...
async function doLogin(){
  ...
  loadTasks(true).then(()=>startRealtime());
  ...
  return new Promise(resolve=>{
    ...
    r.onload=e=>{
      ...
      img.onload=()=>{
        ...
        canvas.toBlob(blob=>{
          // สร้าง File object จาก blob ที่บีบแล้ว
          const compressed=new File([blob],file.name,{type:"image/jpeg",lastModified:Date.now()});
          // อ่านเป็น dataURL สำหรับ preview
...
function confirmFinish(){
  if(S.finishMode==="photo"){
    if(!S.finishPreview&&!S.finishFile){
      ...
      const label=document.querySelector("label[id='finish-photo-label']");
      if(label){label.style.borderColor="#ef4444";setTimeout(()=>{label.style.borderColor="#ef4444";},2000);}
      return;
    }
  }else{
    // โหมดเซ็น — บันทึก canvas เป็น image
    const canvas=document.getElementById("sig-canvas");
    if(!canvas||!S.signatureData){
      showToast("⚠️ กรุณาเซ็นชื่อก่อนยืนยัน","warn");
      if(canvas){canvas.style.border="2px solid #ef4444";setTimeout(()=>{canvas.style.border="2px solid #3b82f6";},2000);}
      return;
    }
  }
  const t=nowTime();
  ...
  if(S.finishMode==="photo"&&S.finishFile&&!DEMO){
    uploadFinishImage(S.selId,S.finishFile,t);
  }else{
    finishTask(S.selId,evidence,t);
  }
}
...
function exportPDF(mode,taskId){
  ...
  const slips=data.map((t,i)=>{
    ...
    return `<div class="slip${i<data.length-1?" page-break":""}">
      ...
      <!-- ภาพหลักฐาน -->
      ${t.image_url?`<div style="margin-top:8px"><div class="field-lbl">📸 ภาพหลักฐาน</div><img src="${esc(t.image_url)}" style="max-width:180px;max-height:140px;border-radius:8px;border:1px solid #e2e8f0;margin-top:4px"></div>`:""}
      <!-- ลายเซ็น -->
      <div class="sig-row">
        <div class="sig-box">
...
// ── Detail List ──
const DETAILS_DEFAULT=["ส่งชุดงานตรวจปล่อย","ส่งดีโอ","รับดีโอ","วางบิล","การ์ดใส่กล่อง","ส่งเอกสาร","รับเอกสาร","ใส่กล่องกลางทุ่ง(แดง)","ใส่กล่องกลางทุ่ง(เหลือง)","ใส่กล่องด่าน","ใส่กล่องประตู3","ส่งเอกสารบนด่าน"];
...
function startGpsTracking(taskId,messengerName){
  if(!navigator.geolocation){showToast("⚠️ อุปกรณ์ไม่รองรับ GPS","warn");return}
  // 🛡 ตรวจสอบว่ามีงานที่กำลังวิ่งจริงก่อน tracking
  const hasActiveTask=S.tasks.some(t=>t.status==="going"&&t.messenger_name===(messengerName||S.currentUser?.username));
  if(!hasActiveTask&&!DEMO){
    showToast("⚠️ ไม่มีงานที่กำลังวิ่ง — GPS ไม่เริ่ม","warn");
    return;
  }
  stopGpsTracking(); // หยุดอันเก่าก่อน
  S.gpsTracking=true;
  // ครั้งแรกใช้ accuracy ต่ำก่อนให้เร็ว
  navigator.geolocation.getCurrentPosition(pos=>{
    const {latitude:lat,longitude:lng}=pos.coords;
    S.gpsLat=lat;S.gpsLng=lng;
    sendLocation(lat,lng,taskId,messengerName);
    render();
  },err=>{
    if(err.code===1){// PERMISSION_DENIED
      ...
      showToast("⚠️ ไม่สามารถเข้าถึง GPS ได้","warn");
    }
  },{enableHighAccuracy:false,timeout:5000,maximumAge:10000});

  // ส่งตำแหน่งทุก 30 วิ — หยุดอัตโนมัติถ้าไม่มีงาน
  _gpsInterval=setInterval(()=>{
    // 🛡 หยุด GPS อัตโนมัติถ้างานเสร็จหรือไม่มีงานแล้ว
    const stillActive=S.tasks.some(t=>t.status==="going"&&t.messenger_name===(messengerName||S.currentUser?.username));
    if(!stillActive&&!DEMO){
      stopGpsTracking();
      clearMessengerLocation(messengerName||S.currentUser?.username||"");
      showToast("📍 GPS หยุดอัตโนมัติ — งานเสร็จแล้ว","ok");
      return;
    }
    navigator.geolocation.getCurrentPosition(pos=>{
      const {latitude:lat,longitude:lng}=pos.coords;
      S.gpsLat=lat;S.gpsLng=lng;
      sendLocation(lat,lng,taskId,messengerName);
    },null,{enableHighAccuracy:true,timeout:8000,maximumAge:20000});
  },30000);
  setState({gpsTracking:true,gpsPermission:"granted"});
  showToast("📍 GPS เปิดแล้ว — กำลังส่งตำแหน่ง","ok");
}
...
function stopGpsTracking(){
  ...
  S.gpsTracking=false;S.gpsLat=null;S.gpsLng=null;
}

async function sendLocation(lat,lng,taskId,messengerName){
  if(DEMO)return;
  const name=messengerName||S.currentUser?.username||"";
  const now=new Date().toISOString();
  try{
    // อัปเดตตำแหน่งปัจจุบัน (delete+insert)
    await fetch(SUPA_URL+"/rest/v1/messenger_locations?messenger_name=eq."+encodeURIComponent(name),{method:"DELETE",headers:SH});
    await fetch(SUPA_URL+"/rest/v1/messenger_locations",{method:"POST",headers:SH,
      body:JSON.stringify({messenger_name:name,task_id:taskId,lat,lng,updated_at:now})});
    // บันทึกประวัติเส้นทางทุกจุด
    await fetch(SUPA_URL+"/rest/v1/messenger_location_history",{method:"POST",headers:SH,
      body:JSON.stringify({messenger_name:name,task_id:taskId,lat,lng,recorded_at:now})});
  }catch(e){console.error("GPS send error",e)}
}

async function clearMessengerLocation(name){
  if(DEMO||!name)return;
  try{await fetch(SUPA_URL+"/rest/v1/messenger_locations?messenger_name=eq."+encodeURIComponent(name),{method:"DELETE",headers:SH});}catch{}
}

async function loadLocations(){
  if(DEMO)return[];
  try{return await sbGet("messenger_locations","order=updated_at.desc");}catch{return[]}
}

async function loadLocationHistory(taskId){
  if(DEMO)return[];
  try{return await sbGet("messenger_location_history","task_id=eq."+encodeURIComponent(taskId)+"&order=recorded_at.asc");}catch{return[]}
}

// ── Map Page ──
let _leafletMap=null;
let _mapRefreshInterval=null;
let _routeLayers={}; // เก็บ polyline แต่ละแมส

function renderMapPage(){
  return '<div style="padding:16px 0 0">'
    +'<div style="display:flex;align-items:center;gap:10px;padding:0 16px 12px">'
    +'<div style="flex:1"><div style="font-size:15px;font-weight:800;color:#fff">📍 แผนที่ติดตามแมส</div>'
    +'<div style="font-size:11px;color:#64748b;margin-top:2px">อัปเดตทุก 30 วินาที · แสดงตำแหน่งปัจจุบัน</div></div>'
    +'<button onclick="refreshMap()" style="background:rgba(6,199,85,.1);border:1px solid rgba(6,199,85,.3);color:#06C755;border-radius:8px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer">🔄 รีเฟรช</button>'
    +'</div>'
    +'<div class="map-legend">'
    +'<div class="legend-item"><div class="legend-dot" style="background:#3b82f6"></div>ตำแหน่งปัจจุบัน</div>'
    +'<div class="legend-item"><div class="legend-dot" style="background:#ef4444"></div>ออฟไลน์</div>'
    +'</div>'
    +'<div id="map-container" class="map-wrap"></div>'
    +'<div id="map-msg-list" style="padding:0 16px 24px"></div>'
    +'<script>initMap();<\/script>'
    +'</div>';
}

async function initMap(){
  // ── Lazy load Leaflet เฉพาะตอนเปิดหน้าแผนที่ ──
  ...
  _routeLayers={};
  _leafletMap=L.map("map-container",{zoomControl:true}).setView([13.6,100.9],12);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap",maxZoom:19}).addTo(_leafletMap);
  await updateMapMarkers();
  _mapRefreshInterval=setInterval(updateMapMarkers,30000);
}

const ROUTE_COLORS=["#f59e0b","#a855f7","#ec4899","#14b8a6","#f97316","#06C755","#3b82f6"];

async function updateMapMarkers(){
  if(!_leafletMap)return;
  const locs=await loadLocations();
  // ลบ marker เก่า (เฉพาะ marker ไม่ต้องลบ polyline เพราะไม่มีแล้ว)
  _leafletMap.eachLayer(l=>{if(l instanceof L.Marker)_leafletMap.removeLayer(l);});

  const isAdminView=canViewAll(S.currentUser);
  const going=S.tasks.filter(t=>{
    if(t.status!=="going")return false;
    if(isAdminView)return true;
    return t.created_by===S.currentUser?.username;
  });
  let listHtml='<div style="font-size:11px;font-weight:700;color:#64748b;letter-spacing:1px;margin-bottom:8px">สถานะ GPS แมส</div>';

  going.forEach((t,i)=>{
    const loc=locs.find(l=>l.messenger_name===t.messenger_name);
    const color=ROUTE_COLORS[i%ROUTE_COLORS.length];

    if(loc&&loc.lat&&loc.lng){
      const icon=L.divIcon({className:"",html:`<div style="background:${color};color:#fff;border-radius:50%;width:38px;height:38px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;border:3px solid #fff;box-shadow:0 2px 10px rgba(0,0,0,.5)">${esc(t.messenger_name?.slice(0,2)||"?")}</div>`,iconSize:[38,38],iconAnchor:[19,19]});
      const updatedAgo=Math.round((Date.now()-new Date(loc.updated_at).getTime())/60000);
      L.marker([loc.lat,loc.lng],{icon})
        .bindPopup(`<div style="font-family:Sarabun,sans-serif;min-width:180px">
          <b style="color:${color}">🏃 ${esc(t.messenger_name)}</b><br>
          <span style="font-size:11px;color:#374151">${esc(t.company||"")}</span><br>
          <span style="font-size:11px;color:#3b82f6">📍 ${esc(t.place||"")}</span><br>
          <hr style="border:none;border-top:1px solid #e5e7eb;margin:5px 0">
          <span style="font-size:10px;color:#6b7280">📡 อัปเดต ${updatedAgo} นาทีที่แล้ว</span>
        </div>`)
        .addTo(_leafletMap);

      listHtml+=`<div style="display:flex;align-items:center;gap:10px;background:#1a2235;border:1px solid #2a3a54;border-radius:10px;padding:10px 14px;margin-bottom:8px;cursor:pointer" onclick="flyToMessenger(${loc.lat},${loc.lng})">
        <div style="width:32px;height:32px;background:${color};border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#fff;flex-shrink:0">${esc(t.messenger_name?.slice(0,2)||"?")}</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:700;color:#e2e8f4">${esc(t.messenger_name)}</div>
          <div style="font-size:11px;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📍 ${esc(t.place||"")}</div>
        </div>
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:10px;color:#06C755;font-weight:700">📡 LIVE</div>
          <div style="font-size:10px;color:#475569">${updatedAgo} นาทีที่แล้ว</div>
        </div>
      </div>`;
    }else{
      const acceptedAt=t.start_time?t.start_time:"–";
      listHtml+=`<div style="display:flex;align-items:center;gap:10px;background:#1a2235;border:1.5px solid rgba(239,68,68,.25);border-radius:10px;padding:10px 14px;margin-bottom:8px;">
        <div style="position:relative;flex-shrink:0">
          <div style="width:32px;height:32px;background:#374151;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#9ca3af">${esc(t.messenger_name?.slice(0,2)||"?")}</div>
          <div style="position:absolute;bottom:-2px;right:-2px;width:11px;height:11px;background:#ef4444;border-radius:50%;border:2px solid #1a2235"></div>
        </div>
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:700;color:#e2e8f4">${esc(t.messenger_name||"–")}</div>
          <div style="font-size:11px;color:#64748b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📍 ${esc(t.place||"")}</div>
        ...
        <div style="text-align:right;flex-shrink:0">
          <div style="font-size:11px;font-weight:800;color:#ef4444;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);padding:2px 8px;border-radius:20px">⛔ OFFLINE</div>
          <div style="font-size:10px;color:#475569;margin-top:3px">รับงาน ${esc(acceptedAt)}</div>
        </div>
      </div>`;
    }
  });

  if(!going.length)listHtml+='<div style="text-align:center;color:#475569;padding:16px;font-size:13px">ไม่มีแมสกำลังวิ่งอยู่</div>';
  const listEl=document.getElementById("map-msg-list");
  if(listEl)listEl.innerHTML=listHtml;
}

function flyToMessenger(lat,lng){
  if(_leafletMap)_leafletMap.flyTo([lat,lng],16,{duration:1});
}

async function refreshMap(){
  await updateMapMarkers();
  showToast("🔄 รีเฟรชแผนที่แล้ว","ok");
}

function reStartGps(){
  const myTask=S.tasks.find(t=>t.status==="going"&&t.messenger_name===S.currentUser?.username);
  if(myTask)startGpsTracking(myTask.id,S.currentUser.username);
}
...
function toggleGps(){
  if(S.gpsTracking){
    // ปิด GPS
    stopGpsTracking();
    if(!DEMO)clearMessengerLocation(S.currentUser?.username||"");
    render();
    showToast("⛔ ปิด GPS แล้ว","warn");
  }else{
    // เปิด GPS — เช็คว่ามีงานที่กำลังวิ่งก่อน
    const myTask=S.tasks.find(t=>t.status==="going"&&t.messenger_name===S.currentUser?.username);
    if(!myTask){
      showToast("⚠️ ต้องรับงานก่อนถึงจะเปิด GPS ได้","warn");
      return;
    }
    requestGpsAndStart(myTask.id,S.currentUser.username);
  }
}

async function openMapForMessenger(messengerName){
  S._focusMessenger=messengerName;
  setState({page:"mapview"});
  // หลัง map โหลดเสร็จ ให้ zoom ไปหาแมสคนนั้น
  setTimeout(async()=>{
    const locs=await loadLocations();
    const loc=locs.find(l=>l.messenger_name===messengerName);
    if(loc&&loc.lat&&loc.lng&&_leafletMap){
      _leafletMap.flyTo([loc.lat,loc.lng],16,{duration:1.2});
    } else if(!loc){
      showToast("⚠️ "+messengerName+" ยังไม่ได้เปิด GPS","warn");
    }
    S._focusMessenger=null;
  },800);
}
...
async function manualRefresh(){
  ...
  if(btn){btn.style.transform="rotate(360deg)";btn.style.transition="transform .5s ease";btn.disabled=true;}
  _lastFetch=0; // reset cache
  await loadTasks(true);
  setTimeout(()=>{if(btn){btn.style.transform="";btn.style.transition="";btn.disabled=false;}},500);
...
function startRealtime(){
  ...
  // ── Always-on Auto Poll 10 วิ (fallback เสมอ) ──
  if(!_autoPollInterval){
    _autoPollInterval=setInterval(async()=>{
      if(!S.currentUser||DEMO)return;
      const prev=S.tasks.length;
      await loadTasks();
      // อัปเดต status ถ้า realtime ไม่ live
  ...
  ws.onopen=()=>{
    ws.send(JSON.stringify({
      ...
      event:"phx_join",
      payload:{config:{broadcast:{self:true},presence:{key:""},postgres_changes:[{event:"*",schema:"public",table:"msg_tasks"}]}},
      ref:"1"
    }));
    S.realtimeStatus="live";
    invalidateZone("zone-hdr");
  ...
  ws.onclose=()=>{
    ...
    invalidateZone("zone-hdr");
    render();
    // reconnect อัตโนมัติใน 5 วิ
    setTimeout(()=>{if(S.currentUser)startRealtime();},5000);
  };
...
function startPollingFallback(){
  // fallback polling 30 วิ ถ้า realtime ใช้ไม่ได้
  if(window._pollingInterval)return;
  window._pollingInterval=setInterval(()=>{
    if(S.page==="list"&&!DEMO)loadTasks();
  },30000);
}
