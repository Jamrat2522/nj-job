...
async function sbGet(t,q=""){
  ...
  const tid=setTimeout(()=>ctrl.abort(),10000); // timeout 10s
  try{
    const r=await fetch(API+"/"+t+(q?"?"+q:""),{headers:SH,signal:ctrl.signal});
    clearTimeout(tid);
    if(!r.ok){
...
function _patchListCounts(cnt,pending,isSA){
  // stats numbers
  const nums={sa:cnt.all,sw:cnt.wait,sg:cnt.going,sdv:cnt.delivered,sd:cnt.done};
  Object.entries(nums).forEach(([cls,val])=>{
    const el=document.querySelector(".stat."+cls+" .stat-num");
    if(el&&el.textContent!=String(val))el.textContent=val;
  });
  // filter bar counts
  [{sel:"fb-num",data:[cnt.all,cnt.wait,cnt.going,cnt.delivered,cnt.done]}].forEach(()=>{
    const btns=document.querySelectorAll(".fb .fb-num");
    const vals=[cnt.all,cnt.wait,cnt.going,cnt.done];
    // delivered แสดงเฉพาะ cnt.delivered > 0
    let i=0;
    btns.forEach(b=>{if(vals[i]!==undefined&&b.textContent!=String(vals[i])){b.textContent=vals[i];}i++;});
  });
  // pending warning
  if(isSA&&pending){
    const pw=document.querySelector(".pending-warn");
    if(pw)pw.textContent="👥 มีผู้ใช้รออนุมัติ "+pending+" คน";
  }
}
...
function computeDerived(){
  ...
  // สร้าง key จาก tasks count + filter + role (ถ้าเหมือนเดิม = ไม่ต้อง compute)
  const k=(S.tasks.length||0)+"|"+(S.filter)+"|"+(S.currentUser?.username||"")+"|"+(S.role||"");
  ...
  const fil=S.filter==="all"?tasks
    :S.filter==="wait"?wait
    :S.filter==="going"?going
    :S.filter==="delivered"?delivered
    :done;
  _derived={
    tasks,wait,going,delivered,done,fil,
    cnt:{all:tasks.length,wait:wait.length,going:going.length,delivered:delivered.length,done:done.length},
...
function invalidateDerived(){_derived=null;_derivedKey="";}
...
function setState(obj){
  ...
  if("filter" in obj&&obj.filter!==S.filter){obj.listPage=1;_forceListRebuild=true;}
  if("listPage" in obj||"page" in obj||"selId" in obj)_forceListRebuild=true;
  Object.assign(S,obj);
  // invalidate derived เฉพาะเมื่อ tasks/users/filter/role เปลี่ยน
  if("tasks" in obj||"users" in obj||"filter" in obj||"role" in obj)invalidateDerived();
  render();
}
  ...
  if(isFirstLoad){
    ...
    setTimeout(async()=>{
      ...
      if(all.length>0){
        S.tasks=all;
        invalidateDerived();
        // อัปเดต counts ใน header/sidebar โดยไม่ rerender list ทั้งหมด
        updateZoneCounts();
        // invalidate zone-main เพื่อให้ render รอบถัดไปรู้ว่า list อาจเปลี่ยน
        if(S.page==="list")invalidateZone("zone-main");
      }
      _bgFetchDone=true;
...
async function createTask(){
  ...
  try{
    ...
    if(!DEMO){
      ...
      if(!ok){
        // insert ล้มเหลว — แสดง error ไม่นำทางออก ข้อมูลไม่หาย
        showToast("❌ บันทึกไม่สำเร็จ กรุณาลองใหม่","warn");
        setState({saving:false});
        _creatingTask=false;
        return;
      }
      // โหลด tasks ใหม่เสมอ (ยืนยันว่า DB มีข้อมูลจริง ไม่พึ่ง realtime อย่างเดียว)
      await loadTasks(true);
    }
    else{S.tasks=[task,...S.tasks];invalidateDerived();}

    setState({saving:false,page:"list",fCompany:"",fJobNJ:"",fDetail:"",fPlace:"",fPickup:"",fDeliver:"",fInspectDate:"",fPayDate:"",fFile:null,fPreview:null,fDate:todayStr(),fCloseMode:"messenger_finish"});
...
async function deleteTask(id){
  ...
  invalidateDerived();invalidateCardCache(id);delete _zoneCache["zone-main"];
  setState({selId:null});
...
function _showDeliveredAlert(task){
  // แสดงเฉพาะถ้า owner ของงานนี้ login อยู่
  if(!task||S.currentUser?.username!==task.created_by)return;
  document.getElementById("delivered-alert")?.remove();

  const el=document.createElement("div");
  el.id="delivered-alert";
  el.style.cssText="position:fixed;inset:0;z-index:900;display:flex;align-items:flex-end;justify-content:center;padding-bottom:env(safe-area-inset-bottom,0)";
  el.innerHTML=
    ...
    +'<button onclick="document.getElementById(\'delivered-alert\').remove();setState({filter:\'delivered\',page:\'list\'})" style="width:100%;margin-top:8px;padding:12px;background:transparent;border:1.5px solid #2a3a54;border-radius:12px;color:#94a3b8;font-size:13px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">ดูทีหลัง</button>'
    +'</div>';
  ...
  // auto-dismiss หลัง 30 วิ
  setTimeout(()=>document.getElementById("delivered-alert")?.remove(),30000);
}
...
function _renderNow(){
  ...
  const mainEl=document.getElementById("zone-main");
  const listExists=mainEl?.querySelector(".task-list");
  if(listExists&&!_forceListRebuild){
    // อัปเดตเฉพาะ card ที่เปลี่ยน
    let anyMissing=false;
    d.fil.slice(0,S.listPage*PAGE_SIZE).forEach(t=>{if(!patchCard(t.id))anyMissing=true;});
    if(!anyMissing){
      // อัปเดต stats + filter bar counts ด้วย DOM ตรง (เบากว่า innerHTML ทั้งก้อน)
...
function renderSidebar(cnt,pending,isSA,isAdmin){
  ...
  return '<div style="padding:14px 18px 12px;border-bottom:1px solid #1e2d44"><div style="font-size:9px;letter-spacing:2px;color:#06C755;font-weight:700;font-family:monospace">MSG SYSTEM</div><div style="font-size:14px;font-weight:800;color:#fff;margin-top:2px">ระบบสั่งงานแมส</div></div>'
    ...
    +[["all","ทั้งหมด","📋",cnt.all],["wait","รอรับ","⏳",cnt.wait],["going","กำลังวิ่ง","🏃",cnt.going],["delivered","ส่งถึงแล้ว","📦",cnt.delivered],["done","เสร็จแล้ว","✅",cnt.done]].map(([k,l,ic,c])=>
...
function renderCard(t){
  ...
  return '<div id="tc-'+t.id+'" class="task-card '+TC[t.status||"wait"]+'" onclick="setState({selId:\''+t.id+'\',showRecip:false,recipName:\'\'})">'
    // Row 1: Job + status
    +'<div class="tc-r1">'
    +'<span class="tc-job">'+esc(t.job_number||t.id.slice(0,8))+'</span>'
    +(t.job_nj?'<span class="tc-nj">NJ: '+esc(t.job_nj.slice(0,20))+'</span>':"")
    +'<span class="tc-title">'+esc(t.type)+(t.detail?' — '+esc(t.detail.slice(0,28))+(t.detail.length>28?"…":""):"")+'</span>'
...
function renderList(d){
  ...
  const stats='<div class="stats">'
    ...
    +'<div class="stat sdv"><div class="stat-num">'+cnt.delivered+'</div><div class="stat-lbl">ส่งถึงแล้ว</div></div>'
  ...
  const exp='<div class="export-bar">'
    ...
    +'</div>';
  let cards="";
  if(S.loading)cards='<div style="padding:0 16px">'+'<div class="skeleton" style="height:100px"></div>'.repeat(4)+'</div>';
    ...
    // ใช้ renderCard() — card แต่ละใบมี id="tc-{id}" สำหรับ patchCard
    cards=visible.map(t=>{
      const html=renderCard(t);
      _cardCache[t.id]=html; // sync cache กับ full render
      return html;
    }).join("");
    if(remaining>0){
...
function renderDetail(){
  ...
  return '<div class="modal-overlay" onclick="if(event.target===this)setState({selId:null})">'
    ...
    +(t.job_nj?'<div style="font-family:monospace;font-size:10px;color:#f59e0b;margin-top:2px">NJ: '+esc(t.job_nj)+'</div>':"")
    +'</div>'
    +'<div style="display:flex;align-items:center;gap:6px">'
    ...
    +'<div><span style="color:#64748b;font-size:11px">📦 ประเภท</span><div style="color:#e2e8f4;margin-top:1px">'+esc(t.type||"–")+'</div></div>'
...
function renderCreate(){
  ...
  return '<div class="create-wrap">'
    ...
    +'</div>'
    +'<div class="fg"><label class="fl">🔒 รูปแบบปิดงาน</label>'
    +'<div class="seg-wrap">'
    +'<button type="button" class="seg-btn'+(S.fCloseMode==="messenger_finish"?" seg-on-green":"")+'" onclick="S.fCloseMode=\'messenger_finish\';document.getElementById(\'f-close-mode\').value=\'messenger_finish\';this.parentNode.querySelectorAll(\'.seg-btn\').forEach(b=>b.classList.remove(\'seg-on-green\',\'seg-on-yellow\'));this.classList.add(\'seg-on-green\')">🟢 จบโดยแมส</button>'
    +'<button type="button" class="seg-btn'+(S.fCloseMode==="user_confirm"?" seg-on-yellow":"")+'" onclick="S.fCloseMode=\'user_confirm\';document.getElementById(\'f-close-mode\').value=\'user_confirm\';this.parentNode.querySelectorAll(\'.seg-btn\').forEach(b=>b.classList.remove(\'seg-on-green\',\'seg-on-yellow\'));this.classList.add(\'seg-on-yellow\')">🟡 รอผู้สั่งยืนยัน</button>'
    +'</div>'
    +'<input type="hidden" id="f-close-mode" value="'+esc(S.fCloseMode||"messenger_finish")+'">'
    +'<div style="font-size:11px;color:#475569;margin-top:5px">'
    +(S.fCloseMode==="messenger_finish"?"แมสกดเสร็จงานได้เลย — เหมาะกับงานส่งเอกสาร":"ต้องให้ผู้สั่งกดยืนยันรับ — เหมาะกับงานรับของ")
    +'</div></div>'
    +(S.fPreview
  ...
  return '<div class="login-page">'
    ...
    +(isL
      ?'<div class="fg"><label class="inp-lbl">Username</label><input class="inp" id="l-user" placeholder="กรอก Username" autocomplete="off" value="" onkeydown="if(event.key===\'Enter\')document.getElementById(\'l-pass\').focus()"></div>'
       ...
       +'<input class="inp" id="l-pass" type="'+(S.showPass?"text":"password")+'" placeholder="กรอก Password" autocomplete="new-password" value="" onkeydown="if(event.key===\'Enter\')doLogin()" style="padding-right:44px">'
...
function njTagsHtml(){
  ...
  return '<div id="nj-tags" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;background:#0f172a;border:1.5px solid #2a3a54;border-radius:10px;padding:8px 10px;min-height:44px;cursor:text" onclick="document.getElementById(\'nj-inp\').focus()">'
    ...
    +' onkeydown="addNjTag(event)" oninput="this.placeholder=\'\'">'
...
const PLACES_KEY="msg_places_v4";
function getPlaces(){
...
function startRealtime(){
  ...
  if(!_autoPollInterval){
    ...
    },15000); // tick ทุก 15 วิ แต่จะ skip ถ้า live และยังไม่ถึง 55 วิ
  }
      ...
      if(msg.event==="postgres_changes"||msg.payload?.data?.type){
        ...
        if(type==="INSERT"&&record){
          ...
          if(existed){
            ...
            if(patched){
              updateZoneCounts();
              if(S.selId===record.id){invalidateZone("zone-modal");patchZone("zone-modal",renderDetail());}
            }else{
              render();
            }
            ...
            if(record.status==="delivered"&&oldStatus!=="delivered"){
              ...
              if(updatedTask)setTimeout(()=>_showDeliveredAlert(updatedTask),300);
            }
          }
...
function _showNotifPopup({icon,title,body,buttons,accent}){
  ...
  // auto-dismiss หลัง 20 วิ (soft) — ไม่บังคับ
...
function closeNotifPopup(){
  clearTimeout(window._notifAutoDismiss);
  const p=document.getElementById("notif-popup");
  if(p){p.style.opacity="0";setTimeout(()=>p?.remove(),250);}
  _notifDismissed=true; // user กดปิดเอง — รอ event ถัดไป
}
...
document.addEventListener("visibilitychange",()=>{
  if(document.visibilityState==="visible"&&S.currentUser){
    _notifDismissed=false; // reset เมื่อกลับมา
    // รอ tasks sync ก่อนนิดนึง
    setTimeout(checkAndShowNotif,1200);
  }
});
