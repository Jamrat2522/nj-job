...
async function sbGet(t,q=""){
  try{
    const r=await fetch(API+"/"+t+(q?"?"+q:""),{headers:SH});
    if(!r.ok){
    ...
    return r.json();
  }catch(e){console.error("sbGet error",t,e);return[]}
}
...
function render(){
  ...
  _renderPending=true;
  requestAnimationFrame(()=>{_renderPending=false;_renderNow();});
}
function _renderNow(){
...
function canEditTask(user,task){
  ...
  // staff เจ้าของงาน + ได้รับสิทธิ์
  const isOwner=task.created_by===user.username;
  const hasPerm=user.can_finish===true||user.can_finish==="true";
  return isOwner&&hasPerm;
}
// สิทธิ์ลบงาน: superadmin/admin ทุกสถานะ + staff เจ้าของ (ทุกสถานะ)
...
function renderList(d){
  ...
  const expData=getFilteredExportData(); // คำนวณครั้งเดียว
  ...
  return stats+filters+pWarn+expFilterPanel+exp+'<div class="task-list">'+cards+'</div>'
