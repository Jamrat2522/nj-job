...
function computeDerived(){
  ...
  const fil=S.filter==="all"?tasks.filter(t=>!isArchived(t))
    :S.filter==="wait"?wait
    :S.filter==="going"?going
    :S.filter==="delivered"?delivered
    :S.filter==="archive"?archived
    :recentDone; // "done" filter → แสดงเฉพาะ done ไม่เกิน 30 วัน
  _derived={
...
function setState(obj){
  ...
  if("filter" in obj&&obj.filter!==S.filter){_forceListRebuild=true;VL._filterKey="";}
