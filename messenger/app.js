...
function computeDerived(){
  if(_derived)return _derived; // ใช้ cache ถ้ายังไม่ dirty
  const tasks=S.tasks;
  ...
  _derived={
    ...
    isAdmin:canViewAll(S.currentUser),
  };
  return _derived;
}
...
const S={
  ...
  showRecip:false,recipName:"",finishPreview:null,finishFile:null,
...
async function acceptTask(id){
  ...
  // ขอ GPS permission ก่อนเริ่ม tracking
...
async function finishTask(id,imgUrl,customTime){
  ...
  setState({showRecip:false,recipName:"",finishPreview:null,finishFile:null,selId:null,gpsTracking:false,gpsTaskId:null});
...
function render(){
  ...
  if(!document.getElementById("zone-hdr")){
    ...
    app.innerHTML='<div id="zone-hdr"></div>'
      +'<div id="zone-sidebar" class="sidebar" style="display:none"></div>'
  ...
  // ── Zone: Header (เปลี่ยนเมื่อ page/saving/gpsTracking เปลี่ยน) ──
  patchZone("zone-hdr", renderHdr(d.cnt,d.pending,d.isSA,d.isAdmin));

  // ── Zone: Sidebar ──
  const sb=renderSidebar(d.cnt,d.pending,d.isSA,d.isAdmin);
  patchZone("zone-sidebar", sb);
  const sbEl=document.getElementById("zone-sidebar");
  if(sbEl)sbEl.style.display=""; // ให้ CSS จัดการ
...
function renderList(d){
  ...
  const filters='<div class="filter-bar">'
    ...
    +'<button id="refresh-btn" onclick="manualRefresh()" style="margin-left:auto;flex-shrink:0;padding:8px 12px;border-radius:10px;border:1.5px solid #2a3a54;background:#1a2235;color:#06C755;font-size:14px;cursor:pointer;transition:all .2s;display:flex;align-items:center;gap:5px" title="รีเฟรชงาน">🔄</button>'
    +'</div>';
...
function renderDetail(){
  ...
  if(S.role==="messenger"||S.currentUser?.role==="messenger"){
    if(t.status==="wait")action='<button class="btn-green" style="background:#3b82f6;font-size:15px" onclick="acceptTask(\''+t.id+'\')">🏃 รับงาน — บันทึกเวลาอัตโนมัติ</button>'
      ...
      if(S.showRecip){
        ...
        action+='<div style="background:#0f172a;border:1px solid #2a3a54;border-radius:12px;padding:14px;margin-bottom:10px">'
          ...
          // แสดงเวลาจริง — ไม่แก้ไขได้
          +'<div style="margin-bottom:12px;display:flex;align-items:center;gap:10px">'
          +'<div style="background:rgba(6,199,85,.08);border:1px solid rgba(6,199,85,.2);border-radius:8px;padding:10px 16px;flex:1">'
          +'<div style="font-size:10px;color:#64748b;font-weight:700;margin-bottom:3px">⏰ เวลาส่งเสร็จ (อัตโนมัติ)</div>'
          +'<div style="font-size:22px;font-weight:800;color:#06C755;font-family:monospace" id="finish-clock">'+nowT+'</div>'
          +'</div></div>'
          // photo — บังคับ
          +'<div style="font-size:11px;color:#ef4444;font-weight:700;margin-bottom:8px">📸 แนบภาพหลักฐาน <span style="color:#ef4444">*บังคับ</span></div>'
          +(S.finishPreview
            ?'<div style="display:flex;align-items:center;gap:10px;margin-bottom:4px"><img src="'+S.finishPreview+'" style="width:70px;height:70px;border-radius:8px;object-fit:cover;border:2px solid #06C755"><span style="font-size:12px;color:#06C755;font-weight:700">✅ แนบแล้ว</span><button onclick="setState({finishPreview:null,finishFile:null})" style="background:none;border:none;color:#64748b;font-size:18px;cursor:pointer;margin-left:auto">×</button></div>'
            :'<label id="finish-photo-label" style="display:block;border:2px dashed #ef4444;border-radius:10px;padding:16px;text-align:center;color:#ef4444;font-size:13px;font-weight:700;cursor:pointer;margin-bottom:4px"><input type="file" accept="image/*" style="display:none" onchange="handleFinishFile(this)">📷 แตะเพื่อแนบภาพ (จำเป็น)</label>'
          )
          +'</div>'
          +'<button class="btn-green" onclick="confirmFinish()" style="'+(S.finishPreview?"":"opacity:.5;background:#475569")+'">'+(S.finishPreview?"✅ ยืนยันส่งเสร็จ":"📷 แนบภาพก่อนยืนยัน")+'</button>';
        // live clock update
        setTimeout(()=>{
          const el=document.getElementById("finish-clock");
          if(el)setInterval(()=>{if(document.getElementById("finish-clock"))el.textContent=nowTime();},1000);
        },100);
      }else{
  ...
  return '<div class="modal-overlay" onclick="if(event.target===this){clearInterval(window._detailPoll);setState({selId:null})}">'
    +'<div class="modal-box">'
    +'<button class="modal-close" onclick="clearInterval(window._detailPoll);setState({selId:null})">×</button>'
...
function confirmFinish(){
  if(!S.finishPreview&&!S.finishFile){
    ...
    const label=document.querySelector("label[style*='dashed']");
    if(label){label.style.borderColor="#ef4444";label.style.color="#ef4444";setTimeout(()=>{label.style.borderColor="#2a3a54";label.style.color="#475569";},2000);}
    return;
  }
  const t=nowTime(); // เวลาจริงตอนกดยืนยัน ห้ามแก้ไข
...
function getFilteredExportData(){
  let data=S.filter==="all"?S.tasks:S.tasks.filter(t=>t.status===S.filter);
  // กรองวันที่เริ่ม
  ...
  // กรองบริษัท
  if(S.expCompany){
    const q=S.expCompany.toLowerCase().trim();
    data=data.filter(t=>(t.company||"").toLowerCase().includes(q));
  }
  return data;
}
...
function exportPDF(mode,taskId){
  ...
  const slips=data.map((t,i)=>{
    ...
    return `<div class="slip${i<data.length-1?" page-break":""}">
      ...
      <!-- ตรวจปล่อย + จ่ายเงิน -->
      <div class="row2">
        <div class="field-box accent-orange">
          <div class="field-lbl">🗓 วันที่ตรวจปล่อย</div>
          <div class="field-val mono">${t.inspect_date?new Date(t.inspect_date).toLocaleDateString("th-TH",{year:"numeric",month:"short",day:"numeric"}):"–"}</div>
        </div>
        <div class="field-box accent-green">
          <div class="field-lbl">💰 จ่ายเงินถึงวันที่</div>
          <div class="field-val mono">${t.pay_date?new Date(t.pay_date).toLocaleDateString("th-TH",{year:"numeric",month:"short",day:"numeric"}):"–"}</div>
        </div>
      </div>
      <!-- รายละเอียด + สถานที่ -->
...
function startRealtime(){
  ...
  // ปิด channel เก่าถ้ามี
  if(_realtimeChannel){
    _realtimeChannel.unsubscribe();
    _realtimeChannel=null;
  }

  // subscribe ผ่าน WebSocket — รับทุก INSERT/UPDATE/DELETE บน msg_tasks
  const evtSource=new EventSource(
    SUPA_URL+"/realtime/v1/sse?apikey="+SUPA_KEY+
    "&schema=public&table=msg_tasks&event=*"
  );

  // ใช้ fetch-based realtime ผ่าน Supabase Realtime API
  ...
  ws.onmessage=async(e)=>{
    try{
      ...
      if(msg.event==="postgres_changes"||msg.payload?.data?.type){
        ...
        const type=chg.type; // INSERT / UPDATE / DELETE
  ...
  ws.onerror=()=>{
    // fallback กลับไปใช้ polling ถ้า realtime ใช้ไม่ได้
    console.warn("Realtime ไม่ได้เปิด — fallback to polling");
...
function stopRealtime(){
  if(_realtimeChannel){_realtimeChannel.close();_realtimeChannel=null;}
