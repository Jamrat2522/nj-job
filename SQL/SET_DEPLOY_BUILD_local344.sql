-- ═══════════════════════════════════════════════════════════════════════════
-- SET_DEPLOY_BUILD_local344.sql
-- ตั้ง Deploy Build = 1.4.1-local344  (Force Update ระดับ BUILD KEY · RUN-46)
--
...
SELECT 'V1 deploy_build = 1.4.1-local344' AS check_name,
  CASE WHEN (SELECT value FROM public.njacc_settings WHERE key='deploy_build')='1.4.1-local344'
       THEN 'PASS' ELSE 'FAIL' END AS result
UNION ALL SELECT 'V2 njacc_app_status() คืน deploy_build ตัวใหม่',
  CASE WHEN (public.njacc_app_status() ->> 'deploy_build')='1.4.1-local344'
       THEN 'PASS'
...
--   2) ป้ายเวอร์ชันมุมบนซ้ายต้องเป็น  v1.4.1-local344
