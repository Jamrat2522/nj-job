...
function computeDerived(){
  ...
  // ── Active tasks first: สำหรับ filter=all เรียง wait → going → delivered → done ──
  const _STATUS_PRIORITY={wait:0,going:1,delivered:2,done:3};
  _allActive.sort((a,b)=>{
    const pa=_STATUS_PRIORITY[a.status]??4;
    const pb=_STATUS_PRIORITY[b.status]??4;
    if(pa!==pb)return pa-pb;
    // ในกลุ่มเดียวกัน: ล่าสุดก่อน
...
function _getBaseFilter(){
  const u=S.currentUser;
  if(u?.role==="staff"){
    return "created_by=eq."+encodeURIComponent(u.username)+"&order=created_at.desc";
  }
  if(u?.role==="messenger"){
    const me=encodeURIComponent(u.username);
    return "or=(status.eq.wait,accepted_by.eq."+me+",messenger_name.eq."+me+")"
      +"&order=created_at.desc";
  }
  return "order=created_at.desc"; // admin/superadmin
}
...
async function loadMoreArchive(){
  ...
  try{
    ...
    q+="status=eq.done&completed_at=lt."+encodeURIComponent(cutoff)
      +"&order=completed_at.desc&limit="+BATCH+"&offset="+_archiveOffset;
...
function renderCharts(){
  ...
  setTimeout(()=>{
    try{
      ...
      if(c3){
        new Chart(c3,{type:"doughnut",data:{labels:["รอรับงาน","กำลังวิ่ง","ส่งถึงแล้ว","เสร็จแล้ว"],datasets:[{data:[wait,going,delivered,done],backgroundColor:["#f59e0b","#3b82f6","#f97316","#06C755"],borderColor:"#252d3e",borderWidth:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom",labels:{color:"#94a3b8",font:{family:"Sarabun",size:11},padding:12}}},cutout:"65%"}});
      }
      ...
      if(msg.event==="postgres_changes"||msg.payload?.data?.type){
        ...
        if(type==="INSERT"&&record){
          if(!S.tasks.find(t=>t.id===record.id)){
            ...
            // ── INSERT: ไม่ set _forceListRebuild อีก — delta update เท่านั้น ──
            if(S.page==="list"){
              const shouldShow=_isTaskVisibleInCurrentFilter(record);
              if(shouldShow){
                // Prepend เฉพาะเมื่อ task จะอยู่ใน VL window ปัจจุบัน
                // (ถ้า scroll ลงไปไกล — task ใหม่อยู่ที่ top, VL จะ fetch เมื่อ scroll ขึ้น)
                VL._fil=null; // force VL recalc next scroll → window bounds changed
                if(VL._start===0){
                  const list=document.getElementById("vl-list");
                  const topEl=list?.firstElementChild; // vl-top spacer
                  if(list&&topEl&&topEl.id==="vl-top"){
                    const cardHtml=renderCard(record);
                    _cardCache[record.id]=cardHtml;
                    topEl.insertAdjacentHTML("afterend",cardHtml);
                    // Register fresh node in VL map so future scroll/patch finds it
                    if(VL._nodes){
                      const newNode=topEl.nextElementSibling;
                      if(newNode)VL._nodes.set(record.id,newNode);
                    }
                  }
                }
              }
