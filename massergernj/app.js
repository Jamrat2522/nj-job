...
async function uploadFinishImage(id,file,customTime){
  ...
  try{
    ...
    if(res.ok){
      ...
      // ── อัปโหลดไม่สำเร็จ → ห้าม fallback base64 (ทำให้ DB ช้า) ──
      // แจ้ง user + ค้าง modal ไว้ให้กดส่งใหม่
      console.warn("upload failed status",res.status);
      showToast("❌ อัปโหลดรูปไม่สำเร็จ กรุณาลองใหม่","warn");
    }
  }catch(err){
    // network error / timeout → ห้าม fallback base64 เช่นกัน
