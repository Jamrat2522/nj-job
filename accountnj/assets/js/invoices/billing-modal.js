...
export async function openBillingModal({ jobId, charge, onSaved }) {
  ...
  /* ── Footer ──
     ซ้าย : ปุ่ม destructive / ย้อนสถานะ  (Delete · Unpost)
            *** เปลี่ยนเฉพาะ "ป้ายบนปุ่ม" ตามที่กำหนด ***
            id / handler / เงื่อนไขสถานะ / RPC เดิมทั้งหมด ไม่แตะแม้แต่บรรทัดเดียว
            ข้อความในกล่องยืนยัน (askInline) ยังเป็นภาษาไทยเดิม เพื่อให้ผู้ใช้
            อ่านเข้าใจตอนตัดสินใจจริง (ลบร่างใบแจ้งหนี้ / UNPOST ใบแจ้งหนี้)
     ขวา  : ปุ่มหลักเรียงตามลำดับ         Print Draft -> POST -> บันทึก
            *** เปลี่ยนเฉพาะ "ป้ายบนปุ่ม" (บันทึกร่าง -> บันทึก) ***
            id #bm-draft · handler doSaveDraft() · RPC njacc_save_invoice_draft
            · สถานะ DRAFT · เงื่อนไข hidden = posted  ไม่แตะแม้แต่บรรทัดเดียว
  ...
  f.innerHTML = `
    <div class="mf-left">
      <button class="btn btn-del"  id="bm-del"     disabled>🗑 <span>Delete</span></button>
      <button class="btn btn-unpost" id="bm-unpost" disabled>↩ <span>Unpost</span></button>
    </div>
    <div class="mf-right">
      ...
      <button class="btn btn-save" id="bm-draft"          >💾 <span>บันทึก</span></button>
      ...
      bar.innerHTML = posted
        ...
        : (hasDoc
          ...
          : `<span class="bm-state-b">ยังไม่บันทึก</span>
             <span class="bm-state-n">กรอกรายการแล้วกดบันทึก</span>`);
    }

    /* ── Delete / Unpost — แสดงคู่กันตลอด เปลี่ยนแค่ Enabled/Disabled ──────────
       *** เงื่อนไขเดิมไม่ถูกเปลี่ยนแม้แต่ตัวเดียว *** ย้ายจาก .hidden -> .disabled
         เดิม  B('#bm-del').hidden    = posted || !hasDoc
               B('#bm-unpost').hidden = !posted
         ใหม่  ทั้งคู่ hidden = false เสมอ · ใช้นิพจน์เดิมเป็น .disabled แทน
       -> Draft   : Delete กดได้ (ถ้ามีร่างแล้ว) · Unpost แสดงแต่ disabled
          POSTED  : Unpost กดได้ · Delete ยังแสดงแต่ disabled (RPC ก็บล็อกอีกชั้น)
          ยังไม่บันทึก (!hasDoc) : ทั้งคู่ disabled — ยังไม่มีร่างให้ลบ/ย้อนสถานะ
       *** ไม่เพิ่มสิทธิ์ลบใหม่ *** ไม่แตะ Handler / RPC / Role / Running Number
       ปุ่มที่ disabled คลิกไม่ติดโดยตัวเบราว์เซอร์เอง -> onclick เดิมไม่ถูกเรียก */
    B('#bm-del').hidden = false;
    B('#bm-unpost').hidden = false;
    B('#bm-del').disabled = posted || !hasDoc;
    B('#bm-unpost').disabled = !posted;
    /* ปุ่มฝั่งขวายังใช้ Logic เดิมทุกประการ (ซ่อนตามสถานะ) — ไม่อยู่ในขอบเขตรอบนี้ */
  ...
  async function doSaveDraft(btn, silent) {
    ...
    try {
      ...
      if (!silent) toast('บันทึกแล้ว (' + res.items + ' รายการ)', 'ok');
      return res;
    } catch (ex) { handleErr(ex); return null; }
    finally { if (btn) btnBusy(btn, false); }
  }

  /* ── ปุ่ม "💾 บันทึก" — บันทึกสำเร็จจริงแล้วจึงปิดหน้าต่าง ────────────────────
     *** ใส่ Logic ปิดที่ onclick ไม่ใช่ใน doSaveDraft() ***
     เพราะ doSaveDraft() ถูกเรียกจากอีก 2 ที่แบบ silent:
       #bm-print -> openDraftDoc()  : บันทึกล่าสุดก่อนเปิด Preview
       #bm-post                     : บันทึกล่าสุดก่อน POST
     ทั้งสองต้องทำงานต่อใน Modal เดิม -> ห้ามปิด
     ── ลำดับ ──
       doSaveDraft() -> collect() validate -> saveInvoiceDraft() (RPC)
       -> setDraftState() -> toast 'บันทึกแล้ว'
       คืน null เมื่อ validate ไม่ผ่าน หรือ RPC error (handleErr แสดงข้อความแล้ว)
       -> return ทันที: ไม่ปิด Modal · ไม่รีเฟรช · ข้อมูลที่กรอกอยู่ครบ
       สำเร็จเท่านั้นจึง onSaved() แล้ว closeModal()
     ── กันบันทึกซ้ำ ──
       btnBusy() disable ปุ่มตลอดช่วงที่รอ RPC (finally คืนค่าเสมอ)
       njacc_save_invoice_draft เป็น upsert ต่อ 1 JOB -> กดซ้ำก็อัปเดตร่างเดิม
       ไม่สร้าง INVOICE ใบใหม่ · ไม่กินเลข (เลขจริงออกตอน POST เท่านั้น)
     ── ไม่แตะ ── POST · Print Draft · Delete · Unpost · Status · RPC · SQL */
  f.querySelector('#bm-draft').onclick = async (e) => {
    const res = await doSaveDraft(e.target, false);
    if (!res) return;
    if (typeof onSaved === 'function') onSaved();   /* รีเฟรชรายการเบื้องหลัง
        (รูปแบบเดียวกับ Delete/POST ที่เรียก onSaved() หลังทำงานสำเร็จ) */
    closeModal();
  };
