...
/* headExtra = HTML เสริมในแถบหัว Modal (opt-in) — วางระหว่าง <h3> กับปุ่ม ✕
   ไม่ส่งมา -> ไม่มี element เกิดขึ้นเลย -> Modal อื่นทั้ง 24 จุดไม่กระทบแม้แต่นิดเดียว */
export function openModal({ title, body, footer, large, fullscreen, wide, cls, headExtra }) {
  closeModal();
  ...
  bk.innerHTML = `<div class="${mCls}">
    <div class="modal-h"><h3>${esc(title)}</h3>
      ${headExtra ? `<div class="modal-h-x">${headExtra}</div>` : ''}
