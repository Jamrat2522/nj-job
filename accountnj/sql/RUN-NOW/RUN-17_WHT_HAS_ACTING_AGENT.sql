-- ═══════════════════════════════════════════════════════════════════════════
-- RUN-17_WHT_HAS_ACTING_AGENT.sql   (ACCOUNT V.196)
-- REPORT > ใบหัก ณ ที่จ่าย — "กระทำการแทนโดย" เป็นตัวเลือกของแต่ละเอกสาร
--
-- ═══ สิ่งที่ไฟล์นี้ทำ ══════════════════════════════════════════════════════
--   (1) njacc_withholding_docs.has_acting_agent boolean *** NULLABLE ***
--       true  = เอกสารนี้มีกระทำการแทน
--       false = ไม่มี
--       NULL  = เอกสารที่บันทึกก่อนรันไฟล์นี้ (ยังไม่เคยระบุ)
--       *** ไม่ตั้ง DEFAULT และไม่ UPDATE ข้อมูลเดิมแม้แถวเดียว ***
--       เอกสารเก่าอ่านสถานะจาก agent_name ที่มีอยู่จริง (ดู VERIFY V4)
--
--   (2) njacc_save_wht_draft — รับ has_acting_agent
--       ไม่ติ๊ก -> เขียน agent_name/tax_id/branch/address เป็น NULL ที่ SQL ด้วย
--       เพื่อให้เอกสารนั้น "ไม่มีกระทำการแทน" จริง ไม่ใช่แค่ซ่อนบนหน้าจอ
--
--   njacc_wht_view ไม่ต้องแก้ — ใช้ to_jsonb(w) คอลัมน์ใหม่จึงไหลออกไปเอง
--   njacc_list_wht ไม่ต้องแก้ — หน้ารายการยังไม่มีความจำเป็นต้องใช้คอลัมน์นี้
--
-- ═══ ไม่ทำ ════════════════════════════════════════════════════════════════
--   ไม่ DROP · ไม่ DELETE · ไม่ TRUNCATE · ไม่ UPDATE ข้อมูลเดิม
--   ไม่แตะ: direction · njacc_post_wht · njacc_unpost_wht · njacc_delete_wht_draft ·
--           njacc_wht_view · njacc_list_wht · njacc_wht_items · การคำนวณภาษี ·
--           RLS · Policy · Role/Permission
--   รันซ้ำได้ปลอดภัย
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                  WHERE n.nspname='public' AND p.proname='njacc_save_wht_draft') THEN
    RAISE EXCEPTION 'PREFLIGHT FAIL — ไม่พบ njacc_save_wht_draft (ต้องรัน RUN-14 ก่อน)'; END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema='public' AND table_name='njacc_withholding_docs'
                    AND column_name='agent_name') THEN
    RAISE EXCEPTION 'PREFLIGHT FAIL — ไม่พบคอลัมน์ agent_name (ต้องรัน RUN-08 ก่อน)'; END IF;
  RAISE NOTICE 'PREFLIGHT PASS';
END $preflight$;

-- P1 · เอกสารเดิมที่มีข้อมูลกระทำการแทน -> หลังรันจะขึ้น Checkbox ติ๊กให้อัตโนมัติ
SELECT 'P1 เอกสารเดิมที่มีกระทำการแทน' AS check_item,
       count(*) FILTER (WHERE agent_name IS NOT NULL) AS มี,
       count(*) FILTER (WHERE agent_name IS NULL)     AS ไม่มี,
       count(*) AS ทั้งหมด
...
ALTER TABLE public.njacc_withholding_docs
  ADD COLUMN IF NOT EXISTS has_acting_agent boolean;

COMMENT ON COLUMN public.njacc_withholding_docs.has_acting_agent IS
  'เอกสารนี้มี "กระทำการแทนโดย" หรือไม่ · NULL = เอกสารก่อน V.196 '
  '(ให้อ่านสถานะจาก agent_name แทน) · คนละเรื่องกับคอลัมน์ direction';

-- 2.2 · njacc_save_wht_draft — ยกนิยามจริงมาทั้งดุ้น เปลี่ยนเฉพาะส่วน agent
...
-- ───────────────────────────────────────────────────────────────────────────
SELECT 'V1 คอลัมน์ has_acting_agent ถูกเพิ่มแล้ว' AS check_item,
 ...
 WHERE table_schema='public' AND table_name='njacc_withholding_docs'
   AND column_name='has_acting_agent'

UNION ALL
SELECT 'V2 คอลัมน์ agent_* เดิมยังอยู่ครบ 4 ตัว',
       string_agg(column_name, ', ' ORDER BY column_name),
       CASE WHEN count(*)=4 THEN 'PASS' ELSE 'FAIL' END
  FROM information_schema.columns
 WHERE table_schema='public' AND table_name='njacc_withholding_docs'
   AND column_name IN ('agent_name','agent_tax_id','agent_branch','agent_address')

UNION ALL
SELECT 'V3 save_wht_draft รับ has_acting_agent และล้าง agent เมื่อไม่ติ๊ก',
       'v_has_agent',
       CASE WHEN pg_get_functiondef('public.njacc_save_wht_draft(jsonb)'::regprocedure)
                   LIKE '%has_acting_agent%'
             AND pg_get_functiondef('public.njacc_save_wht_draft(jsonb)'::regprocedure)
                   LIKE '%CASE WHEN v_has_agent THEN nullif(btrim(coalesce(p->>''agent_name''%'
            THEN 'PASS' ELSE 'FAIL' END

UNION ALL
SELECT 'V4 เอกสารเก่าไม่ถูกแก้ (has_acting_agent ยังเป็น NULL)',
       count(*)::text || ' ใบ · ในนั้นมี agent_name อยู่ '
         || count(*) FILTER (WHERE agent_name IS NOT NULL)::text || ' ใบ',
       'อ่านอย่างเดียว'
  FROM public.njacc_withholding_docs
 WHERE has_acting_agent IS NULL

UNION ALL
SELECT 'V5 njacc_wht_view ส่งคอลัมน์ใหม่ออกไปเอง (to_jsonb)',
       'to_jsonb(w)',
       CASE WHEN pg_get_functiondef('public.njacc_wht_view(uuid)'::regprocedure)
                   LIKE '%to_jsonb(w)%'
            THEN 'PASS' ELSE 'FAIL' END;


-- ═══ ROLLBACK ══════════════════════════════════════════════════════════════
--   รัน RUN-NOW/RUN-14_WHT50_CATEGORY_FUNDS.sql ใหม่
--   (คืน njacc_save_wht_draft เป็นชุดก่อนหน้า)
--   คอลัมน์ has_acting_agent ปล่อยไว้ได้ ไม่มีผลกับโค้ดชุดเก่า
