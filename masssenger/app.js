...
console.log("%c[NJ Dispatch BUILD]","color:#06C755;font-weight:bold","1775622050-finalclose","| Loaded at",new Date().toLocaleTimeString());

// ── EARLY GLOBAL: __closeDetail (กัน script defer + cache stale ปัญหา) ──
// ฟังก์ชันนี้ติด window ทันทีตอน parse app.js — ไม่ต้องรอ DOMContentLoaded
window.__closeDetail=function(){
  // Method 1: setState (ปกติ)
  try{if(typeof setState==="function"){setState({selId:null});return;}}catch(e){console.warn("close m1",e);}
  // Method 2: Direct state mutation + render
  try{if(typeof S==="object"&&S){S.selId=null;}if(typeof render==="function"){render();return;}}catch(e){console.warn("close m2",e);}
  // Method 3: Clear modal zone DOM
  try{const m=document.getElementById("zone-modal");if(m){m.innerHTML="";return;}}catch(e){console.warn("close m3",e);}
  // Method 4: Reload (last resort)
  console.error("__closeDetail: all methods failed, reload");
  location.reload();
};

// ── Document delegation — clicks on [data-close="detail"] always close detail modal ──
// กันทุก edge case: cache เก่า, scope, button replaced
if(typeof document!=="undefined"){
  document.addEventListener("click",function(e){
    const el=e.target&&e.target.closest&&e.target.closest('[data-close="detail"]');
    if(el){e.stopPropagation();window.__closeDetail();}
  },true); // capture phase
  // ESC = close detail modal
  document.addEventListener("keydown",function(e){
    if((e.key==="Escape"||e.keyCode===27)&&typeof S==="object"&&S&&S.selId){
      window.__closeDetail();
    }
  });
  // Mark as bound — กันโค้ดด้านล่างที่จะ register ซ้ำ
  window._escBound=true;
  window._delegClickBound=true;
}
...
// Global: last Supabase insert error (สำหรับ toast แสดง error จริง)
let _lastSbErr=null;
async function sbInsert(t,d){
  try{
    ...
    if(!r.ok){
      ...
      _lastSbErr=r.status+" "+(errText?errText.slice(0,80):r.statusText);
      return false;
    }
    _lastSbErr=null;
    return true;
  }catch(e){
    console.error("sbInsert error",e);
    _lastSbErr=e.message||"network error";
    return false;
  }
}
...
function invalidateZone(id){delete _zoneCache[id];if(id==="zone-hdr")delete _zoneCache["_hdrFp"];} // บังคับ re-render zone นั้น
...
// ── Prune stale entries: ลบ cache ของ task ที่ไม่อยู่ใน S.tasks แล้ว (กัน memory leak บน long session) ──
function _pruneCardCache(){
  const valid=new Set(S.tasks.map(t=>t.id));
  for(const id in _cardCache){
    if(!valid.has(id))delete _cardCache[id];
  }
}

// ════════════════════════════════════════════════════════
//  ANDROID + MOBILE SAFETY (สำหรับ Mass บนมือถือ)
// ════════════════════════════════════════════════════════
const _UA = navigator.userAgent || "";
const IS_ANDROID = /Android/i.test(_UA);
const IS_IOS     = /iPhone|iPad|iPod/i.test(_UA);
const IS_MOBILE  = IS_ANDROID || IS_IOS || /Mobile/i.test(_UA);
const IS_OLD_ANDROID = IS_ANDROID && /Android [4-7]\./.test(_UA); // Android เก่า
// Line in-app browser: UA จะมี "Line/x.x.x" — block WSS / upload เพี้ยนบ่อย
const IS_LINE_WEBVIEW = /Line\//i.test(_UA);

// Mobile fetch limits — ลด JSON parse + memory
const FETCH_INIT  = IS_MOBILE ? 15 : 30;
const FETCH_BATCH = IS_MOBILE ? 15 : 30;
const MOBILE_TASK_CAP = 120; // cap S.tasks บน mobile กัน OOM

// Tag <html> ให้ CSS targeting ได้
try{
...
// ── Line WebView Banner — เตือนให้เปิดใน Chrome/Safari ──
// (Line in-app browser block WSS, อัปโหลดรูปเพี้ยน, ไม่มี Add to Home Screen)
function _showLineBanner(){
  if(!IS_LINE_WEBVIEW)return;
  // ถ้า user dismiss ไปแล้วใน session นี้ → ไม่แสดงอีก
  try{if(sessionStorage.getItem("lineBannerDismissed")==="1")return;}catch{}
  // ป้องกัน double mount
  if(document.getElementById("line-warn-banner"))return;
  const url=location.href;
  const html=
    '<div id="line-warn-banner" style="position:fixed;top:0;left:0;right:0;z-index:9999;background:linear-gradient(135deg,#dc2626,#b91c1c);color:#fff;padding:10px 14px;font-family:Sarabun,sans-serif;font-size:13px;line-height:1.4;box-shadow:0 4px 12px rgba(220,38,38,.4);display:flex;align-items:flex-start;gap:10px">'
      +'<div style="font-size:20px;flex-shrink:0">⚠️</div>'
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-weight:800;margin-bottom:3px">คุณเปิดผ่าน LINE — ระบบจะช้าและรูปอัปโหลดอาจเพี้ยน</div>'
        +'<div style="font-size:11.5px;opacity:.95;margin-bottom:6px">ให้กด ⋮ มุมขวาบน → "เปิดในเบราว์เซอร์อื่น" → เลือก Chrome / Safari</div>'
        +'<div style="display:flex;gap:6px;flex-wrap:wrap">'
          +'<button onclick="_copyAppUrl()" style="background:rgba(255,255,255,.22);border:1px solid rgba(255,255,255,.4);color:#fff;border-radius:6px;font-size:11px;padding:4px 10px;cursor:pointer;font-family:Sarabun,sans-serif;font-weight:700">📋 คัดลอกลิงก์</button>'
          +'<button onclick="_dismissLineBanner()" style="background:transparent;border:1px solid rgba(255,255,255,.4);color:#fff;border-radius:6px;font-size:11px;padding:4px 10px;cursor:pointer;font-family:Sarabun,sans-serif">เข้าใจแล้ว</button>'
        +'</div>'
      +'</div>'
    +'</div>';
  ...
  // เพิ่ม padding-top body เพื่อให้ banner ไม่ทับ content
  document.body.style.paddingTop="110px";
}
function _dismissLineBanner(){
  const el=document.getElementById("line-warn-banner");
  if(el)el.remove();
  document.body.style.paddingTop="";
  try{sessionStorage.setItem("lineBannerDismissed","1");}catch{}
}
function _copyAppUrl(){
  const url=location.href;
  if(navigator.clipboard?.writeText){
    navigator.clipboard.writeText(url).then(()=>{
      try{showToast("✅ คัดลอกลิงก์แล้ว — เปิดใน Chrome/Safari","ok");}catch{}
    }).catch(()=>{
      try{prompt("คัดลอกลิงก์นี้:",url);}catch{}
    });
  }else{
    try{prompt("คัดลอกลิงก์นี้:",url);}catch{}
  }
}
window._dismissLineBanner=_dismissLineBanner;
window._copyAppUrl=_copyAppUrl;
// Mount banner หลัง DOM พร้อม
if(typeof document!=="undefined"){
  if(document.readyState==="loading"){
    document.addEventListener("DOMContentLoaded",_showLineBanner,{once:true});
  }else{
    setTimeout(_showLineBanner,0);
  }
}

// Cleanup หลัง mutate S.tasks (เรียกหลัง loadTasks/realtime updates)
function _postUpdateCleanup(){
  _pruneCardCache();
  // ลบ VL nodes ที่ไม่อยู่ใน S.tasks แล้ว
  if(VL._nodes&&VL._nodes.size){
    const live=new Set(S.tasks.map(t=>t.id));
    for(const [id,node] of VL._nodes){
      if(!live.has(id)){node.remove();VL._nodes.delete(id);}
    }
  }
  // Cap mobile (ยกเว้น filter=archive ที่ user ตั้งใจดูเก่า)
  if(IS_MOBILE&&S.filter!=="archive"&&S.tasks.length>MOBILE_TASK_CAP){
    S.tasks.sort((a,b)=>(b.created_at||"").localeCompare(a.created_at||""));
    const dropped=S.tasks.splice(MOBILE_TASK_CAP);
    for(const t of dropped)delete _cardCache[t.id];
  }
}
...
function patchCard(taskId){
  ...
  el.remove();
  // Sync VL node map — fresh node replaces old ref
  if(VL._nodes&&next)VL._nodes.set(taskId,next);
  return true;
}
...
let _stateRev=0;    // revision counter — bump ทุกครั้งที่ S.tasks/S.users mutate (stronger than length-only)
function _bumpRev(){_stateRev=(_stateRev+1)|0;}
...
function computeDerived(){
  // Fingerprint: _stateRev bump ทุกครั้งที่ task/user mutate → catch in-place updates (accept/deliver/finish ฯลฯ) ที่ length ไม่เปลี่ยน
  const k=_stateRev+"|"+(S.filter)+"|"+(S.typeFilter||"")+"|"+(S.currentUser?.username||"")+"|"+(S.role||"");
  ...
  // ── Single-pass bucketing: เดิมวน S.tasks 8 รอบ (filter wait/going/delivered/done/archive × 2/active/count) ──
  // รอบเดียว: แยกเข้า bucket ตาม status + archive flag พร้อมนับไปเลย
  const wait=[], going=[], delivered=[], done=[], archived=[], recentDone=[], _allActive=[];
  let cntActiveAll=0;
  for(let i=0;i<tasks.length;i++){
    const t=tasks[i];
    const s=t.status;
    if(s==="wait"){ wait.push(t); _allActive.push(t); cntActiveAll++; }
    else if(s==="going"){ going.push(t); _allActive.push(t); cntActiveAll++; }
    else if(s==="delivered"){ delivered.push(t); _allActive.push(t); cntActiveAll++; }
    else if(s==="done"){
      done.push(t);
      if(isArchived(t)){ archived.push(t); }
      else { recentDone.push(t); _allActive.push(t); cntActiveAll++; }
    }
  }
  // ── Sort rule (FINAL PATCH): created_at ASC + id ASC tiebreaker ──
  // งานสั่งก่อน อยู่บน — ห้ามใช้ status priority, updated_at, accepted_at, completed_at
  const _cmpAsc=(a,b)=>{
  ...
  _allActive.sort(_cmpAsc);
  wait.sort(_cmpAsc);
  going.sort(_cmpAsc);
  delivered.sort(_cmpAsc);
  recentDone.sort(_cmpAsc);
  archived.sort(_cmpAsc);
  const _statusFil=S.filter==="all"?_allActive
    :S.filter==="wait"?wait
    :S.filter==="going"?going
    :S.filter==="delivered"?delivered
    :S.filter==="archive"?archived
    :recentDone;
  // typeFilter: กรอง AND บนสถานะ (typeFilter="" = แสดงทั้งหมด) — normalize เพื่อให้ค่าเก่ายังกรองเข้ากลุ่มใหม่ได้
  const fil=S.typeFilter?_statusFil.filter(t=>_getType(t.type)===S.typeFilter):_statusFil;
  _derived={
    tasks,wait,going,delivered,done,fil,archived,recentDone,
    cnt:{all:cntActiveAll,wait:wait.length,going:going.length,delivered:delivered.length,done:recentDone.length,archive:archived.length},
...
function invalidateDerived(){_derived=null;_derivedKey="";_taskIndex=null;_bumpRev();}

// ── เช็คว่า task 1 ใบจะ visible ใน current filter หรือไม่ (cheap — ไม่ต้อง compute ใหม่ทั้ง list) ──
function _isTaskVisibleInCurrentFilter(t){
  if(!t)return false;
  // Role-based visibility (เหมือน computeDerived)
  ...
  if(!isAdmin){
    if(role==="messenger"){
      if(t.status!=="wait"&&(t.accepted_by||t.messenger_name)!==me)return false;
    }else{
      if(t.created_by!==me)return false;
    }
  }
  // Status filter
  const archived=isArchived(t);
  if(S.filter==="all"&&archived)return false;
  else if(S.filter==="wait"&&t.status!=="wait")return false;
  else if(S.filter==="going"&&t.status!=="going")return false;
  else if(S.filter==="delivered"&&t.status!=="delivered")return false;
  else if(S.filter==="done"){if(t.status!=="done"||archived)return false;}
  else if(S.filter==="archive"){if(t.status!=="done"||!archived)return false;}
  // Type filter
  if(S.typeFilter&&_getType(t.type)!==S.typeFilter)return false;
  return true;
}
...
async function updateUserRole(id,role){
  ...
  if(S.currentUser?.id===id){S.currentUser={...S.currentUser,role};sessionStorage.setItem("msg_cu",JSON.stringify(S.currentUser));}
}
...
async function getJobNo(date,skipCount){
  // ── FINAL PATCH: numeric max from job_number prefix ──
  // ห้ามใช้ string sort / order=desc / localeCompare
  // ใช้ Math.max หา highest แบบ numeric เท่านั้น
  if(typeof skipCount!=="number")skipCount=0;
  const prefix=date.replace(/-/g,"")+"-"; // "20260507-"

  if(DEMO){
    const same=S.tasks.filter(t=>t.date===date);
    return prefix+(same.length+1+skipCount);
  }

  try{
    // Query เฉพาะ job_number ที่ขึ้นต้นด้วย prefix นี้
    // (ไม่ใช้ date=eq เพราะ column date type อาจเป็น text หรือ date — prefix-based ปลอดภัยกว่า)
    const rows=await sbGet(
      "msg_tasks",
      "job_number=like."+prefix+"*&select=job_number&limit=500"
    );

    let max=0;
    for(const r of rows||[]){
      const j=r.job_number||"";
      if(!j.startsWith(prefix))continue;
      const num=parseInt(j.slice(prefix.length),10);
      if(!isNaN(num)&&num>max){
        max=num;
      }
    }

    const next=max+1+skipCount;

    console.log("JOBNO_MAX",max);
    console.log("JOBNO_NEXT",next);

    return prefix+next;
  }catch(e){
    console.error("getJobNo error",e);
    // Last resort: timestamp-based suffix (เลขจะเอกลักษณ์ ไม่ชนใคร)
...
const S={
  ...
  fJob:"",fCompany:"",fJobNJ:"",fType:"รับ-ส่งทั่วไป",fDetail:"",fPlace:"",
  ...
  // ── Dashboard date filter (กรอง งานเสร็จตาม "วันที่ทำงาน" t.date) ──
  dashDateFrom:"",dashDateTo:"",
  listPage:1, // legacy — VL no longer uses pagination
};
try{const cu=JSON.parse(sessionStorage.getItem("msg_cu")||"null");if(cu)S.currentUser=cu;}catch{}
// ── Detail Modal Close — Multiple bulletproof mechanisms ──
// (ใช้แทน inline onclick ที่อาจมีปัญหา cache/scope)
function __closeDetail(){
  try{setState({selId:null});}catch(e){
    console.error("__closeDetail fail",e);
    // Fallback: direct mutation + force render
    try{S.selId=null;if(typeof render==="function")render();}catch{}
  }
}
window.__closeDetail=__closeDetail;

// ESC key → close detail modal (ถ้าเปิดอยู่)
if(typeof document!=="undefined"&&!window._escBound){
  window._escBound=true;
  document.addEventListener("keydown",function(e){
    if(e.key==="Escape"||e.keyCode===27){
      if(S&&S.selId){__closeDetail();}
    }
  });
}

// Delegated click — รับคลิกจาก [data-close="detail"] บน modal X button (เผื่อ inline onclick ล้ม)
if(typeof document!=="undefined"&&!window._delegClickBound){
  window._delegClickBound=true;
  document.addEventListener("click",function(e){
    const el=e.target&&e.target.closest&&e.target.closest('[data-close="detail"]');
    if(el){
  ...
  },true); // capture phase — fires ก่อน inline handlers
}

function setState(obj){
  // ถ้าเปิดหน้า create ปกติ (ไม่ใช่ copy) → reset ทุก field
  if(obj.page==="create"&&!S._isCopy){
    S.fCompany="";S.fJobNJ="";S.fType="รับ-ส่งทั่วไป";S.fDetail="";
    S.fPlace="";S.fPickup="";S.fDeliver="";S.fDate=todayStr();
    S.fCloseMode="messenger_finish";S.fFile=null;S.fPreview=null;
    S.fInspectDate="";S.fPayDate="";
  }
  if(obj.page==="create")S._isCopy=false; // reset flag หลัง set page
  // ── Force rebuild เฉพาะเมื่อ list identity เปลี่ยนจริงๆ ──
  // filter/typeFilter เปลี่ยน → เปลี่ยน fil[] → rebuild list
  ...
  // page เปลี่ยน list ↔ other pages → rebuild เฉพาะตอนเข้า list ใหม่
  if("page" in obj&&obj.page!==S.page&&(obj.page==="list"||S.page==="list"))_forceListRebuild=true;
  // selId เปลี่ยน = แค่ modal เปิด/ปิด → list เดิมไม่กระทบ ไม่ rebuild
  Object.assign(S,obj);
  // ── Archive lazy load: trigger เมื่อคลิก archive filter (ครั้งแรก) ──
  if("filter" in obj&&obj.filter==="archive"&&!_archiveLoaded&&!_archiveFetching&&!DEMO){
    loadMoreArchive();
  }
...
// ── Infinite scroll / Archive lazy load state ──
let _moreLoading=false;      // ป้องกัน concurrent loadMore
let _noMoreBase=false;       // DB หมดแล้ว (active tasks)
let _archiveLoaded=false;    // archive โหลดครบแล้ว (offset หมด)
let _archiveOffset=0;        // offset สำหรับ pagination
let _archiveFetching=false;  // trigger กันซ้ำ
let _baseFetchedCount=0;     // จำนวน rows ที่ fetch มาจาก base query (ไม่รวม archive / realtime inserts)

// ── Build PostgREST base filter ตาม role (reuse ทั้ง loadTasks / loadMoreTasks) ──
// IMPORTANT: server fetch DESC (ล่าสุดก่อน) เพื่อให้ได้งาน active (wait/going) ที่อยู่บนสุด
// Client จะ sort ASC อีกครั้งใน computeDerived เพื่อแสดง "งานสั่งก่อนอยู่บน" ใน UI
function _getBaseFilter(){
  const u=S.currentUser;
  const SORT="&order=created_at.desc,id.desc"; // server: ล่าสุดก่อน → ได้งาน active เสมอ
  if(u?.role==="staff"){
    return "created_by=eq."+encodeURIComponent(u.username)+SORT;
  }
  if(u?.role==="messenger"){
    const me=encodeURIComponent(u.username);
    return "or=(status.eq.wait,accepted_by.eq."+me+",messenger_name.eq."+me+")"+SORT;
  }
  return SORT.slice(1); // admin/superadmin (strip leading &)
}

// ── Infinite scroll: โหลด active tasks เพิ่ม (offset-based) ──
async function loadMoreTasks(){
  if(_moreLoading||_noMoreBase||DEMO||!S.currentUser)return;
  _moreLoading=true;
  try{
    const BATCH=FETCH_BATCH; // mobile=15, desktop=30
    const fetched=await sbGet("msg_tasks",_getBaseFilter()+"&limit="+BATCH+"&offset="+_baseFetchedCount);
    if(!fetched||fetched.length<BATCH)_noMoreBase=true;
    if(fetched&&fetched.length){
      _baseFetchedCount+=fetched.length;
      const existing=new Set(S.tasks.map(t=>t.id));
      const fresh=fetched.filter(t=>!existing.has(t.id));
      if(fresh.length){
        S.tasks=S.tasks.concat(fresh);
        invalidateDerived();_taskIndex=null;VL._fil=null;
        _postUpdateCleanup(); // prune + cap mobile
        if(S.page==="list")render();
      }
    }
  }catch(e){console.warn("loadMoreTasks error",e);}
  _moreLoading=false;
}

// ── Archive lazy load: fetch done tasks เก่ากว่า 30 วัน on-demand ──
async function loadMoreArchive(){
  if(_archiveFetching||_archiveLoaded||DEMO||!S.currentUser)return;
  _archiveFetching=true;
  try{
    const u=S.currentUser;
    const cutoff=new Date(Date.now()-ARCHIVE_DAYS*86400000).toISOString();
    let q="";
    if(u.role==="staff")q="created_by=eq."+encodeURIComponent(u.username)+"&";
    else if(u.role==="messenger"){
      const me=encodeURIComponent(u.username);
      q="or=(accepted_by.eq."+me+",messenger_name.eq."+me+")&";
    }
    const BATCH=30;
    q+="status=eq.done&completed_at=lt."+encodeURIComponent(cutoff)
      +"&order=created_at.desc,id.desc&limit="+BATCH+"&offset="+_archiveOffset;
    const fetched=await sbGet("msg_tasks",q);
    _archiveOffset+=BATCH;
    if(!fetched||fetched.length<BATCH)_archiveLoaded=true;
    if(fetched&&fetched.length){
      const existing=new Set(S.tasks.map(t=>t.id));
      const fresh=fetched.filter(t=>!existing.has(t.id));
      if(fresh.length){
        S.tasks=S.tasks.concat(fresh);
        invalidateDerived();_taskIndex=null;VL._fil=null;
        if(S.page==="list"&&S.filter==="archive")render();
      }
    }
  }catch(e){console.warn("loadMoreArchive error",e);}
  _archiveFetching=false;
}

async function loadTasks(force=false){
  const now=Date.now();
  // ── Pause ทั้งหมดเมื่อ tab ซ่อน (ประหยัด network + CPU) ──
  ...
  const baseFilter=_getBaseFilter();
  const isFirstLoad=force||S.tasks.length===0;

  if(isFirstLoad){
    // ── Mobile: 15 รายการ / Desktop: 30 — UI เปิดเร็วสุดบน Android เก่า ──
    if(force){_noMoreBase=false;_archiveLoaded=false;_archiveOffset=0;_baseFetchedCount=0;}
    const first=await sbGet("msg_tasks",baseFilter+"&limit="+FETCH_INIT);
    if(first.length>0||S.tasks.length===0){S.tasks=first;}
    _baseFetchedCount=first.length;
    if(first.length<FETCH_INIT)_noMoreBase=true;
    invalidateDerived();_taskIndex=null;VL._fil=null;
    _postUpdateCleanup(); // prune + cap mobile
    ...
    // ── Poll ปกติ: FETCH_INIT (mobile=15, desktop=30) ──
    const fetched=await sbGet("msg_tasks",baseFilter+"&limit="+FETCH_INIT);
    if(fetched.length>0||S.tasks.length===0||force){S.tasks=fetched;_baseFetchedCount=fetched.length;}
    if(fetched.length<FETCH_INIT)_noMoreBase=true;
    invalidateDerived();_taskIndex=null;VL._fil=null;
    _postUpdateCleanup();
...
async function createTask(){
  ...
  try{
    ...
    // ── นul empty strings สำหรับ date columns (Postgres ไม่รับ "" สำหรับ type=date) ──
    const _safeDate=v=>(v&&typeof v==="string"&&v.trim())?v:null;
    const task={id:uid(),job_number:jobNo,job_nj:jobnj,company,type,detail,place,
      date:_safeDate(date)||todayStr(),
      pickup_time:S.fPickup,deliver_time:S.fDeliver||"–",
      inspect_date:_safeDate(inspectDate),
      pay_date:_safeDate(payDate),
    ...
    if(!DEMO){
      let ok=await sbInsert("msg_tasks",task);
      // ── Retry loop on 409 (duplicate job_number) — race condition / inconsistent DB ──
      // ลองได้สูงสุด 5 ครั้ง — แต่ละครั้งใช้ skipCount เพิ่มเพื่อข้ามเลขที่ชน
      const _isDup=()=>_lastSbErr&&(_lastSbErr.indexOf("23505")>=0||_lastSbErr.indexOf("duplicate")>=0||_lastSbErr.indexOf("409 ")===0);
      let attempt=0;
      while(!ok&&attempt<5&&_isDup()){
        attempt++;
        console.warn("createTask: duplicate, retry attempt "+attempt);
        const newJobNo=await getJobNo(date,attempt); // skipCount = attempt → ข้ามเลขขึ้น
        if(newJobNo&&newJobNo!==task.job_number){
          task.job_number=newJobNo;
          ok=await sbInsert("msg_tasks",task);
        }else{
          break; // ได้เลขเดิม → ไม่ช่วย
        }
      }
      if(!ok){
        // แสดง error จริงจาก server เพื่อให้ debug ง่าย
        const detail=_lastSbErr?(" — "+_lastSbErr):"";
        showToast("❌ บันทึกไม่สำเร็จ"+detail,"warn");
        setState({saving:false});
        _creatingTask=false;
        return;
      }
      // ใช้ jobNo ที่จริง (อาจเปลี่ยนจาก retry)
      ...
      invalidateDerived();
      // _forceListRebuild=true; ← ลบออก: setState ด้านล่าง set เองจาก filter+typeFilter+page change
    }
    ...
    showToast("❌ เกิดข้อผิดพลาด — "+msg.slice(0,80),"warn");
    setState({saving:false});
  }finally{
    _creatingTask=false;
  }
}
// ── FIX (STAGE 2): conditional UPDATE — UPDATE เฉพาะถ้า status ยัง match ──
// PostgREST: ?id=eq.X&status=eq.wait → return rows ที่ update สำเร็จ
// ถ้า return [] → คนอื่นรับไปแล้ว (race condition)
async function _sbUpdateIfStatus(t, id, requiredStatus, d){
  try{
    const r = await fetch(
      API+"/"+t+"?id=eq."+id+"&status=eq."+encodeURIComponent(requiredStatus),
      {method:"PATCH", headers:{...SH, Prefer:"return=representation"}, body:JSON.stringify(d)}
    );
    if(!r.ok) return null;
    const rows = await r.json().catch(()=>[]);
    return Array.isArray(rows)&&rows.length>0 ? rows[0] : null; // null = ไม่มี row matched (race)
  }catch(e){console.error("_sbUpdateIfStatus error",e); return null;}
}
...
async function acceptTask(id){
  ...
  // ── Race-safe accept: UPDATE เฉพาะถ้า status ยัง = "wait" บน server ──
  const updated = await _sbUpdateIfStatus("msg_tasks", id, "wait", updates);
  if(!updated){
    // อีกคนรับไปแล้ว → reload task ใบนี้จาก server แสดงสถานะจริง
    showToast("⚠️ งานนี้ถูกรับโดยแมสคนอื่นแล้ว","warn");
    try{
      const rows = await sbGet("msg_tasks","id=eq."+id+"&limit=1");
      if(rows&&rows[0]){
        const fresh = rows[0];
        S.tasks = S.tasks.map(x=>x.id===id ? fresh : x);
        invalidateDerived();invalidateCardCache(id);
        // patch UI โดยไม่ rebuild ทั้งหน้า
        if(typeof patchCard==="function") patchCard(id);
    ...
    }catch(e){console.warn("acceptTask reload failed",e);}
    return;
  }

  // สำเร็จ — ใช้ row จาก server (อาจมี timestamp ที่ server gen เอง)
  S.tasks=S.tasks.map(x=>x.id===id?{...x,...updated}:x);
  invalidateDerived();
...
async function deleteTask(id){
  ...
  invalidateDerived();invalidateCardCache(id);
  _pruneCardCache(); // cleanup stale entries
  _invalidateStatEls();
...
// ── Copy JOB: สร้างใบงานใหม่โดย copy ข้อมูลจากใบงานเดิม ──
function copyJob(id){
  const t=S.tasks.find(x=>x.id===id);
  if(!t){showToast("⚠️ ไม่พบงาน","warn");return;}
  // ล้าง form แล้วใส่ข้อมูลจาก task เดิม
  S.fCompany=t.company||"";
  S.fJobNJ=t.job_nj||"";  // copy JOB NJ เดิม แก้ไขได้
  S.fType=_getType(t.type); // normalize ค่าเก่า → 1 ใน 7 เสมอ
  S.fDate=todayStr();
  S.fPickup=t.pickup_time||"";
  S.fDeliver=t.deliver_time||"";
  S.fPlace=t.place||"";
  S.fDetail=t.detail||"";
  S.fCloseMode=t.close_mode||"messenger_finish";
  S.fFile=null;S.fPreview=null;
  S.fInspectDate="";S.fPayDate="";
  S._isCopy=true;
  setState({page:"create",selId:null});
  showToast("📋 Copy ข้อมูลจาก JOB "+esc(t.job_number||t.id.slice(0,8))+" แล้ว","ok");
}
...
async function finishTask(id,imgUrl,customTime){
  ...
  if(!DEMO)await sbUpdate("msg_tasks",id,updates);
  S.tasks=S.tasks.map(x=>x.id===id?{...x,...updates}:x);
  invalidateDerived(); // ← สำคัญ: ล้าง cache ให้ computeDerived คำนวณใหม่
  VL._fil=null;        // ← reset VL
  invalidateCardCache(id);
  setState({showRecip:false,recipName:"",finishPreview:null,finishFile:null,finishRemark:"",finishMode:"photo",signatureData:null,selId:null,finishTaskId:null,filter:"done"}); // ← auto-switch ไปหน้าเสร็จแล้ว
...
function _showDeliveredAlert(task,force=false){
  ...
  if(!el)return;
  el.innerHTML=
    '<div style="width:100%;max-width:480px;background:#252d3e;border:2px solid #f97316;border-radius:20px 20px 0 0;padding:20px 18px 24px;box-shadow:0 -8px 40px rgba(249,115,22,.3);animation:notifSlideUp .3s cubic-bezier(.34,1.3,.64,1)">'
    ...
    +'</div>'
    +'<button onclick="document.getElementById(\'delivered-alert\').remove()" style="background:none;border:1px solid #313d52;border-radius:8px;color:#64748b;font-size:18px;padding:3px 9px;cursor:pointer;line-height:1">×</button>'
    +'</div>'
    // Task info
    +'<div style="background:#1e2538;border-radius:10px;padding:10px 14px;margin-bottom:14px">'
    ...
    +'<button onclick="_dismissDeliveredAlert(\''+task.id+'\');setState({filter:\'delivered\',page:\'list\'})" style="width:100%;margin-top:8px;padding:12px;background:transparent;border:1.5px solid #313d52;border-radius:12px;color:#94a3b8;font-size:13px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">ดูทีหลัง</button>'
    +'</div>';
...
// ── FIX (STAGE 2): Form element cache — เปิด page create/edit ครั้งเดียว, reuse ตลอดอายุของ DOM ──
const _FORM_IDS = ["nj-inp","co-search-inp","f-place","f-type","f-date","f-detail-search","f-detail","f-pickup-inp","f-deliver-inp","f-close-mode"];
const _formElCache = {}; // {id: HTMLElement | null}
function _formElGet(id){
  // เช็ค cached element ยังอยู่ใน DOM (isConnected = false → DOM ถูก replace แล้ว)
  let el = _formElCache[id];
  if(el && el.isConnected) return el;
  el = document.getElementById(id);
  _formElCache[id] = el || null;
  return el;
}
function _formElInvalidate(){
  for(const id of _FORM_IDS) _formElCache[id] = null;
}

function syncCreateForm(){
  if(S.page!=="create"&&S.page!=="edit"){
    // ออกจากหน้า form → clear cache
    _formElInvalidate();
    return;
  }
  const r = _formElGet; // alias — ใช้ cached lookup แทน getElementById ทุกครั้ง
  // NJ
...
function _renderNow(){
  // ── Skip full render ระหว่าง scroll เพื่อกัน jank ──
  // VL._raf ใน vlOnScroll จะจัดการ window update เอง (rAF-debounced)
  // scroll guard timeout (150ms) จะ set _isScrolling=false แล้ว render รอบถัดไปจะทำงานเอง
  if(_isScrolling&&S.page==="list"&&S.selId===null&&!S.loading)return;
  ...
  if(!S.currentUser){
    ...
    // ถ้า login form มีอยู่แล้ว — อัปเดตเฉพาะ error msg + tab active state
    // ไม่ rebuild ทั้งหน้า (ทำให้ iOS ล้าง keyboard + ค่าที่พิมพ์)
    const loginCard=document.getElementById("login-card-root");
    if(loginCard){
      // patch เฉพาะ error message
      const errEl=document.getElementById("l-err-msg");
      if(errEl&&errEl.textContent!==S.lErr)errEl.textContent=S.lErr;
      const rErrEl=document.getElementById("r-err-msg");
      if(rErrEl&&rErrEl.textContent!==S.rErr)rErrEl.textContent=S.rErr;
  ...
  // ── Zone: Header (fingerprint skip — เทียบแค่ค่าที่ hdr แสดง ไม่ต้อง gen HTML ถ้าไม่เปลี่ยน) ──
  const hdrFp=d.cnt.all+"|"+d.cnt.wait+"|"+d.cnt.going+"|"+d.cnt.delivered+"|"+d.cnt.done
    +"|"+d.pending+"|"+S.realtimeStatus+"|"+S.page+"|"+S.role+"|"+(S.saving?1:0);
  if(_zoneCache["_hdrFp"]!==hdrFp){
    _zoneCache["_hdrFp"]=hdrFp;
    patchZone("zone-hdr", renderHdr(d.cnt,d.pending,d.isSA,d.isAdmin));
  }
  ...
  // ── FIX 3: ออกจาก finish modal → clear timer ทันที (ไม่รอ self-cleanup รอบถัดไป) ──
  if(!(S.finishTaskId&&S.showRecip)&&window._finishClock){
    clearInterval(window._finishClock);window._finishClock=null;
  }
  ...
  _formElInvalidate();  // FIX 2: zone-main rebuilt → form elements ใน cache ถูก replace แล้ว
  // Clear VL node map: old <vl-list> will be replaced → old node refs are orphaned
  if(VL._nodes)VL._nodes.clear();
  VL._topEl=null;VL._btmEl=null;
  ...
  if(!S.loading&&d.fil.length>0){
    requestAnimationFrame(()=>{
      ...
      attachScrollListener(VL._scroller); // idempotent — no-op ถ้าแนบแล้ว
      vlUpdate(d.fil,true);
    });
  }
...
function renderSidebar(cnt,pending,isSA,isAdmin){
  ...
  return '<div style="padding:14px 18px 12px;border-bottom:1px solid #2a3347"><div style="font-size:9px;letter-spacing:2px;color:#06C755;font-weight:700;font-family:monospace">MSG SYSTEM</div><div style="font-size:14px;font-weight:800;color:#fff;margin-top:2px">ระบบสั่งงานแมส</div></div>'
    +'<div style="padding:10px 0;flex:1"><div class="sb-lbl">งาน</div>'
    +[["all","ทั้งหมด","📋",cnt.all],["wait","รอรับ","⏳",cnt.wait],["going","กำลังวิ่ง","🏃",cnt.going],["done","เสร็จแล้ว","✅",cnt.done],...(cnt.archive>0?[["archive","เก็บถาวร","🗂",cnt.archive]]:[])].map(([k,l,ic,c])=>
    ...
    +'<button onclick="clearCacheAndReload()" style="margin-top:8px;background:none;border:1px solid #f59e0b;border-radius:6px;color:#f59e0b;font-size:11px;padding:4px 10px;cursor:pointer;width:100%" title="ล้าง cache เมื่อระบบมีปัญหา/ช้า">🔄 ล้าง Cache + รีโหลด</button>'
    +'<button onclick="doLogout()" style="margin-top:6px;background:none;border:1px solid #313d52;border-radius:6px;color:#64748b;font-size:11px;padding:4px 10px;cursor:pointer;width:100%">ออกจากระบบ</button>'
    +'</div>';
}
...
// ══════════════════════════════════════════════════════
//  TASK TYPE — SINGLE SOURCE OF TRUTH (7 ตัวเท่านั้น)
// ══════════════════════════════════════════════════════
// ชื่อประเภทงานที่ระบบยอมรับ: ต้องตรง 1 ใน 7 เสมอ
const TASK_TYPES=[
  "รับ-ส่งทั่วไป",
  "รับ-ส่งดีโอ",
  "รับ-ส่งวางบิล",
  "รับ-ส่งเอกสารชิปปิ้ง",
  "ชุดงานปล่อยFZ ระยองเหมราช",
  "ชุดงานปล่อยFZ อมตะซิตี้",
  "ชุดงานปล่อยFZ อื่นๆ",
];
// Alias: map ค่าเก่าใน DB → ค่ามาตรฐาน (code-only, ไม่เขียนกลับ DB จนกว่าผู้ใช้แก้เอง)
const _TYPE_ALIAS={
  // explicit mapping ที่ผู้ใช้กำหนด
  "รับ-ส่งตู้":       "รับ-ส่งดีโอ",
  "รับ-ส่งตู้เปล่า":  "รับ-ส่งดีโอ",
  // ค่าเดิมของ v4 (ก่อน rename) → map ไป closest match
  "ส่งเอกสาร":  "รับ-ส่งทั่วไป",
  "รับเอกสาร":  "รับ-ส่งทั่วไป",
  "FZ":          "ชุดงานปล่อยFZ อื่นๆ",
  "ล่วงเวลา":   "รับ-ส่งทั่วไป",
};
// normalize: แปลงค่าใดๆ ให้เป็น 1 ใน 7 เสมอ (ใช้ที่ UI + comparison ทุกจุด)
function _getType(raw){
  if(!raw)return "รับ-ส่งทั่วไป";
  const mapped=_TYPE_ALIAS[raw];
  if(mapped)return mapped;
  // ถ้าตรง 1 ใน 7 อยู่แล้ว → ใช้เลย
  if(TASK_TYPES.indexOf(raw)>=0)return raw;
  // ค่าอื่นที่ไม่รู้จัก → ถือเป็น "ทั่วไป"
  return "รับ-ส่งทั่วไป";
}

// ── Type badge: สี/ข้อความตามประเภทงาน (7 ตัวเท่านั้น) ──
const TYPE_COLOR={
  "รับ-ส่งทั่วไป":              {bg:"rgba(59,130,246,.15)", border:"rgba(59,130,246,.4)", text:"#60a5fa"},
  "รับ-ส่งดีโอ":                {bg:"rgba(249,115,22,.15)", border:"rgba(249,115,22,.4)", text:"#fb923c"},
  "รับ-ส่งวางบิล":              {bg:"rgba(251,191,36,.15)", border:"rgba(251,191,36,.4)", text:"#fbbf24"},
  "รับ-ส่งเอกสารชิปปิ้ง":       {bg:"rgba(168,85,247,.15)", border:"rgba(168,85,247,.4)", text:"#c084fc"},
  "ชุดงานปล่อยFZ ระยองเหมราช":  {bg:"rgba(34,211,238,.15)", border:"rgba(34,211,238,.4)", text:"#22d3ee"},
  "ชุดงานปล่อยFZ อมตะซิตี้":    {bg:"rgba(244,114,182,.15)",border:"rgba(244,114,182,.4)",text:"#f472b6"},
  "ชุดงานปล่อยFZ อื่นๆ":        {bg:"rgba(45,212,191,.15)", border:"rgba(45,212,191,.4)", text:"#2dd4bf"},
};
function typeBadge(type){
  const n=_getType(type); // normalize ก่อน lookup — ไม่มีทางโชว์ค่าเก่า
  const c=TYPE_COLOR[n]||{bg:"rgba(100,116,139,.12)",border:"rgba(100,116,139,.3)",text:"#94a3b8"};
  return '<span style="display:inline-flex;align-items:center;font-size:11px;font-weight:700;padding:2px 8px;border-radius:5px;border:1px solid '+c.border+';background:'+c.bg+';color:'+c.text+';flex-shrink:0;letter-spacing:.3px">'+esc(n)+'</span>';
}
function renderCard(t){
  ...
  // ── Edit / Delete / Copy ──
  if(canCreate(u)){
    btns+='<button class="card-btn btn-copy" onclick="event.stopPropagation();copyJob(\''+t.id+'\')" title="Copy สร้างใบงานใหม่">📋</button>';
  }
  if(canEditTask(u,t)){
  ...
  return '<div id="tc-'+t.id+'" class="task-card '+TC[t.status||"wait"]+'" onclick="openDetailInstant(\''+t.id+'\')">'
    // Row 1: Job + type + NJ + status badge
    +'<div class="tc-r1">'
    +'<span class="tc-job">'+esc(t.job_number||t.id.slice(0,8))+'</span>'
    +typeBadge(t.type)
    +(t.job_nj?'<span class="tc-nj">NJ: '+esc(t.job_nj.slice(0,14))+'</span>':"")
    +'<span class="tc-badge '+BC[t.status||"wait"]+'">'+SL[t.status||"wait"]+'</span>'
    +'</div>'
    // Row 1.5: รายละเอียดงาน
    +(t.detail||t.job_nj?'<div class="tc-detail">'+(t.job_nj?'<span style="color:#f59e0b;font-family:monospace;font-size:11px;margin-right:6px">NJ:'+esc(t.job_nj.slice(0,16))+'</span>':'')+(t.detail?'📝 '+esc(t.detail.slice(0,50))+(t.detail.length>50?"…":""):'')+'</div>':"")
...
const VL = {
  ...
  OVERSCAN: 2,    // extra cards rendered above + below viewport (ลดจาก 4 → 2 = DOM เบาลง)
...
// ── Shared parser: single throwaway container, reused for every HTML→DOM parse ──
// ไม่สร้าง new div ทุกครั้ง (เดิมใน vlUpdate + vlPatch สร้างใหม่ทุก scroll tick) → GC pressure ลดลง
const _vlParseHost = document.createElement("template");
function _vlParse(html) {
  _vlParseHost.innerHTML = html;
  const node = _vlParseHost.content.firstElementChild;
  _vlParseHost.innerHTML = ""; // clear ref to release node
  return node;
}

// ── Virtual List: Pure DOM-reuse system (zero list.innerHTML) ──
// Map<taskId, DOMNode> — keep reference to every live card node in the DOM
// Create once, reuse on scroll, remove only when leaving viewport
function vlUpdate(fil, force) {
  const list = document.getElementById("vl-list");
  if (!list) return false;

  // Detect filter/data change (triggers full prune on identity swap)
  const fk = S.filter + "|" + (S.currentUser?.username || "") + "|" + fil.length;
  const filterChanged = (fk !== VL._filterKey);
  if (filterChanged) { VL._filterKey = fk; }
  ...
  // Skip if window identical and no force/filter change (hot path — bail immediately)
  if (!force && !filterChanged && start === VL._start && end === VL._end) return true;
  ...
  // Ensure top/bottom spacers exist (create once, reuse forever — pure DOM API, no innerHTML)
  if (!VL._topEl || VL._topEl.parentNode !== list) {
    const t = document.createElement("div"); t.id = "vl-top";
    t.style.flexShrink = "0"; t.setAttribute("aria-hidden","true");
    list.insertBefore(t, list.firstChild);
    VL._topEl = t;
  }
  if (!VL._btmEl || VL._btmEl.parentNode !== list) {
    const b = document.createElement("div"); b.id = "vl-btm";
    b.style.flexShrink = "0"; b.setAttribute("aria-hidden","true");
    list.appendChild(b);
    VL._btmEl = b;
  }
  VL._topEl.style.height = topH + "px";
  VL._btmEl.style.height = btmH + "px";

  // Node map: taskId → live DOM node (created once, reused)
  if (!VL._nodes) VL._nodes = new Map();

  // Compute new-visible Set (O(window size), not O(fil.length))
  const newVisible = new Set();
  for (let i = start; i < end; i++) newVisible.add(fil[i].id);

  // Diff: remove only nodes that left the viewport (single pass over Map)
  for (const [id, node] of VL._nodes) {
    if (!newVisible.has(id)) { node.remove(); VL._nodes.delete(id); }
  }

  // Insert / reuse in correct order — walk target window BACKWARDS so anchor is stable
  // insertBefore is a no-op if node is already in correct position (DOM engine short-circuit)
  let anchor = VL._btmEl;
  for (let i = end - 1; i >= start; i--) {
    const t = fil[i];
    let node = VL._nodes.get(t.id);
    if (!node) {
      // Create once — parse cached card HTML into a real DOM node via shared template
      let cardHtml = _cardCache[t.id];
      if (!cardHtml) { cardHtml = renderCard(t); _cardCache[t.id] = cardHtml; }
      node = _vlParse(cardHtml);
      if (!node) continue;
      VL._nodes.set(t.id, node);
    }
    // Reuse: move node to correct position if needed
...
// Updates content of existing DOM node in place via replaceChild (keeps Map ref synced)
function vlPatch(taskId) {
  if (!VL._fil) return false;
  // O(1) check: is this task currently in the live DOM map?
  const node = VL._nodes && VL._nodes.get(taskId);
  if (!node) return false; // not visible → caller should just update state+cache, no DOM work
  const t = getTaskById(taskId);
  if (!t) return false;
  const html = renderCard(t);
  if (_cardCache[taskId] === html) return true; // nothing changed
  _cardCache[taskId] = html;
  // Parse new HTML + swap in place (preserves surrounding DOM, scroll pos)
  const fresh = _vlParse(html);
  if (!fresh) return false;
  node.parentNode.replaceChild(fresh, node);
  VL._nodes.set(taskId, fresh);
  return true;
}

// ── scroll handler: debounced via rAF + infinite-scroll trigger ──
function vlOnScroll() {
  if (!VL._fil || S.page !== "list") return;
  cancelAnimationFrame(VL._raf);
  VL._raf = requestAnimationFrame(() => {
    vlUpdate(VL._fil, false);
    // ── Infinite scroll: trigger loadMore เมื่อใกล้ bottom ~300px (ไม่ eager เกิน) ──
    const sc = VL._scroller;
    if (!sc) return;
    const isWin = sc === window;
    const scrollTop   = isWin ? sc.scrollY      : sc.scrollTop;
    const viewportH   = isWin ? sc.innerHeight  : (sc.clientHeight || 700);
    const scrollH     = isWin ? document.documentElement.scrollHeight : sc.scrollHeight;
    const distToBtm   = scrollH - scrollTop - viewportH;
    // ≤ 300px ถึง bottom + render ถึง end ของ fil → trigger
    if (distToBtm < 300 && VL._end >= VL._fil.length - 3) {
      if (S.filter === "archive") loadMoreArchive();
      else if (!_noMoreBase)      loadMoreTasks();
    }
  });
}


// ── buildTypeFilterBar: แถวกรองประเภทงาน (7 ตัวเท่านั้น) ──
function buildTypeFilterBar(tasks,wait,going,delivered,archived,recentDone){
  ...
  // ── ประเภท 7 ตัว (single source of truth) — แสดงครบเสมอ ไม่ซ่อน ──
  const TYPE_DEFS=[
    {k:"รับ-ส่งทั่วไป",             ic:"📦",col:"#60a5fa"},
    {k:"รับ-ส่งดีโอ",               ic:"📋",col:"#fb923c"},
    {k:"รับ-ส่งวางบิล",             ic:"💰",col:"#fbbf24"},
    {k:"รับ-ส่งเอกสารชิปปิ้ง",      ic:"🚢",col:"#c084fc"},
    {k:"ชุดงานปล่อยFZ ระยองเหมราช", ic:"🏭",col:"#22d3ee"},
    {k:"ชุดงานปล่อยFZ อมตะซิตี้",   ic:"🏭",col:"#f472b6"},
    {k:"ชุดงานปล่อยFZ อื่นๆ",       ic:"🚛",col:"#2dd4bf"},
  ];
  ...
  // นับโดยใช้ _getType เพื่อให้งานที่มี type เก่าใน DB นับรวมเข้ากลุ่มใหม่ด้วย
  TYPE_DEFS.forEach(({k,ic,col})=>{
    const n=_base.filter(t=>_getType(t.type)===k).length;
...
function renderList(d){
  ...
  const stats="";
  const filters='<div class="filter-section">'
  ...
  const menuItems=filTasks.slice(0,12).map(t=>
    '<div onclick="exportPDF(\'single\',\''+t.id+'\');closeMenus()" style="padding:8px 14px;font-size:12px;color:#94a3b8;cursor:pointer;border-top:1px solid #2a3347;display:flex;align-items:center;gap:6px" onmouseenter="this.style.background=\'rgba(239,68,68,.06)\'" onmouseleave="this.style.background=\'\'">'
    +'<span style="font-family:monospace;font-size:10px;color:#f87171">'+esc(t.job_number||t.id.slice(0,8))+'</span>'
    +'<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(t.company||_getType(t.type))+'</span></div>'
  ).join("")+(filTasks.length>12?'<div style="padding:8px 14px;font-size:11px;color:#475569;text-align:center;border-top:1px solid #2a3347">...และอีก '+(filTasks.length-12)+' งาน</div>':"");

  // Export filter bar
  const expFilterPanel=S.showExportFilter
    ?'<div style="margin:0 16px 10px;background:#252d3e;border:1px solid #313d52;border-radius:12px;padding:14px">'
      ...
      +'<input type="date" id="exp-date-from" value="'+esc(S.expDateFrom)+'" onchange="S.expDateFrom=this.value" style="width:100%;background:#1a1f2e;border:1.5px solid #313d52;border-radius:8px;padding:8px;color:#e2e8f4;font-size:12px;outline:none;color-scheme:dark"></div>'
      +'<div><div style="font-size:10px;color:#64748b;font-weight:700;margin-bottom:4px">📅 วันที่สิ้นสุด</div>'
      +'<input type="date" id="exp-date-to" value="'+esc(S.expDateTo)+'" onchange="S.expDateTo=this.value" style="width:100%;background:#1a1f2e;border:1.5px solid #313d52;border-radius:8px;padding:8px;color:#e2e8f4;font-size:12px;outline:none;color-scheme:dark"></div>'
      +'</div>'
      ...
      +'<input type="text" id="exp-company" value="'+esc(S.expCompany)+'" placeholder="เช่น AAR, 4Care..." onchange="S.expCompany=this.value" style="width:100%;background:#1a1f2e;border:1.5px solid #313d52;border-radius:8px;padding:8px;color:#e2e8f4;font-size:12px;outline:none" oninput="S.expCompany=this.value"></div>'
      +'<div style="display:flex;align-items:center;gap:6px">'
      +'<span id="exp-count" style="font-size:11px;color:#06C755;font-weight:700">'+expData.length+' รายการ</span>'
      +'<button onclick="S.expDateFrom=\'\';S.expDateTo=\'\';S.expCompany=\'\';setState({showExportFilter:true})" style="background:none;border:1px solid #313d52;border-radius:6px;padding:3px 8px;font-size:11px;color:#64748b;cursor:pointer">ล้าง</button>'
      +'</div>'
      +'</div>'
    :"";
  ...
  const exp=S.currentUser?.role==="messenger"?"":('<div class="export-bar">'
    ...
    +'<div id="pdf-menu" style="display:none;position:absolute;top:100%;left:0;margin-top:4px;background:#252d3e;border:1.5px solid rgba(239,68,68,.4);border-radius:10px;min-width:190px;z-index:200;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.4)">'
    +'<div onclick="exportPDF(\'all\');closeMenus()" style="padding:10px 14px;font-size:13px;color:#e2e8f4;cursor:pointer;border-bottom:1px solid #313d52;display:flex;align-items:center;gap:8px;font-weight:700" onmouseenter="this.style.background=\'rgba(239,68,68,.08)\'" onmouseleave="this.style.background=\'\'">📄 ทุกงาน ('+expData.length+' ใบ)</div>'
...
function renderDetail(){
  ...
  const tl='<div class="tl">'+tlItems.map(item=>'<div class="tl-row">'
    +'<div class="tl-dot" style="background:'+(item.done?item.color:"#313d52")+'"></div>'
  ...
  const img=t.image_url?'<img class="detail-img" loading="lazy" src="'+esc(t.image_url)+'">'
    :(t.finish_remark?'<div style="background:#252d3e;border:1px solid #313d52;border-radius:8px;padding:10px 12px;font-size:12px;color:#94a3b8">📝 '+esc(t.finish_remark)+'</div>'
    :'<div style="height:50px;border:1px dashed #313d52;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#64748b;font-size:12px">ไม่มีภาพแนบ</div>');
  ...
  return '<div class="modal-overlay" onclick="if(event.target===this)setState({selId:null})">'
    ...
    +'<button type="button" data-close="detail" onclick="event.stopPropagation();__closeDetail()" style="background:none;border:1px solid #313d52;border-radius:8px;color:#94a3b8;font-size:22px;padding:4px 12px;cursor:pointer;line-height:1;font-weight:700">×</button>'
    +'</div></div>'
    ...
    +'</div>'
    // Info
    +'<div style="background:#1e2538;border-radius:10px;padding:12px 14px;margin-bottom:12px;font-size:13px">'
...
function renderFinishModal(){
  ...
  return '<div class="modal-overlay" onclick="closeFinishModal()">'
    ...
    +'<button type="button" onclick="event.stopPropagation();closeFinishModal()" style="background:none;border:1px solid #313d52;border-radius:8px;color:#64748b;font-size:18px;padding:2px 8px;cursor:pointer;line-height:1">×</button>'
    +'</div>'
    ...
    +'<textarea id="finish-remark" placeholder="เช่น ลูกค้าไม่อยู่..." oninput="S.finishRemark=this.value" style="width:100%;background:#252d3e;border:1.5px solid #313d52;border-radius:8px;padding:10px 12px;color:#e2e8f4;font-size:13px;font-family:Sarabun,sans-serif;resize:none;min-height:56px;outline:none">'+esc(S.finishRemark||'')+'</textarea></div>'
    +'<div style="display:flex;background:#0a0f1e;border-radius:10px;padding:3px;margin-bottom:12px;gap:3px">'
    +'<button type="button" onclick="setState({finishMode:\'photo\',signatureData:null})" style="flex:1;padding:8px;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;background:'+(S.finishMode==="photo"?"#06C755":"transparent")+';color:'+(S.finishMode==="photo"?"#fff":"#64748b")+'">📷 ถ่ายรูป</button>'
    +'<button type="button" onclick="setState({finishMode:\'sign\',finishPreview:null,finishFile:null})" style="flex:1;padding:8px;border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;background:'+(S.finishMode==="sign"?"#3b82f6":"transparent")+';color:'+(S.finishMode==="sign"?"#fff":"#64748b")+'">✍️ เซ็นชื่อ</button>'
    +'</div>'
    +(S.finishMode==="photo"
      ?'<div style="font-size:11px;color:#ef4444;font-weight:700;margin-bottom:8px">📸 แนบภาพหลักฐาน *บังคับ</div>'
        +(S.finishPreview
          ?'<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px"><img src="'+S.finishPreview+'" style="width:70px;height:70px;border-radius:8px;object-fit:cover;border:2px solid #06C755"><span style="font-size:12px;color:#06C755;font-weight:700">✅ แนบแล้ว</span><button type="button" onclick="event.stopPropagation();_revokeFinishPreview();setState({finishPreview:null,finishFile:null})" style="background:none;border:none;color:#64748b;font-size:18px;cursor:pointer;margin-left:auto">×</button></div>'
          :'<div style="display:flex;gap:8px;margin-bottom:10px">'
            // ปุ่มถ่ายรูป (camera)
            +'<label style="flex:1;display:block;border:2px dashed #ef4444;border-radius:10px;padding:14px 8px;text-align:center;color:#ef4444;font-size:13px;font-weight:700;cursor:pointer">'
              +'<input type="file" accept="image/*" capture="environment" style="display:none" onchange="handleFinishFile(this)">'
              +'📷 ถ่ายรูป</label>'
            // ปุ่มเลือกจาก gallery (ไม่มี capture)
            +'<label style="flex:1;display:block;border:2px dashed #3b82f6;border-radius:10px;padding:14px 8px;text-align:center;color:#3b82f6;font-size:13px;font-weight:700;cursor:pointer">'
              +'<input type="file" accept="image/*" style="display:none" onchange="handleFinishFile(this)">'
              +'🖼️ เลือกรูป</label>'
          +'</div>')
      :'<div style="font-size:11px;color:#3b82f6;font-weight:700;margin-bottom:8px">✍️ เซ็นชื่อ *บังคับ</div>'
        +'<div style="position:relative;border:2px solid #3b82f6;border-radius:10px;overflow:hidden;margin-bottom:8px;background:#fff"><canvas id="sig-canvas" style="width:100%;height:150px;display:block;touch-action:none;cursor:crosshair"></canvas>'
        +(S.signatureData?'<div style="position:absolute;top:6px;right:6px"><button type="button" onclick="event.stopPropagation();clearSignature()" style="background:rgba(239,68,68,.8);border:none;border-radius:6px;color:#fff;font-size:11px;padding:3px 8px;cursor:pointer">ลบ</button></div>':"")
    ...
    +'<button type="button" class="btn-green" '+(S.uploadingFinish?'disabled':'')+' onclick="confirmFinish()" style="font-size:15px;'+((S.finishMode==="photo"&&!S.finishPreview)||(S.finishMode==="sign"&&!S.signatureData)||S.uploadingFinish?"opacity:.45":"")+';">'
    +(S.uploadingFinish?"⏳ กำลังบันทึก...":(S.finishMode==="photo"?(S.finishPreview?"✅ ยืนยันเสร็จงาน":"📷 แนบภาพก่อนยืนยัน"):(S.signatureData?"✅ ยืนยันเสร็จงาน":"✍️ เซ็นชื่อก่อนยืนยัน")))
    +'</button>'
    +'</div></div>';
...
function renderCreate(){
  ...
  const TYPES=TASK_TYPES; // 7 ตัวเท่านั้น (shared source of truth)
  const now=new Date();
  ...
  return '<div class="create-wrap">'
    +'<div style="display:flex;align-items:center;gap:10px;padding:16px 0 14px;border-bottom:1px solid #2a3347;margin-bottom:18px">'
    ...
    +'<div class="job-card">'
    +'<div style="flex:1">'
    +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:11px;color:#64748b;font-weight:700;letter-spacing:1px">เลขที่ใบงาน (JOB)</span><span style="font-size:10px;color:#475569;background:#1a1f2e;padding:1px 7px;border-radius:10px">อัตโนมัติ</span></div>'
    ...
    +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><span style="font-size:11px;color:#64748b;font-weight:700;letter-spacing:1px">วันที่ / เวลา</span><span style="font-size:10px;color:#475569;background:#1a1f2e;padding:1px 7px;border-radius:10px">อัตโนมัติ</span></div>'
    ...
    +'<div id="co-tags" style="display:flex;flex-wrap:wrap;gap:6px;background:#1a1f2e;border:1.5px solid #313d52;border-radius:10px 10px 0 0;padding:8px 10px;min-height:44px;cursor:text" onclick="document.getElementById(\'co-search-inp\').focus()">'
    ...
    +'<div id="co-dropdown" style="display:none;position:absolute;left:0;right:0;background:#252d3e;border:1.5px solid #c084fc;border-top:none;border-radius:0 0 10px 10px;max-height:240px;overflow-y:auto;z-index:200"></div>'
    +'</div>'
    +'<div class="frow">'
    ...
    +'<button type="button" onclick="openClock(\'pickup\')" style="background:#252d3e;border:1.5px solid #313d52;border-radius:10px;padding:10px 12px;color:#64748b;font-size:16px;cursor:pointer;flex-shrink:0;transition:all .2s" onmouseenter="this.style.borderColor=\'#06C755\'" onmouseleave="this.style.borderColor=\'#313d52\'">🕐</button>'
    +'</div></div>'
    ...
    +'<button type="button" onclick="openClock(\'deliver\')" style="background:#252d3e;border:1.5px solid #313d52;border-radius:10px;padding:10px 12px;color:#64748b;font-size:16px;cursor:pointer;flex-shrink:0;transition:all .2s" onmouseenter="this.style.borderColor=\'#06C755\'" onmouseleave="this.style.borderColor=\'#313d52\'">🕑</button>'
    +'</div></div>'
    +'</div>'
    ...
    +'<div id="place-list" style="display:none;position:absolute;top:100%;left:0;right:0;background:#252d3e;border:1.5px solid #06C755;border-top:none;border-radius:0 0 10px 10px;max-height:220px;overflow-y:auto;z-index:200"></div>'
    +'</div></div>'
    ...
    +'<div id="detail-list" style="display:none;position:absolute;top:100%;left:0;right:0;background:#252d3e;border:1.5px solid #3b82f6;border-radius:0 0 10px 10px;max-height:200px;overflow-y:auto;z-index:200;border-top:none"></div>'
    +'</div>'
...
function renderEdit(){
  ...
  // ใช้ TASK_TYPES (7 ตัวเท่านั้น) — S.fType ถูก normalize ใน openEditTask แล้ว
  const TYPES=TASK_TYPES;
   return '<div class="create-wrap edit-wrap">'
    +'<div style="display:flex;align-items:center;gap:10px;padding:16px 0 14px;border-bottom:1px solid #2a3347;margin-bottom:18px">'
    ...
    +'<div class="desktop-only-bar" style="margin-top:16px;display:flex;gap:10px;grid-column:1/3">'    +'<button class="btn-green" onclick="saveEditTask()"'+(S.saving?' disabled':'')+' style="flex:1;max-width:360px">'+( S.saving?"⏳ กำลังบันทึก...":"✅ บันทึกการแก้ไข")+'</button>'    +'<button class="btn-outline" onclick="setState({page:\'list\',editTaskId:null})" style="max-width:140px">ยกเลิก</button>'    +'</div>';
}
...
function renderAdmin(){
  ...
  const card=(u)=>'<div class="admin-card" style="flex-wrap:wrap;gap:10px">'
    ...
    +(u.password_hash
      ...
      :'<span id="pw-'+u.id+'" style="font-family:monospace;font-size:12px;color:#64748b;letter-spacing:1px">••••••••</span>'
       +'<button onclick="togglePw(\''+u.id+'\',\''+esc(u.password)+'\')" style="background:none;border:1px solid #313d52;border-radius:4px;color:#475569;font-size:10px;padding:1px 6px;cursor:pointer">แสดง</button>'
    )
    +'</div>'
    +'</div>'
    ...
    +(u.status==="pending"
      ...
      :u.status==="approved"
        ?'<select onchange="changeRole(\''+u.id+'\',this.value)" style="background:#1a1f2e;border:1px solid #313d52;border-radius:6px;color:#e2e8f4;font-size:11px;padding:4px 8px;cursor:pointer">'
         +["staff","messenger","admin"].map(r=>'<option value="'+r+'"'+(u.role===r?' selected':'')+'>'+ROLE_LABEL[r]+'</option>').join("")
         +'</select>'
         // ปุ่ม toggle can_finish — เฉพาะ staff
         +(u.role==="staff"
           ?'<button onclick="toggleCanFinish(\''+u.id+'\','+(!u.can_finish)+')" style="background:'+(u.can_finish?"rgba(6,199,85,.12)":"rgba(100,116,139,.1)")+';border:1px solid '+(u.can_finish?"rgba(6,199,85,.4)":"#313d52")+';color:'+(u.can_finish?"#06C755":"#64748b")+';border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;font-weight:700">'+(u.can_finish?"✅ ปิดงานได้":"➕ ให้ปิดงาน")+'</button>'
           :"")
  ...
  const guide='<div style="background:#1e2538;border:1px solid #2a3347;border-radius:12px;padding:14px 18px;margin-bottom:16px">'
...
function renderLogin(){
  ...
  return '<div class="login-page">'
    ...
    +'<div class="login-card" id="login-card-root">'
    +'<div class="tab-row">'
    +'<button class="tab-btn'+(isL?" active":"")+'" onclick="document.getElementById(\'login-card-root\')?.remove();setState({loginTab:\'login\',lErr:\'\',rErr:\'\',rOk:false})">🔒 เข้าสู่ระบบ</button>'
    +'<button class="tab-btn'+(!isL?" active":"")+'" onclick="document.getElementById(\'login-card-root\')?.remove();setState({loginTab:\'register\',lErr:\'\',rErr:\'\',rOk:false})">📝 สมัครใช้งาน</button>'
    +'</div>'
    +(isL
      ?'<div class="fg"><label class="inp-lbl">Username</label><input class="inp" id="l-user" placeholder="กรอก Username" autocomplete="off" value="" onkeydown="if(event.key===\'Enter\')document.getElementById(\'l-pass\').focus()"></div>'
       ...
       +'</div></div>'
       +'<div class="err-msg" id="l-err-msg">'+S.lErr+'</div>'
       +'<button class="btn-green" onclick="doLogin()" type="button">เข้าสู่ระบบ →</button>'
      :S.rOk
        ?'<div class="pending-box">'+'<div style="font-size:40px;margin-bottom:10px">✅</div>'+'<div style="font-size:16px;font-weight:800;color:#06C755;margin-bottom:6px">สมัครสำเร็จแล้ว!</div>'+'<div style="font-size:13px;color:#e2e8f4;margin-bottom:4px">บัญชีของคุณอยู่ระหว่างรอการอนุมัติ</div>'+'<div style="font-size:12px;color:#64748b">⏳ Admin จะอนุมัติบัญชีของคุณเร็วๆ นี้</div>'+'<div style="font-size:11px;color:#475569;margin-top:8px">ระบบจะกลับหน้าล็อกอินใน 5 วินาที...</div>'+'</div>'
        ...
        :'<div class="fg"><label class="inp-lbl">Username</label><input class="inp" id="r-user" placeholder="ตั้ง Username" autocomplete="off"></div>'
         ...
         +'<div class="err-msg" id="r-err-msg">'+S.rErr+'</div>'
...
async function doLogin(){
  ...
  if(!found){setState({lErr:"❌ Username หรือ Password ไม่ถูกต้อง — กรุณาลองใหม่"});const p=document.getElementById("l-pass");if(p){p.style.animation="shake .4s";setTimeout(()=>p.style.animation="",400);}return}
  ...
  sessionStorage.setItem("msg_cu",JSON.stringify(found));
  S.currentUser=found;
...
async function doRegister(){
  ...
  showToast("✅ สมัครเรียบร้อย! รอ Admin อนุมัติก่อนเข้าระบบ","ok");
  // auto กลับหน้า login หลัง 5 วิ
  setTimeout(()=>{document.getElementById("login-card-root")?.remove();setState({loginTab:"login",rOk:false,lErr:"",rErr:""});},5000);
}
function doLogout(){stopRealtime();sessionStorage.removeItem("msg_cu");setState({currentUser:null,page:"login",tasks:[],users:[],lErr:""})}

// ── Clear Cache & Reload — ปุ่ม UI ให้ user แก้ปัญหา cache ค้างเอง ──
async function clearCacheAndReload(){
  if(!confirm("⚠️ ล้าง cache + รีโหลดระบบ?\n\nใช้เมื่อ:\n• ระบบช้า / ค้าง\n• ข้อมูลไม่อัพเดท\n• เห็นเวอร์ชันเก่า\n\nกด OK เพื่อล้าง"))return;
  try{
    showToast("🔄 กำลังล้าง cache...","ok");
    // 1) แจ้ง SW ลบ cache ทั้งหมด (ถ้ามี SW)
    if(navigator.serviceWorker?.controller){
      navigator.serviceWorker.controller.postMessage({type:'CLEAR_CACHES'});
    }
    // 2) ลบ Cache API ฝั่ง browser ตรง (ใน case SW ไม่ตอบ)
    if('caches' in window){
      try{
    ...
    // 3) Unregister SW เพื่อให้ load ใหม่ทั้งหมด
    ...
    // 4) Reload (skip browser cache — ?_v=timestamp)
    setTimeout(()=>{location.replace(location.pathname+"?_v="+Date.now());},400);
  }catch(e){
    console.warn("clearCacheAndReload error",e);
    location.reload();
  }
}
...
async function toggleCanFinish(id,val){
  ...
  if(S.currentUser?.id===id){
    ...
    sessionStorage.setItem("msg_cu",JSON.stringify(S.currentUser));
  }
  S.users=await getUsers();
  render();
...
function compressImage(file,maxPx=1200,quality=0.7){
  // Return: File (image/jpeg) — ไม่คืน preview แล้ว (handleFinishFile ใช้ createObjectURL เอง)
  return new Promise((resolve,reject)=>{
    const doCompress=(bitmap)=>{
      try{
        let w=bitmap.width,h=bitmap.height;
        if(!w||!h){bitmap.close?.();return reject(new Error("invalid_image_dimensions"));}
        if(w>maxPx||h>maxPx){
        ...
        if(!ctx){bitmap.close?.();return reject(new Error("canvas_ctx_failed"));}
        ctx.drawImage(bitmap,0,0,w,h);
        canvas.toBlob(blob=>{
          bitmap.close?.();
          if(!blob){return reject(new Error("canvas_toBlob_failed"));}
          if(!blob.size){return reject(new Error("canvas_toBlob_empty"));}
          // ── Always return File with safe name + image/jpeg type ──
          const finalFile=new File([blob],"finish.jpg",{type:"image/jpeg",lastModified:Date.now()});
          if(!finalFile.size){return reject(new Error("file_size_zero"));}
          resolve(finalFile);
        },"image/jpeg",quality);
      }catch(e){bitmap.close?.();reject(e);}
    };
    // Path 1: createImageBitmap (faster, doesn't block main thread)
    if(typeof createImageBitmap!=="undefined"){
      createImageBitmap(file).then(doCompress).catch(()=>{
        // Path 2 (fallback): FileReader + Image element
        _compressViaImage(file,doCompress,reject);
      });
    }else{
      _compressViaImage(file,doCompress,reject);
    }
...
// Helper: fallback compression via <img> element (สำหรับเครื่องที่ไม่ support createImageBitmap)
function _compressViaImage(file,onReady,onFail){
  const r=new FileReader();
  r.onerror=()=>onFail(new Error("filereader_failed"));
  r.onload=e=>{
    const img=new Image();
    img.onload=()=>onReady(img);
    img.onerror=()=>onFail(new Error("image_decode_failed"));
    img.src=e.target.result;
  };
  r.readAsDataURL(file);
}

// ── Adaptive compression: บีบซ้ำจนกว่าจะ ≤ 200KB (กัน DB/upload bandwidth) ──
async function _compressAdaptive(file){
  const TARGET=200*1024; // 200KB
  // Returns File (no preview — caller uses URL.createObjectURL)
  let f=await compressImage(file,900,0.65);
  if(f.size<=TARGET)return f;
  f=await compressImage(file,900,0.55);
  if(f.size<=TARGET)return f;
  f=await compressImage(file,800,0.5);
  if(f.size<=TARGET)return f;
  f=await compressImage(file,720,0.45);
  if(f.size<=TARGET)return f;
  f=await compressImage(file,640,0.4);
  if(f.size<=TARGET)return f;
  f=await compressImage(file,560,0.35);
  if(f.size<=TARGET)return f;
  return await compressImage(file,480,0.3);
}

// Helper: ปลอดภัย revoke blob URL ใน state (กัน memory leak)
function _revokeFinishPreview(){
  try{
    if(S.finishPreview&&typeof S.finishPreview==="string"&&S.finishPreview.indexOf("blob:")===0){
      URL.revokeObjectURL(S.finishPreview);
    }
  }catch{}
}
window._revokeFinishPreview=_revokeFinishPreview;

function handleFinishFile(inp){
  const f=inp&&inp.files&&inp.files[0];
  // clear input ทันที — เลือกไฟล์เดิมซ้ำได้
  const _resetInp=()=>{try{inp.value="";}catch{}};
  if(!f){_resetInp();return;}
  // เช็ค type ต้องเป็น image/*
  if(!f.type||!/^image\//i.test(f.type)){
    showToast("⚠️ กรุณาเลือกไฟล์รูปภาพ","warn");_resetInp();return;
  }
  // ── รับรูปใหญ่สุด 25MB ──
  if(f.size>25*1024*1024){
    showToast("⚠️ รูปใหญ่เกิน 25MB — กรุณาใช้รูปอื่น","warn");_resetInp();return;
  }
  showToast("⏳ กำลังประมวลผลรูป...","warn");
  _compressAdaptive(f).then(file=>{
    // ── ตรวจสอบ output ครบถ้วน ──
    if(!(file instanceof Blob)){
      showToast("❌ ไฟล์รูปไม่สมบูรณ์","warn");_resetInp();return;
    }
    if(!file.size){
      showToast("❌ ไฟล์รูปเสีย","warn");_resetInp();return;
    }
    // ── Revoke preview เก่า + ตั้ง preview ใหม่ผ่าน createObjectURL (ไม่ใช้ DataURL) ──
    _revokeFinishPreview();
    const previewUrl=URL.createObjectURL(file);
    setState({finishFile:file,finishPreview:previewUrl});
    showToast("✅ แนบรูปเรียบร้อย ("+Math.round(file.size/1024)+"KB)","ok");
    _resetInp();
  }).catch(err=>{
    console.warn("compress error",err);
    // Fallback: ถ้าไฟล์เดิมไม่ใหญ่มาก (<= 5MB) → ใช้ original
    if(f.size<=5*1024*1024){
      _revokeFinishPreview();
      const previewUrl=URL.createObjectURL(f);
      setState({finishFile:f,finishPreview:previewUrl});
      showToast("⚠️ ใช้รูปต้นฉบับ ("+Math.round(f.size/1024)+"KB)","warn");
      _resetInp();
    }else{
      showToast("❌ ประมวลผลรูปไม่ได้ — กรุณาเลือกรูปขนาดเล็กกว่า 5MB","warn");
      _resetInp();
    }
  });
}
...
function confirmFinish(){
  ...
  const t=nowTime();
  if(S.finishMode==="photo"){
    if(S.finishFile&&!DEMO){
      uploadFinishImage(taskId,S.finishFile,t);
    }else if(DEMO){
      // DEMO: ใช้ preview ที่มี (DataURL ในเครื่อง — ไม่กระทบ DB จริง)
      finishTask(taskId,S.finishPreview||null,t);
    }else{
      // No file but have preview (rare) → ปฏิเสธ
      showToast("⚠️ ไม่พบไฟล์รูป — กรุณาแนบใหม่","warn");
    }
  }else{
    // Signature mode → upload PNG ไป Storage แทน base64 ลง DB
    if(DEMO){
      finishTask(taskId,S.signatureData,t);
    }else{
      uploadSignatureImage(taskId,S.signatureData,t);
    }
  }
}

// ── Close Finish Modal — central cleanup (clear timer + reset state + revoke blob) ──
function closeFinishModal(){
  try{if(window._finishClock){clearInterval(window._finishClock);window._finishClock=null;}}catch{}
  // Revoke blob URL ก่อน reset (ป้องกัน memory leak)
  try{_revokeFinishPreview();}catch{}
  setState({
    finishTaskId:null,
    showRecip:false,
    finishPreview:null,
...
function initSignatureCanvas(){
  const canvas=document.getElementById("sig-canvas");
  if(!canvas)return;
  // กัน double-bind (ถ้า re-init ซ้ำ event listener จะซ้อน)
  if(canvas.dataset.sigReady==="1")return;
  canvas.dataset.sigReady="1";

  // ── Resize canvas ตาม devicePixelRatio (เส้นคมขึ้น ไม่เบลอ) ──
  ...
  function getPos(e){
    ...
    return{x:e.clientX-r.left,y:e.clientY-r.top}; // ใช้ CSS coordinates (transform จัดการ DPR แล้ว)
  }
  function onDown(e){
    e.preventDefault();
    drawing=true;
  ...
  function onUp(e){
    ...
    // บันทึก signature เป็น dataURL (ห้ามเรียก render — canvas จะ reset)
    try{S.signatureData=canvas.toDataURL("image/png");}catch(err){console.warn("toDataURL fail",err);}
  }

  // ── Pointer Events: ครอบคลุม touch + mouse + pen ในตัวเดียว ──
  canvas.addEventListener("pointerdown",onDown);
  canvas.addEventListener("pointermove",onMove);
  canvas.addEventListener("pointerup",onUp);
  canvas.addEventListener("pointercancel",onUp);
  // กัน drag image / context menu บนมือถือ
...
function clearSignature(){
  const canvas=document.getElementById("sig-canvas");
  if(!canvas)return;
  // clear ใน CSS coordinates — ใช้ rect.width/height เพราะ ctx.setTransform(dpr,...)
  const r=canvas.getBoundingClientRect();
  if(_sigCtx){_sigCtx.clearRect(0,0,r.width||canvas.width,r.height||canvas.height);}
  S.signatureData=null;
  // ลบ flag เพื่อ re-init ใหม่ได้
  delete canvas.dataset.sigReady;
  // re-init ครั้งใหม่ — ห้ามปิด modal
  setTimeout(()=>initSignatureCanvas(),100);
}

async function uploadFinishImage(id,file,customTime){
  setState({uploadingFinish:true});
  showToast("⏳ กำลังอัปโหลดรูป...","warn");
  // ── Safe path: ห้ามใช้ original filename, ห้ามมีภาษาไทย/space/: — lowercase + alnum ──
  const safeName=Date.now()+"_"+Math.random().toString(36).slice(2)+".jpg";
  const path="finish/"+id+"/"+safeName;
  const url=SUPA_URL+"/storage/v1/object/msg-images/"+path;
  const publicUrl=SUPA_URL+"/storage/v1/object/public/msg-images/"+path;
  // ── Debug log (ดูใน console เมื่อมีปัญหา) ──
  try{console.log("UPLOAD_DEBUG",{
    bucket:"msg-images",
    path,
    type:file&&file.type,
    size:file&&file.size,
  ...
  // ── Retry rule: retry เฉพาะ network error / timeout / 5xx ──
  // ── ห้าม retry: 400, 401, 403 ──
  try{
    let res;
    try{res=await _doUpload();}
    catch(err){
      console.warn("upload attempt 1 failed (network)",err);
      showToast("⏳ ลองใหม่...","warn");
      res=await _doUpload(); // retry network error
    }
    if(res.ok){
      finishTask(id,publicUrl,customTime); // finishTask reset state
      return;
    }
    // 5xx → retry 1 ครั้ง
    if(res.status>=500&&res.status<600){
      console.warn("upload server err",res.status,"retrying");
      showToast("⏳ Server busy, ลองใหม่...","warn");
      const res2=await _doUpload();
      if(res2.ok){finishTask(id,publicUrl,customTime);return;}
      const t2=await res2.text().catch(()=>res2.statusText);
      console.error("SUPABASE_UPLOAD_ERROR",res2.status,t2);
      showToast("❌ Server error ("+res2.status+") - ลองใหม่อีกครั้ง","warn");
    }else if(res.status===400){
      // 400 → ห้าม retry (path/format ผิด)
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR",400,errText);
      showToast("❌ Storage path/file format ไม่ถูกต้อง (400)","warn");
    }else if(res.status===401||res.status===403){
      // 401/403 → ห้าม retry (policy)
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR",res.status,errText);
      showToast("❌ Supabase Storage policy ไม่อนุญาต ("+res.status+")","warn");
    }else if(res.status===413){
      showToast("❌ รูปใหญ่เกินไป (413) - ลองรูปอื่น","warn");
    }else{
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR",res.status,errText);
      showToast("❌ อัปโหลดไม่สำเร็จ ("+res.status+")","warn");
    }
  }catch(err){
    console.error("SUPABASE_UPLOAD_ERROR",err);
    if(err.name==="AbortError"){
      showToast("❌ อัปโหลดหมดเวลา 30 วิ - ตรวจสอบเน็ต","warn");
    }else{
      showToast("❌ อัปโหลดไม่สำเร็จ — "+(err.message||"network error"),"warn");
    }
  }finally{
    setState({uploadingFinish:false});
  }
}

// ── Upload signature เป็น PNG file → Supabase Storage (เหมือนรูป) ──
async function uploadSignatureImage(id,dataURL,customTime){
  setState({uploadingFinish:true});
  showToast("⏳ กำลังบันทึกลายเซ็น...","warn");
  const safeName=Date.now()+"_"+Math.random().toString(36).slice(2)+".png";
  const path="sign/"+id+"/"+safeName;
  const url=SUPA_URL+"/storage/v1/object/msg-images/"+path;
  const publicUrl=SUPA_URL+"/storage/v1/object/public/msg-images/"+path;
  let blob,mime;
  try{
    const arr=dataURL.split(",");
    mime=(arr[0].match(/:(.*?);/)||[null,"image/png"])[1]||"image/png";
    const bin=atob(arr[1]);
    const u8=new Uint8Array(bin.length);
    for(let i=0;i<bin.length;i++)u8[i]=bin.charCodeAt(i);
    blob=new Blob([u8],{type:mime});
    if(!blob.size)throw new Error("blob_size_zero");
  }catch(e){
    console.error("SUPABASE_UPLOAD_ERROR signature parse",e);
    showToast("❌ ลายเซ็นไม่ถูกต้อง — กรุณาเซ็นใหม่","warn");
    setState({uploadingFinish:false});return;
  }
  try{console.log("UPLOAD_DEBUG",{
    bucket:"msg-images",path,type:mime,size:blob.size,isBlob:blob instanceof Blob
  });}catch{}
  const _doUpload=async()=>{
    const ctrl=new AbortController();
  ...
  try{
    ...
    try{res=await _doUpload();}
      ...
      console.warn("sig upload attempt 1 failed (network)",e);
      showToast("⏳ ลองใหม่...","warn");
      res=await _doUpload();
    }
    if(res.ok){finishTask(id,publicUrl,customTime);return;}
    if(res.status>=500&&res.status<600){
      const r2=await _doUpload();
      if(r2.ok){finishTask(id,publicUrl,customTime);return;}
      const t2=await r2.text().catch(()=>r2.statusText);
      console.error("SUPABASE_UPLOAD_ERROR signature",r2.status,t2);
      showToast("❌ Server error ("+r2.status+") - ลองใหม่","warn");
    }else if(res.status===400){
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR signature",400,errText);
      showToast("❌ Storage path/file format ไม่ถูกต้อง (400)","warn");
    }else if(res.status===401||res.status===403){
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR signature",res.status,errText);
      showToast("❌ Supabase Storage policy ไม่อนุญาต ("+res.status+")","warn");
    }else if(res.status===413){
      showToast("❌ ลายเซ็นใหญ่เกินไป (413) - ลองใหม่","warn");
    }else{
      const errText=await res.text().catch(()=>res.statusText);
      console.error("SUPABASE_UPLOAD_ERROR signature",res.status,errText);
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ ("+res.status+")","warn");
    }
  }catch(err){
    console.error("SUPABASE_UPLOAD_ERROR signature",err);
    if(err.name==="AbortError"){
      showToast("❌ อัปโหลดหมดเวลา 30 วิ - ตรวจสอบเน็ต","warn");
    }else{
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ — "+(err.message||"network error"),"warn");
    }
  }finally{
    setState({uploadingFinish:false});
  }
...
function njTagsHtml(){
  ...
  return '<div id="nj-tags" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;background:#1a1f2e;border:1.5px solid #313d52;border-radius:10px;padding:8px 10px;min-height:44px;cursor:text" onclick="document.getElementById(\'nj-inp\').focus()">'
    +tags
...
function handleFile(inp){
  ...
  // ── รับรูปใหญ่สุด 25MB (กล้องมือถือใหม่ๆ ถ่ายได้ 8-15MB ปกติ) ──
  if(f.size>25*1024*1024){
    showToast("⚠️ รูปใหญ่เกิน 25MB — กรุณาใช้รูปอื่น","warn");
    inp.value="";return;
  }
  showToast("⏳ กำลังประมวลผลรูป...","warn");
  _compressAdaptive(f).then(({file,preview})=>{
    S.fFile=file;
    setState({fPreview:preview});
    showToast("✅ แนบรูปเรียบร้อย ("+Math.round(file.size/1024)+"KB)","ok");
  }).catch(err=>{
    console.warn("compress error",err);
    showToast("❌ ไม่สามารถประมวลผลรูปได้ ลองรูปอื่น","warn");
    inp.value="";
  });
}
...
function buildClock(){
  ...
  document.getElementById("clock-hint").textContent="เลือกชั่วโมง (ซ้าย) และนาที (ขวา)";

  // ── แสดงชั่วโมงและนาทีพร้อมกัน 2 คอลัมน์ ──
  const ms=[0,5,10,15,20,25,30,35,40,45,50,55];
  const btnH=(h)=>'<button onclick="pickH('+h+')" style="padding:9px 2px;border-radius:7px;border:1.5px solid '+(_cH===h?"#06C755":"#2a3347")+';background:'+(_cH===h?"#06C755":"#252d3e")+';color:'+(_cH===h?"#000":"#cbd5e0")+';font-size:14px;font-weight:800;cursor:pointer;font-family:monospace;touch-action:manipulation;width:100%">'+h+'</button>';
  const btnM=(m)=>'<button onclick="pickM('+m+')" style="padding:9px 2px;border-radius:7px;border:1.5px solid '+(_cM===m?"#06C755":"#2a3347")+';background:'+(_cM===m?"#06C755":"#252d3e")+';color:'+(_cM===m?"#000":"#cbd5e0")+';font-size:14px;font-weight:800;cursor:pointer;font-family:monospace;touch-action:manipulation;width:100%">'+pad(m)+'</button>';

  let html='<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:6px 0">';
  // คอลัมน์ชั่วโมง
  html+='<div><div style="text-align:center;font-size:11px;font-weight:700;color:#06C755;letter-spacing:1px;margin-bottom:6px">ชั่วโมง</div>';
  html+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px">';
  for(let i=0;i<24;i++)html+=btnH(i);
  html+='</div></div>';
  // คอลัมน์นาที
  html+='<div><div style="text-align:center;font-size:11px;font-weight:700;color:#06C755;letter-spacing:1px;margin-bottom:6px">นาที</div>';
  html+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px">';
  ms.forEach(m=>html+=btnM(m));
  html+='</div></div>';
  html+='</div>';

  face.innerHTML=html;
}
function pickH(h){_cH=h;buildClock()}
...
function exportCSV(){
  ...
  const rows=data.map(t=>[t.job_number||t.id.slice(0,8),t.job_nj||"",t.company||"",_getType(t.type),t.detail||"",t.place||"",t.date||"",t.pickup_time||"",t.deliver_time||"",t.inspect_date||"",t.pay_date||"",{wait:"รอรับ",going:"กำลังวิ่ง",done:"เสร็จ"}[t.status]||"",t.created_by||"",t.messenger_name||"",t.start_time||"",t.end_time||""].map(v=>'"'+String(v).replace(/"/g,'""')+'"'));
...
function exportExcel(){
  ...
  data.forEach(t=>{
    ...
    const row=[
      ...
      t.company||"",                     // E: บริษัท
      (_getType(t.type))+(t.detail?" — "+t.detail:""), // F: ประเภท — รายละเอียด (normalized)
      t.place||"",                       // G: สถานที่
  ...
  const slips=data.map((t,i)=>{
    ...
    return `<div class="slip${i<data.length-1?" page-break":""}">
      ...
      <div class="field-line">
        ...
        <span class="field-val">${esc(_getType(t.type))}${t.detail?" — "+esc(t.detail):""}</span>
      </div>
      <div class="field-line">
...
.sig-name{font-size:11px;color:#1a1f2e;font-weight:600;min-height:14px;margin-top:2px}
...
const PLACES_KEY="msg_places_v7";
function getPlaces(){
...
function buildPlaceHTML(filter){
  ...
  places.forEach(({g,items})=>{
    ...
    if(!items.length){
      ...
      html+='<div onclick="selectPlace(\''+safe+'\');" style="padding:9px 16px;font-size:13px;color:#e2e8f4;cursor:pointer;border-bottom:1px solid #2a3347" onmouseenter="this.style.background=\'rgba(6,199,85,.1)\'" onmouseleave="this.style.background=\'\'" >'+esc(g)+'</div>';
      return;
    }
    const matched=q?items.filter(it=>it.toLowerCase().includes(q)||gLow.includes(q)):items;
    if(!matched.length)return;
    html+='<div style="padding:5px 12px 2px;font-size:10px;color:#475569;font-weight:700;letter-spacing:1px;background:#1e2538;position:sticky;top:0;z-index:1">'+esc(g)+'</div>';
    matched.forEach(it=>{
      const full=g+" : "+it;
      const safe=full.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
      html+='<div onclick="selectPlace(\''+safe+'\');" style="padding:9px 16px;font-size:13px;color:#e2e8f4;cursor:pointer;border-bottom:1px solid #2a3347" onmouseenter="this.style.background=\'rgba(6,199,85,.1)\'" onmouseleave="this.style.background=\'\'" >'+esc(full)+'</div>';
    });
  });
  if(!html){
    ...
    if(val.trim())html+='<div onclick="selectPlace(\''+val.replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\' )" style="padding:9px 16px;font-size:13px;color:#06C755;cursor:pointer;border-top:1px solid #2a3347;font-weight:600" onmouseenter="this.style.background=\'rgba(6,199,85,.08)\'" onmouseleave="this.style.background=\'\'">✚ ใช้ "'+esc(val)+'" (กรอกเอง)</div>';
  }
  html+='<div onclick="setState({page:\'places\'})" style="padding:9px 16px;font-size:12px;color:#3b82f6;cursor:pointer;border-top:1px solid #313d52" onmouseenter="this.style.background=\'rgba(59,130,246,.08)\'" onmouseleave="this.style.background=\'\'">⚙️ จัดการรายการสถานที่</div>';
  return html;
}
...
// ── FIX 2: filterPlaces debounce 150ms (มือถือเก่าพิมพ์ลื่นขึ้น) ──
function _filterPlacesImpl(val){const el=document.getElementById("place-list");if(!el||el.style.display==="none")return;el.innerHTML=buildPlaceHTML(val)}
let _flPlacesTm=null;
function filterPlaces(val){clearTimeout(_flPlacesTm);_flPlacesTm=setTimeout(()=>_filterPlacesImpl(val),150);}
...
function renderPlacePage(){
  ...
  return '<div style="padding:20px 28px 40px;max-width:800px">'
    ...
    +'</div>'
    // add new card
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:14px;padding:18px;margin-bottom:20px">'
...
function renderPlaceList(q){
  ...
  places.forEach((pg,gi)=>{
    ...
    html+='<div style="background:#252d3e;border:1px solid #313d52;border-radius:12px;margin-bottom:10px;overflow:hidden">'
      +'<div style="display:flex;align-items:center;padding:10px 14px;background:#1e2538;gap:8px">'
    ...
    (sq?matched:pg.items).forEach(it=>{
      ...
      html+='<div id="pr-'+gi+'-'+ri+'" style="display:flex;align-items:center;padding:8px 14px;border-top:1px solid #2a3347;gap:8px">'
...
function editItem(gi,ri){
  ...
  const groups=places.map(p=>p.g);
  row.innerHTML='<select id="ei-g" style="background:#1a1f2e;border:1px solid #3b82f6;border-radius:6px;color:#e2e8f4;font-size:12px;padding:5px 8px">'
    +groups.map(g=>'<option value="'+esc(g)+'"'+(g===places[gi].g?' selected':'')+'>'+esc(g)+'</option>').join("")+'</select>'
    +'<input id="ei-n" value="'+esc(oldName)+'" style="flex:1;background:#1a1f2e;border:1px solid #3b82f6;border-radius:6px;color:#e2e8f4;font-size:13px;padding:5px 10px;outline:none;min-width:0" onkeydown="if(event.key===\'Enter\')saveItem('+gi+','+ri+')">'
    ...
    +'<button onclick="renderPlaceList(document.getElementById(\'pmgr-search\')?.value||\'\')" style="background:#222228;border:1px solid #313d52;color:#94a3b8;border-radius:6px;padding:5px 8px;font-size:11px;cursor:pointer">ยกเลิก</button>';
...
function buildCoDropHTML(q){
  ...
  matched.slice(0,40).forEach(c=>{
    ...
    html+='<div onclick="toggleCoTag(\''+c.replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')" style="padding:8px 14px;font-size:13px;cursor:pointer;border-bottom:1px solid #2a3347;display:flex;align-items:center;gap:8px;'+(isSel?"background:rgba(168,85,247,.08)":"")+'"'
      +' onmouseenter="this.style.background=\'rgba(168,85,247,.1)\'" onmouseleave="this.style.background=\''+(isSel?"rgba(168,85,247,.08)":"")+'\';">'
      +(isSel?'<span style="color:#c084fc;font-size:14px">✓</span>':'<span style="color:#313d52;font-size:14px">○</span>')
  ...
  if(sq&&!list.includes(q)){
    ...
    if(val)html+='<div onclick="addNewCustomer(\''+val.replace(/'/g,"\\'")+'\')" style="padding:9px 14px;font-size:13px;color:#c084fc;cursor:pointer;border-top:1px solid #313d52;font-weight:600"'
      +' onmouseenter="this.style.background=\'rgba(168,85,247,.08)\'" onmouseleave="this.style.background=\'\'">✚ เพิ่ม "'+esc(val)+'" ลงรายการ</div>';
  }
  html+='<div onclick="openCustomerMgr()" style="padding:8px 14px;font-size:12px;color:#3b82f6;cursor:pointer;border-top:1px solid #313d52"'
...
// ── FIX 2: filterCo debounce 150ms ──
function _filterCoImpl(val){
  const el=document.getElementById("co-dropdown");
  if(!el||el.style.display==="none")return;
  el.innerHTML=buildCoDropHTML(val);
}
let _flCoTm=null;
function filterCo(val){clearTimeout(_flCoTm);_flCoTm=setTimeout(()=>_filterCoImpl(val),150);}
...
function openCustomerMgr(){
  ...
  const html='<div id="cmgr-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:600;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)" onclick="if(event.target===this)closeCustomerMgr()">'
    +'<div style="background:#252d3e;border-radius:18px;width:560px;max-width:96vw;max-height:88vh;display:flex;flex-direction:column;border:1px solid #313d52">'
    +'<div style="padding:16px 20px;border-bottom:1px solid #313d52;display:flex;align-items:center;gap:10px">'
    ...
    +'</div>'
    +'<div style="padding:12px 20px;border-bottom:1px solid #2a3347">'
    ...
    +'<div style="padding:14px 20px;border-top:1px solid #313d52;background:#1e2538;border-radius:0 0 18px 18px">'
...
function renderCmgrList(q){
  ...
  el.innerHTML=items.map(({c,i})=>
    '<div id="ci-'+i+'" style="display:flex;align-items:center;padding:7px 20px;border-bottom:1px solid #2a3347;gap:8px">'
...
function editCustomerItem(idx){
  ...
  row.innerHTML='<input id="ci-inp-'+idx+'" value="'+esc(list[idx])+'" style="flex:1;background:#1a1f2e;border:1px solid #3b82f6;border-radius:6px;color:#e2e8f4;font-size:13px;padding:5px 10px;outline:none;min-width:0" onkeydown="if(event.key===\'Enter\')saveCustomerItem('+idx+')">'
    ...
    +'<button onclick="renderCmgrList(document.getElementById(\'cmgr-search\')?.value||\'\')" style="background:#222;border:1px solid #313d52;color:#94a3b8;border-radius:6px;padding:4px 8px;font-size:11px;cursor:pointer;margin-left:4px">ยกเลิก</button>';
...
// ── Detail List ──
const DETAILS_DEFAULT=["ส่งชุดงานตรวจปล่อย","ส่งชุดงานตรวจปล่อย+โอที","ส่งดีโอ","รับดีโอ","วางบิล","การ์ดใส่กล่อง","ส่งเอกสาร","รับเอกสาร","ใส่กล่องกลางทุ่ง(แดง)","ใส่กล่องกลางทุ่ง(เหลือง)","ใส่กล่องด่าน","ใส่กล่องประตู3","ส่งเอกสารบนด่าน","ชำระค่าภาษี","ชำระค่าธรรมเนียม","ขอล่วงเวลา(โอที)","รับเช็ค"];
const DETAILS_KEY="msg_details_v3";
...
function buildDetailHTML(q){
  ...
  let html="";
  matched.forEach(d=>{
    html+='<div onclick="selectDetail(\''+d.replace(/'/g,"&#39;")+'\')" style="padding:9px 16px;font-size:13px;color:#e2e8f4;cursor:pointer;border-bottom:1px solid #2a3347"'
  ...
  if(!html){
    ...
    html='<div style="padding:10px 14px;font-size:12px;color:#64748b">ไม่พบในรายการ</div>';
    if(val.trim())html+='<div onclick="selectDetail(\''+val.replace(/'/g,"&#39;")+'\')" style="padding:9px 16px;font-size:13px;color:#3b82f6;cursor:pointer;border-top:1px solid #2a3347;font-weight:600"'
      +' onmouseenter="this.style.background=\'rgba(59,130,246,.08)\'" onmouseleave="this.style.background=\'\'">✚ ใช้ "'+esc(val)+'"</div>';
  }
  html+='<div onclick="openDetailMgr()" style="padding:9px 16px;font-size:12px;color:#3b82f6;cursor:pointer;border-top:1px solid #313d52"'
...
// ── FIX 2: filterDetails debounce 150ms ──
function _filterDetailsImpl(val){
  const el=document.getElementById("detail-list");
  if(!el||el.style.display==="none")return;
  el.innerHTML=buildDetailHTML(val);
}
let _flDetailsTm=null;
function filterDetails(val){clearTimeout(_flDetailsTm);_flDetailsTm=setTimeout(()=>_filterDetailsImpl(val),150);}
...
function openDetailMgr(){
  ...
  const html='<div id="dmgr-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:600;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px)" onclick="if(event.target===this)closeDetailMgr()">'
    +'<div style="background:#252d3e;border-radius:18px;width:480px;max-width:96vw;max-height:85vh;display:flex;flex-direction:column;border:1px solid #313d52">'
    +'<div style="padding:16px 20px;border-bottom:1px solid #313d52;display:flex;align-items:center;gap:10px">'
...
function renderDmgrList(q){
  ...
  el.innerHTML=items.map(({d,i})=>{
    ...
    return '<div id="di-'+realIdx+'" style="display:flex;align-items:center;padding:8px 20px;border-bottom:1px solid #2a3347;gap:8px">'
...
function editDetailItem(idx){
  ...
  const old=list[idx];
  row.innerHTML='<input id="di-inp-'+idx+'" value="'+esc(old)+'" style="flex:1;background:#1a1f2e;border:1px solid #3b82f6;border-radius:6px;color:#e2e8f4;font-size:13px;padding:5px 10px;outline:none;min-width:0"'
    ...
    +'<button onclick="renderDmgrList(document.getElementById(\'dmgr-search\')?.value||\'\')" style="background:#222228;border:1px solid #313d52;color:#94a3b8;border-radius:6px;padding:4px 8px;font-size:11px;cursor:pointer;margin-left:4px">ยกเลิก</button>';
...
function _getDashTasks(){
  const from=S.dashDateFrom, to=S.dashDateTo;
  if(!from&&!to)return S.tasks;
  return S.tasks.filter(t=>{
    // ใช้ t.date (วันที่ทำงาน) เป็นหลัก, fallback: created_at
...
function setDashDate(from,to){
  setState({dashDateFrom:from||"",dashDateTo:to||""});
}
function dashQuickRange(days){
  // days=0 → ล้าง, days=1 → วันนี้, days=N → N วันย้อนหลัง
  if(days===0){setDashDate("","");return;}
  const toD=new Date();
  const fromD=new Date();fromD.setDate(fromD.getDate()-(days-1));
  const toS=toD.getFullYear()+"-"+pad(toD.getMonth()+1)+"-"+pad(toD.getDate());
  const frS=fromD.getFullYear()+"-"+pad(fromD.getMonth()+1)+"-"+pad(fromD.getDate());
  setDashDate(frS,toS);
}

function renderDashboard(){
  const _all=_getDashTasks();
  const done=_all.filter(t=>t.status==="done");
  const going=_all.filter(t=>t.status==="going").length;
  const wait=_all.filter(t=>t.status==="wait").length;
  const delivered=_all.filter(t=>t.status==="delivered").length;
  // แยกตาม close_mode
  ...
  // ── สร้าง days[] จาก date range ที่เลือก (default: 7 วันล่าสุด) ──
  const days={};
  const MAX_DAYS=31; // กันกราฟแน่นเกินไป
  let dFrom,dTo;
  if(S.dashDateFrom&&S.dashDateTo){
    dFrom=new Date(S.dashDateFrom);dTo=new Date(S.dashDateTo);
  }else if(S.dashDateFrom){
    dFrom=new Date(S.dashDateFrom);dTo=new Date();
  }else if(S.dashDateTo){
    dTo=new Date(S.dashDateTo);dFrom=new Date(S.dashDateTo);dFrom.setDate(dFrom.getDate()-6);
  }else{
    dTo=new Date();dFrom=new Date();dFrom.setDate(dFrom.getDate()-6);
  }
  // cap ที่ MAX_DAYS
  const diffDays=Math.floor((dTo-dFrom)/86400000);
  if(diffDays>MAX_DAYS){dFrom=new Date(dTo);dFrom.setDate(dFrom.getDate()-MAX_DAYS+1);}
  ...
  const msgRows=Object.entries(byMsg).sort((a,b)=>b[1].done-a[1].done).map(([m,v])=>{
    ...
    return '<tr style="border-bottom:1px solid #2a3347"><td style="padding:10px 12px;font-weight:700;color:#e2e8f4">🏃 '+esc(m)+'</td><td style="padding:10px 12px;text-align:center;font-family:monospace;font-size:18px;font-weight:800;color:#06C755">'+v.done+'</td><td style="padding:10px 12px">'+topRecip+'</td></tr>';
  }).join("");

  // ── Date-filter bar ──
  const hasFilter=!!(S.dashDateFrom||S.dashDateTo);
  const rangeLabel=hasFilter
    ? (S.dashDateFrom&&S.dashDateTo?(esc(S.dashDateFrom)+" → "+esc(S.dashDateTo))
      :S.dashDateFrom?("ตั้งแต่ "+esc(S.dashDateFrom)):("ถึง "+esc(S.dashDateTo)))
    : "7 วันล่าสุด";
  const dailyTitle="📈 งานเสร็จ "+(hasFilter?"ช่วง "+rangeLabel:"7 วันล่าสุด");
  const dateBar='<div style="background:#252d3e;border:1px solid #313d52;border-radius:12px;padding:12px 14px;margin-bottom:14px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">'
    +'<span style="font-size:12px;font-weight:700;color:#06C755;letter-spacing:.5px;margin-right:4px">📅 ช่วงวันที่</span>'
    +'<input type="date" id="dash-from" value="'+esc(S.dashDateFrom)+'" onchange="setDashDate(this.value,document.getElementById(\'dash-to\').value)" style="background:#1a1f2e;border:1.5px solid #313d52;border-radius:8px;padding:6px 8px;color:#e2e8f4;font-size:12px;outline:none;color-scheme:dark;font-family:Sarabun,sans-serif">'
    +'<span style="color:#64748b;font-size:12px">ถึง</span>'
    +'<input type="date" id="dash-to" value="'+esc(S.dashDateTo)+'" onchange="setDashDate(document.getElementById(\'dash-from\').value,this.value)" style="background:#1a1f2e;border:1.5px solid #313d52;border-radius:8px;padding:6px 8px;color:#e2e8f4;font-size:12px;outline:none;color-scheme:dark;font-family:Sarabun,sans-serif">'
    +'<button onclick="dashQuickRange(1)" style="background:'+(hasFilter&&S.dashDateFrom===S.dashDateTo&&S.dashDateFrom===todayStr()?"#06C755":"rgba(6,199,85,.1)")+';border:1.5px solid rgba(6,199,85,.4);border-radius:8px;padding:6px 10px;color:'+(hasFilter&&S.dashDateFrom===S.dashDateTo&&S.dashDateFrom===todayStr()?"#fff":"#06C755")+';font-size:12px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">วันนี้</button>'
    +'<button onclick="dashQuickRange(7)" style="background:rgba(59,130,246,.08);border:1.5px solid rgba(59,130,246,.35);border-radius:8px;padding:6px 10px;color:#60a5fa;font-size:12px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">7 วัน</button>'
    +'<button onclick="dashQuickRange(30)" style="background:rgba(168,85,247,.08);border:1.5px solid rgba(168,85,247,.35);border-radius:8px;padding:6px 10px;color:#c084fc;font-size:12px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">30 วัน</button>'
    +'<button onclick="dashQuickRange(0)" style="background:none;border:1.5px solid #313d52;border-radius:8px;padding:6px 10px;color:#64748b;font-size:12px;font-weight:700;cursor:pointer;font-family:Sarabun,sans-serif">ทั้งหมด</button>'
    +'<span style="margin-left:auto;font-size:11px;color:#64748b">'
    +(hasFilter?'🔍 <strong style="color:#06C755">'+_all.length+'</strong> ใบงาน ใน '+rangeLabel
      :'📋 รวม <strong style="color:#06C755">'+S.tasks.length+'</strong> ใบงาน')
    +'</span></div>';
  ...
  return '<div style="padding:20px 32px 40px">'
    ...
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#06C755;font-family:monospace">'+done.length+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">เสร็จทั้งหมด</div></div>'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#3b82f6;font-family:monospace">'+going+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">กำลังวิ่ง</div></div>'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#f59e0b;font-family:monospace">'+wait+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">รอรับงาน</div></div>'
    +'</div>'
    +'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px">'
    +'<div style="background:#252d3e;border:1px solid rgba(245,158,11,.3);border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#f59e0b;font-family:monospace">'+delivered+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">📦 รอผู้สั่งยืนยัน</div></div>'
    +'<div style="background:#252d3e;border:1px solid rgba(6,199,85,.2);border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#06C755;font-family:monospace">'+doneByMessenger+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">🟢 จบโดยแมส</div></div>'
    +'<div style="background:#252d3e;border:1px solid rgba(99,102,241,.2);border-radius:12px;padding:14px;text-align:center"><div style="font-size:26px;font-weight:800;color:#818cf8;font-family:monospace">'+doneByUser+'</div><div style="font-size:11px;color:#64748b;margin-top:2px">🟡 จบโดยผู้สั่ง</div></div>'
    +'</div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:14px;padding:16px"><div style="font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:1px;margin-bottom:12px">📊 งานเสร็จแยกตามแมส</div><div style="position:relative;height:220px"><canvas id="chart-msg"></canvas></div></div>'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:14px;padding:16px"><div style="font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:1px;margin-bottom:12px">'+dailyTitle+'</div><div style="position:relative;height:220px"><canvas id="chart-daily"></canvas></div></div>'
    +'</div>'
    +'<div style="display:grid;grid-template-columns:280px 1fr;gap:16px;margin-bottom:24px">'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:14px;padding:16px"><div style="font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:1px;margin-bottom:12px">🍩 สัดส่วนสถานะ</div><div style="position:relative;height:200px"><canvas id="chart-status"></canvas></div></div>'
    +'<div style="background:#252d3e;border:1px solid #313d52;border-radius:14px;padding:16px;overflow:auto"><div style="font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:1px;margin-bottom:12px">🏆 แมสแยกผู้รับ</div>'
    +(msgRows?'<table style="width:100%;border-collapse:collapse"><tr style="border-bottom:1px solid #313d52"><th style="padding:8px 12px;text-align:left;font-size:11px;color:#64748b;font-weight:700">แมส</th><th style="padding:8px 12px;text-align:center;font-size:11px;color:#64748b;font-weight:700">งานเสร็จ</th><th style="padding:8px 12px;text-align:left;font-size:11px;color:#64748b;font-weight:700">ผู้รับที่พบ</th></tr>'+msgRows+'</table>'
...
function renderCharts(){
  ...
  setTimeout(()=>{
    try{
      ...
      // ── FIX MEMORY LEAK: destroy existing chart instances ก่อนสร้างใหม่ ──
      if(!window._chartInst)window._chartInst={msg:null,daily:null,status:null};
      const ci=window._chartInst;
      if(ci.msg){try{ci.msg.destroy();}catch{}ci.msg=null;}
      if(ci.daily){try{ci.daily.destroy();}catch{}ci.daily=null;}
      if(ci.status){try{ci.status.destroy();}catch{}ci.status=null;}
      const c1=document.getElementById("chart-msg");
      if(c1)ci.msg=new Chart(c1,{type:"bar",data:{labels:msgs,datasets:[{data:msgVals,backgroundColor:msgs.map((_,i)=>colors[i%colors.length]),borderRadius:6,borderSkipped:false}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:"#94a3b8",font:{family:"Sarabun",size:12}},grid:{display:false}},y:{ticks:{color:"#94a3b8"},grid:{color:"rgba(255,255,255,.05)"},beginAtZero:true}}}});
      ...
      if(c2){
        ...
        ci.daily=new Chart(c2,{type:"line",data:{labels:dLabels,datasets:[{data:Object.values(days),borderColor:"#06C755",backgroundColor:"rgba(6,199,85,.08)",tension:.4,fill:true,pointBackgroundColor:"#06C755",pointRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:"#94a3b8",font:{family:"Sarabun",size:11}},grid:{display:false}},y:{ticks:{color:"#94a3b8"},grid:{color:"rgba(255,255,255,.05)"},beginAtZero:true}}}});
      }
      const c3=document.getElementById("chart-status");
      if(c3){
        ci.status=new Chart(c3,{type:"doughnut",data:{labels:["รอรับงาน","กำลังวิ่ง","ส่งถึงแล้ว","เสร็จแล้ว"],datasets:[{data:[wait,going,delivered,done],backgroundColor:["#f59e0b","#3b82f6","#f97316","#06C755"],borderColor:"#252d3e",borderWidth:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom",labels:{color:"#94a3b8",font:{family:"Sarabun",size:11},padding:12}}},cutout:"65%"}});
      }
...
// ── WebSocket connection lock (กัน startRealtime() เรียกซ้ำใน race condition) ──
let _wsConnecting=false;

function startRealtime(){
  if(DEMO||!S.currentUser)return;
  // ── Lock guard: ถ้ากำลัง connect อยู่ → skip (กัน double connect ที่ทำให้ server overload) ──
  if(_wsConnecting)return;
  // ── ถ้ามี WSS live อยู่แล้ว → ไม่ต้องสร้างใหม่ (เพิ่มจากเดิมที่ check แค่ _realtimeChannel ไม่พอ) ──
  if(_realtimeChannel&&_realtimeChannel.readyState===WebSocket.OPEN&&S.realtimeStatus==="live")return;
  _wsConnecting=true;
  if(_realtimeChannel){
  ...
  if(!_autoPollInterval){
    _autoPollInterval=setInterval(async()=>{
      ...
      if(document.hidden)return; // pause when tab hidden — ประหยัด battery + network
      const isLive=S.realtimeStatus==="live";
      // ✨ PRIORITY 2: Offline fallback ลดเหลือ 8s (จาก 12s) — user เห็นข้อมูลใหม่เร็วขึ้น 4 วิ
      // Live: 55s ตามเดิม (realtime คอย sync อยู่แล้ว)
      const minGap=isLive?55000:8000;
      if(Date.now()-_lastFetch<minGap)return; // ยังไม่ถึงเวลา
      await loadTasks();
      if(S.realtimeStatus!=="live"){invalidateZone("zone-hdr");render();}
    },10000); // tick ทุก 10 วิ (จาก 20s — ตรวจ gap บ่อยขึ้น แต่ยัง throttle ด้วย minGap)
  }
  ...
  ws.onopen=()=>{
    _wsConnecting=false; // release lock — connection established
    ...
    const wasOffline=(S.realtimeStatus!=="live");
    S.realtimeStatus="live";
    invalidateZone("zone-hdr");
    render();
    showToast("📡 Realtime เชื่อมต่อแล้ว","ok");
    // ✨ PRIORITY 1: Catch-up sync — ถ้า WSS เคยหลุด + fetch เก่ากว่า 10 วิ → fetch ทันที
    // (ปิด gap ระหว่าง WSS หลุด ที่ event อาจหายไป — ไม่ต้องรอ polling tick 20s)
    if(wasOffline&&_lastFetch>0&&Date.now()-_lastFetch>10000){
      // silent fetch — ไม่ block UI, ไม่ toast
      loadTasks().catch(e=>console.warn("catch-up sync failed",e));
    }
  };
      ...
      if(msg.event==="postgres_changes"||msg.payload?.data?.type){
        ...
        if(type==="INSERT"&&record){
          if(!S.tasks.find(t=>t.id===record.id)){
            // FINAL PATCH (ASC sort): task ใหม่ created_at มาก → ไปอยู่ล่างสุด → append
            S.tasks.push(record);
            invalidateDerived();invalidateCardCache(record.id);
            _postUpdateCleanup(); // mobile cap (Android safety)
            // ── INSERT: ไม่ DOM prepend อีก (ASC) — ปล่อย VL จัดวาง ──
            if(S.page==="list"){
              VL._fil=null; // force VL recalc next render
              const shouldShow=_isTaskVisibleInCurrentFilter(record);
              if(shouldShow){
                // Trigger VL re-render รอบเดียว → insert DOM ที่ตำแหน่งถูก
                requestAnimationFrame(()=>{
                  const d=computeDerived();
                  if(d.fil&&d.fil.length)vlUpdate(d.fil,false);
                });
              }
              // Update counts เสมอ (wait/going/delivered เปลี่ยน)
              updateZoneCounts();
            }else{
              updateZoneCounts(); // ไม่ใช่ list page → แค่ update counts
            }
          ...
          if(existed){
            ...
            // patchCard = update DOM ถ้า card อยู่ใน VL window เท่านั้น
            const patched=patchCard(record.id);
            // ไม่ว่า card อยู่ใน DOM หรือไม่ → update counts เสมอ (wait/going counter เปลี่ยน)
            updateZoneCounts();
            if(patched){
            ...
            // ถ้า card อยู่นอก DOM window → ไม่ full render
            // VL จะ refresh เองตอน scroll หรือ render รอบถัดไปเมื่อ user interact
          ...
          invalidateDerived();invalidateCardCache(old.id);
          _pruneCardCache();
          if(S.selId===old.id){setState({selId:null});return;}
          // Remove card element if currently in DOM window + drop from node map
          const node=VL._nodes?.get(old.id);
          if(node){node.remove();VL._nodes.delete(old.id);}
          VL._fil=null; // force VL recalc next scroll (window bounds changed)
          updateZoneCounts(); // count เปลี่ยนเสมอ ไม่ต้อง full render
        }
      }
  ...
  ws.onerror=()=>{
    _wsConnecting=false; // release lock — connection failed
  ...
  ws.onclose=()=>{
    _wsConnecting=false; // release lock — connection closed
    S.realtimeStatus="offline";
    invalidateZone("zone-hdr");
    updateZoneCounts(); // patch header only — ไม่ full render (ลด jank บนมือถือเก่า)
    // Exponential backoff: 5s → 10s → 20s → 40s → max 60s
    // + Random jitter 0-2000ms กัน 70 users reconnect พร้อมกัน (thundering herd)
    const jitter = Math.random() * 2000;
    setTimeout(()=>{if(S.currentUser)startRealtime();},_reconnectDelay+jitter);
...
function stopRealtime(){
  ...
  // ── FIX 3: cleanup intervals ที่อาจค้าง (ป้องกัน accumulation บน long session) ──
  if(window._finishClock){clearInterval(window._finishClock);window._finishClock=null;}
  if(window._detailPoll){clearInterval(window._detailPoll);window._detailPoll=null;}
  if(window._notifAutoDismiss){clearTimeout(window._notifAutoDismiss);window._notifAutoDismiss=null;}
}
...
document.addEventListener("visibilitychange",()=>{
  if(document.hidden){
    ...
    // Tab กลับมา → reconnect realtime เฉพาะเมื่อหลุด
    _notifDismissed=false;
    if(S.realtimeStatus!=="live"){
...
window.addEventListener("focus",()=>{
  ...
  const now=Date.now();
  // ── Throttle 30 วิ (เดิม 15) — กัน user สลับ tab บ่อยทำให้ fetch ทุก focus ──
  if(now-_focusThrottle<30000)return;
  _focusThrottle=now;
  _notifDismissed=false;
  // ── Smart refresh: skip loadTasks ถ้า realtime live (WSS sync อยู่แล้ว ไม่ต้อง fetch) ──
  // โหลดใหม่เฉพาะเมื่อ:
  //   (1) realtime หลุด (offline) AND
  //   (2) fetch ล่าสุดเก่ากว่า 30 วิ
  if(!document.hidden){
    const isLive=S.realtimeStatus==="live";
    const stale=(now-_lastFetch)>30000;
    if(!isLive&&stale){
      loadTasks(true); // reload เฉพาะเคสที่จำเป็นจริงๆ
    }
  }
  setTimeout(checkAndShowNotif,800);
...
let _scrollAttachedEl=null; // element ที่แนบ listener อยู่ปัจจุบัน (prevent dup)
function _onScroll(){
  _isScrolling=true;
...
// ── Single-source scroll attach: ถ้าแนบที่ element เดิมอยู่แล้ว → no-op ──
function attachScrollListener(el){
  if(!el||_scrollAttachedEl===el)return; // idempotent — กัน listener ซ้อน
  if(_scrollAttachedEl){
    try{_scrollAttachedEl.removeEventListener("scroll",_onScroll);}catch{}
  }
  el.addEventListener("scroll",_onScroll,{passive:true});
  _scrollAttachedEl=el;
}
// Initial attach: ลอง .main-col ก่อน ตกไป window
(function _initScroll(){
  const sc=document.querySelector(".main-col");
  attachScrollListener(sc||window);
})();
// ไม่แนบ document scroll อีกแล้ว — _attachScroll + .main-col ครอบคลุมแล้ว
// (ก่อนหน้านี้แนบ 3 ที่ — scroll 1 ครั้ง → ยิง handler 3 รอบ)
