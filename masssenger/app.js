    ...
    const doCompress=(bitmap)=>{
      try{
        ...
        canvas.toBlob(blob=>{
          ...
          const compressed=new File([blob],"image.jpg",{type:"image/jpeg",lastModified:Date.now()});
          const r2=new FileReader();
          r2.onload=ev=>resolve({file:compressed,preview:ev.target.result});
          r2.onerror=()=>reject(new Error("filereader_preview_failed"));
...
async function _compressAdaptive(file){
  ...
  // รอบ 1: คุณภาพปกติ
  let r=await compressImage(file,900,0.65);
  if(r.file.size<=TARGET)return r;
  // รอบ 2: ลด quality
  r=await compressImage(file,900,0.55);
  if(r.file.size<=TARGET)return r;
  // รอบ 3: ลด resolution
  r=await compressImage(file,800,0.5);
  if(r.file.size<=TARGET)return r;
  // รอบ 4: ลดลงอีก
  r=await compressImage(file,720,0.45);
  if(r.file.size<=TARGET)return r;
  // รอบ 5: บีบหนัก
  r=await compressImage(file,640,0.4);
  if(r.file.size<=TARGET)return r;
  // รอบ 6: บีบแน่น
  r=await compressImage(file,560,0.35);
  if(r.file.size<=TARGET)return r;
  // รอบ 7: สุดท้าย — บีบหนักที่สุด (อ่านยังออก แต่เล็กแน่นอน)
...
function handleFinishFile(inp){
  ...
  _compressAdaptive(f).then(({file,preview})=>{
    setState({finishFile:file,finishPreview:preview});
    showToast("✅ แนบรูปเรียบร้อย ("+Math.round(file.size/1024)+"KB)","ok");
    _resetInp();
  }).catch(err=>{
    console.warn("compress error",err);
    // Fallback: ถ้าไฟล์เดิมไม่ใหญ่มาก (<= 5MB) → ใช้ original ไป upload เลย
    if(f.size<=5*1024*1024){
      const r=new FileReader();
      r.onload=ev=>{
        setState({finishFile:f,finishPreview:ev.target.result});
        showToast("⚠️ ใช้รูปต้นฉบับ ("+Math.round(f.size/1024)+"KB)","warn");
        _resetInp();
      };
      r.onerror=()=>{
        showToast("❌ อ่านไฟล์รูปไม่ได้ ลองรูปอื่น","warn");_resetInp();
      };
      r.readAsDataURL(f);
    }else{
...
// ── Close Finish Modal — central cleanup (clear timer + reset state) ──
...
async function uploadFinishImage(id,file,customTime){
  ...
  const fname="finish_"+id+"_"+Date.now()+".jpg";
  const url=SUPA_URL+"/storage/v1/object/msg-images/"+fname;
  const publicUrl=SUPA_URL+"/storage/v1/object/public/msg-images/"+fname;
  const _doUpload=async(attempt)=>{
    const ctrl=new AbortController();
  ...
  try{
    let res;
    try{
      res=await _doUpload(1);
    }catch(err){
      // network error / timeout → retry 1 ครั้ง
      console.warn("upload attempt 1 failed",err);
      showToast("⏳ ลองใหม่...","warn");
      res=await _doUpload(2);
    }
    if(res.ok){
      finishTask(id,publicUrl,customTime); // finishTask จะ reset state+ uploadingFinish
      return;
    }
    // 5xx → retry 1 ครั้ง
    if(res.status>=500&&res.status<600){
      ...
      const res2=await _doUpload(2);
      if(res2.ok){finishTask(id,publicUrl,customTime);return;}
      const t2=await res2.text().catch(()=>res2.statusText);
      console.warn("upload failed",res2.status,t2);
      showToast("❌ อัปโหลดไม่สำเร็จ ("+res2.status+") ลองใหม่","warn");
    }else if(res.status===401||res.status===403){
      showToast("❌ Storage ปฏิเสธสิทธิ์ ("+res.status+") - ตรวจ bucket policy","warn");
    }else if(res.status===413){
      showToast("❌ รูปใหญ่เกินไป (413) - ลองรูปอื่น","warn");
    }else{
      const errText=await res.text().catch(()=>res.statusText);
      console.warn("upload failed",res.status,errText);
      showToast("❌ อัปโหลดไม่สำเร็จ ("+res.status+") กรุณาลองใหม่","warn");
    }
  }catch(err){
    console.warn("upload error",err);
    if(err.name==="AbortError"){
      showToast("❌ อัปโหลดหมดเวลา 30 วิ - ตรวจสอบเน็ต/ลองใหม่","warn");
    }else{
      showToast("❌ อัปโหลดไม่สำเร็จ — ตรวจสอบเน็ต/ลองใหม่","warn");
    }
  }finally{
...
async function uploadSignatureImage(id,dataURL,customTime){
  ...
  const fname="sign_"+id+"_"+Date.now()+".png";
  const url=SUPA_URL+"/storage/v1/object/msg-images/"+fname;
  const publicUrl=SUPA_URL+"/storage/v1/object/public/msg-images/"+fname;
  let blob,mime;
  try{
    ...
    console.warn("signature parse error",e);
  ...
  try{
    ...
    try{res=await _doUpload();}
      ...
      console.warn("sig upload attempt 1 failed",e);
      showToast("⏳ ลองใหม่...","warn");
      res=await _doUpload();
    }
    ...
    if(res.status>=500&&res.status<600){
      ...
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ ("+r2.status+") ลองใหม่","warn");
    }else if(res.status===401||res.status===403){
      showToast("❌ Storage ปฏิเสธสิทธิ์ ("+res.status+") - ตรวจ bucket policy","warn");
    }else if(res.status===413){
      ...
      const errText=await res.text().catch(()=>res.statusText);
      console.warn("sig upload failed",res.status,errText);
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ ("+res.status+") กรุณาลองใหม่","warn");
    }
  }catch(err){
    console.warn("sig upload error",err);
    if(err.name==="AbortError"){
      showToast("❌ อัปโหลดหมดเวลา 30 วิ - ตรวจสอบเน็ต/ลองใหม่","warn");
    }else{
      showToast("❌ บันทึกลายเซ็นไม่สำเร็จ — ตรวจสอบเน็ต/ลองใหม่","warn");
    }
  }finally{
    setState({uploadingFinish:false});
  }
