...
const Data = {
  ...
  // attendance วันนี้ของ "ผู้ใช้ที่ล็อกอิน" (อ่านอย่างเดียว — สำหรับการ์ดเข้า/ออกงาน)
  // คืน { check_in, check_out, status, place } | null  (DEMO/ไม่มีข้อมูล -> null)
  async myAttendanceToday() {
    if (USE_DEMO || !sb) return null;
    try {
      let employee_id = null;
      if (Auth.current?.emp_code) {
        const { data: emp } = await sb.from('employees').select('id').eq('emp_code', Auth.current.emp_code).maybeSingle();
      ...
      const { data } = await sb.from('attendance')
        .select('check_in,check_out,status,place').eq('employee_id', employee_id).eq('work_date', today).maybeSingle();
      return data || null;
    } catch (_) { return null; }
  },
