...
def resolve_country(code: str) -> str:
    ...
    Handles split-country suffixes from the Mapping Excel mode where countries
    with many billing documents are split into multiple sheets:
        "CN"   → "MADE IN CHINA"
        "CN-2" → "MADE IN CHINA-2"
        "DE-3" → "MADE IN GERMANY-3"
    """
    c = str(code).strip().upper()
    # Detect split suffix like "-2", "-3", ...
    suffix = ""
    base = c
    m = re.match(r"^([A-Z]{2,3})(-\d+)$", c)
    if m:
        base, suffix = m.group(1), m.group(2)
    name = COUNTRY_MAP.get(base, base)     # fallback: use value as-is if not in map
    return f"MADE IN {name}{suffix}"
...
def parse_summary(excel_bytes: bytes, filename: str) -> dict:
    ...
    for _, row in data_df.iterrows():
        ...
        # Strip split-suffix like "-2" so CN and CN-2 both map to CHINA (once).
        code_upper = country_code.upper()
        m = re.match(r"^([A-Z]{2,3})(-\d+)$", code_upper)
        base_code = m.group(1) if m else code_upper
        full = COUNTRY_MAP.get(base_code, base_code)
        if full not in seen:
            seen.add(full)
...
def find_origin_table(doc: Document):
    ...
    ORIGIN, QTY, N.W., G.W., FOB USD (case-insensitive, space/separator
    tolerant). Real templates write the qty header as "QTY/PCS" or
    "QTY/ PCS", so matching is by despaced-uppercase CONTAINMENT, not exact
    equality. Returns the table or None.
    """
    def _key(s):
        return "".join(str(s).split()).upper()   # drop all whitespace, upper
    targets = ["ORIGIN", "QTY", "N.W.", "G.W.", "FOB USD"]
    tkeys = [_key(t) for t in targets]
    for table in doc.tables:
        if not table.rows:
            continue
        cell_keys = [_key(c.text) for c in table.rows[0].cells]
        # every required header must be a substring of SOME header cell
        if all(any(tk in ck for ck in cell_keys) for tk in tkeys):
            return table
    return None
...
def _get_col_index(header_row, col_name: str) -> int:
    """
    Return index of the header cell that matches col_name. Match is
    despaced-uppercase: exact first, else containment (so "QTY" resolves to
    a "QTY/ PCS" header). Returns -1 if not found.
    """
    def _key(s):
        return "".join(str(s).split()).upper()
    want = _key(col_name)
    keys = [_key(c.text) for c in header_row.cells]
    for i, k in enumerate(keys):          # exact match first
        if k == want:
            return i
    for i, k in enumerate(keys):          # then containment
        if want and want in k:
            return i
    return -1
...
def _remove_body_pictures(doc: Document) -> int:
    """
    Remove every picture in the document BODY only — i.e. the signature and
    company-stamp images the master template embeds as floating drawings.

    Scope is deliberately narrow and non-destructive:
      • Only <w:drawing> and legacy <w:pict> elements inside <w:body> are
        removed (plus the wrapping <w:r> run if it becomes empty).
      • Headers, footers, all paragraphs, runs, tables, placeholders and any
        other content are NOT touched.
      • The image files stay in the package as harmless orphans; Word simply
    ...
    removed = 0
    # qualified-name xpath via python-docx's docx nsmap
    for pic in list(body.xpath('.//w:drawing')) + list(body.xpath('.//w:pict')):
        run = pic.getparent()
        pic.getparent().remove(pic)
        removed += 1
        # tidy up an empty run that only held the picture
        if run is not None and run.tag == qn('w:r') and len(run) == 0:
...
def _clear_highlights(doc: Document) -> int:
    """
    Remove ALL text highlighting (the yellow marker) everywhere in the
    document — body, tables, headers and footers. Only the <w:highlight>
    run-property element is deleted; text, fonts, sizes and every other
    formatting attribute are left exactly as-is. Returns count removed.
    """
...
def generate_letter(
    ...
    # 4. จำนวน — left BLANK intentionally (per request). The total quantity
    # is NOT written into the body sentence; "จำนวน" keeps the template's
    # original empty value. (Rule removed deliberately — do not re-add.)

    # 4b. น้ำหนัก G.W <value> KGM — ensure single space before KGM
    if gw_total:
        replacements.append((
    ...
    # Step 6 — REMOVE signature + company stamp from the letter BODY.
    # The master template embeds them as floating images in the body; this
    # strips ONLY body pictures (<w:drawing>/<w:pict>). Header/footer graphics
    # (letterhead) and all text/tables/placeholders are left completely
    # untouched. Backend signature/stamp injection also stays disabled.
    # inject_signature_and_stamp(doc)   # (kept defined, intentionally not called)
    _remove_body_pictures(doc)
    _clear_highlights(doc)              # remove the yellow marker — no fill colour
